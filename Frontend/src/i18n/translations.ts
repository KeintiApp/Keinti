export const SUPPORTED_LANGUAGES = ['es', 'en', 'fr', 'pt', 'de', 'it'] as const;

export type Language = (typeof SUPPORTED_LANGUAGES)[number];

export type TranslationKey =
  | 'language.label'
  | 'language.spanish'
  | 'language.english'
  | 'language.french'
  | 'language.portuguese'
  | 'language.german'
  | 'language.italian'
  | 'gender.male'
  | 'gender.female'
  | 'gender.unspecified'
  | 'common.loading'
  | 'common.logout'
  | 'common.notifications'
  | 'common.keys'
  | 'common.or'
  | 'common.unlock'
  | 'common.view'
  | 'common.forgotPassword'
  | 'common.cancel'
  | 'common.confirm'
  | 'common.delete'
  | 'common.report'
  | 'common.block'
  | 'common.check'
  | 'common.apply'
  | 'common.accept'
  | 'common.create'
  | 'keys.convert'
  | 'keys.dailyGoals'
  | 'keys.enterAmount'
  | 'keys.convertMultipleOfTenHint'
  | 'keys.convertInsufficientWhiteKeys'
  | 'keys.convertRateHint'
  | 'keys.redeem'
  | 'keys.redeemSubtitle'
  | 'keys.redeemOption10'
  | 'keys.redeemOption20'
  | 'keys.redeemOption50'
  | 'keys.redeemOption100'
  | 'keys.redeemOption500'
  | 'keys.redeemInsufficientKeys'
  | 'keys.redeemNote'
  | 'keys.lockedMessage'
  | 'keys.dailyGoal1'
  | 'keys.dailyGoal2'
  | 'keys.dailyGoal3'
  | 'keys.dailyGoal4'
  | 'keys.dailyGoal5'
  | 'keys.dailyGoal6'
  | 'keys.dailyGoal7'
  | 'keys.dailyGoal8'
  | 'keys.dailyGoal9'
  | 'auth.sessionRequiredTitle'
  | 'auth.signInToBlockUsers'
  | 'auth.signInToReportUsers'

  | 'home.blockConfirmQuestion'
  | 'home.blockReasonPlaceholder'
  | 'home.reportSelectReason'

  | 'report.reason.sexualContent'
  | 'report.reason.harassment'
  | 'report.reason.offensiveLanguage'
  | 'report.reason.scam'
  | 'report.reason.violence'
  | 'report.reason.spam'
  | 'report.reason.impersonation'
  | 'report.reason.illegalContent'
  | 'report.reason.childSexualAbuse'
  | 'report.reason.weaponsOrDrugsIncitement'
  | 'report.reason.inappropriateConduct'
  | 'report.reason.other'

  | 'toast.blocked'
  | 'toast.reportSent'

  | 'errors.unableToBlockUser'
  | 'errors.unableToSendReport'

  | 'login.email'
  | 'login.password'
  | 'login.emailPlaceholder'
  | 'login.passwordPlaceholder'
  | 'login.signIn'
  | 'login.fillAllFields'
  | 'login.invalidCredentials'
  | 'login.noAccount'
  | 'login.signUp'
  | 'login.googleNotRegistered'
  | 'login.googlePkceMissing'

  | 'login.resetTitle'
  | 'login.resetSend'
  | 'login.resetVerify'
  | 'login.resetChangePassword'
  | 'login.resetEmailNotRegistered'
  | 'login.resetInvalidEmail'
  | 'login.resetCodeSent'
  | 'login.resetPasswordChanged'
  | 'login.resetCodeLabel'
  | 'login.resetNewPasswordLabel'
  | 'login.resetRepeatNewPasswordLabel'

  | 'register.email'
  | 'register.username'
  | 'register.usernameAlreadyInUse'
  | 'register.birthDate'
  | 'register.birthDatePlaceholder'
  | 'register.gender'
  | 'register.selectGender'
  | 'register.nationality'
  | 'register.password'
  | 'register.confirmPassword'
  | 'register.confirmPasswordPlaceholder'
  | 'register.next'
  | 'register.signUp'
  | 'register.registering'
  | 'register.successMessage'
  | 'register.privacyPrefix'
  | 'register.privacyPolicies'
  | 'register.privacyPolicyLink'
  | 'register.cookiesAdsPolicyLink'
  | 'register.termsConditionsLink'
  | 'register.privacySeparator1'
  | 'register.privacySeparator2'
  | 'register.privacySuffix'
  | 'register.errorTitle'
  | 'chat.generalTab'
  | 'chat.yourThreadsTab'
  | 'chat.recommendationsTab'
  | 'chat.eventsTab'
  | 'chat.readingsTab'
  | 'chat.imagesTab'
  | 'chat.signInToSendMessages'
  | 'chat.unableToSendMessage'
  | 'chat.connectionErrorSendingMessage'
  | 'chat.addCaptionPlaceholder'
  | 'chat.imageCaptionPlaceholder'
  | 'reading.screenTitle'
  | 'reading.titlePlaceholder'
  | 'reading.subtitlePlaceholder'
  | 'reading.leadPlaceholder'
  | 'reading.dateLabel'
  | 'reading.datePlaceholder'
  | 'reading.chooseCategoryLabel'
  | 'reading.categoryLabel'
  | 'reading.searchCategoryPlaceholder'
  | 'reading.hypeCostLabel'
  | 'reading.hypeCostInfo'
  | 'reading.hypeCostAmountLabel'
  | 'reading.bodyPlaceholder'
  | 'reading.intertitlePlaceholder'
  | 'reading.imageLimitReachedTitle'
  | 'reading.imageLimitReachedBody'
  | 'reading.galleryPermissionTitle'
  | 'reading.galleryPermissionMessage'
  | 'reading.galleryPermissionAskLater'
  | 'reading.galleryPermissionRequiredTitle'
  | 'reading.galleryPermissionRequiredBody'
  | 'reading.openSettings'
  | 'reading.imagePickErrorTitle'
  | 'reading.imagePickErrorBody'
  | 'reading.citeSelectionToastBody'
  | 'reading.intertitleToastBody'
  | 'reading.bodyMinSuffix'
  | 'reading.addImage'
  | 'reading.maxImagesHint'
  | 'reading.selectedCitationLabel'
  | 'reading.citedUsersTitle'
  | 'reading.citedUsersInputLabel'
  | 'reading.voiceNoteIntroTitle'
  | 'reading.voiceNoteIntroDescription'
  | 'reading.voiceNoteInlineTitle'
  | 'reading.voiceNoteInlineDescription'
  | 'reading.voiceNoteOutroTitle'
  | 'reading.voiceNoteOutroDescription'
  | 'reading.voiceNoteRecordAction'
  | 'reading.voiceNoteRerecordAction'
  | 'reading.voiceNoteRecordingStatus'
  | 'register.unableToComplete'
  | 'register.selectNationality'
  | 'register.searchCountry'
  | 'register.noCountriesFound'

  | 'register.emailVerificationTitle'
  | 'register.emailVerificationHint'
  | 'register.emailVerificationLinkHint'
  | 'register.iConfirmedEmail'
  | 'register.confirmedCheckHint'
  | 'register.code'
  | 'register.codePlaceholder'
  | 'register.timeRemaining'
  | 'register.verifyAndSignUp'
  | 'register.verifying'
  | 'register.resendCode'
  | 'register.codeExpired'
  | 'register.codeIncorrect'
  | 'register.codeInvalidInline'
  | 'register.remainingAttempts'
  | 'register.remainingAttemptsInline'
  | 'register.emailTemporarilyBlocked'
  | 'register.emailLockedInlineError'
  | 'register.emailLockedRetryInline'
  | 'register.rectificationTitle'
  | 'register.rectificationPlaceholder'
  | 'register.rectificationSend'
  | 'register.rectificationSent'
  | 'register.tryLaterOrCheckEmailExists'
  | 'register.mustVerifyEmailFirst'
  | 'register.cancelledNotice'
  | 'register.emailAlreadyInUse'
  | 'register.emailSendFailed'
  | 'register.missingProfileData'
  | 'register.signupExpiredNotice'
  | 'register.signupExpiredTitle'
  | 'register.emailLocked24h'
  | 'register.attemptsRemaining'
  | 'register.pendingConfirmation'
  | 'register.emailLockedTemp'
  | 'register.emailLockedPermanent'

  | 'validation.invalidDateFormat'
  | 'validation.invalidDate'
  | 'validation.mustBeAdult'
  | 'validation.passwordMinLength'
  | 'validation.passwordMaxLength'
  | 'validation.passwordNeedsLowercase'
  | 'validation.passwordNeedsUppercase'
  | 'validation.passwordNeedsLetter'
  | 'validation.passwordNeedsNumber'
  | 'validation.passwordNeedsSpecial'
  | 'validation.passwordsDontMatch'

  | 'front.editProfilePhoto'
  | 'front.yourSocialNetworks'
  | 'front.userPlaceholder'
  | 'front.selectCategory'
  | 'front.noPublicationsAvailable'
  | 'front.forYou'
  | 'front.publicationsCount'
  | 'front.socialNetworksSingular'
  | 'front.socialNetworksPlural'
  | 'front.publish'
  | 'front.remove'
  | 'front.yourProfile'
  | 'front.yourPresentation'
  | 'front.intimacies'
  | 'front.time'
  | 'front.loadingImages'
  | 'front.readMore'
  | 'front.readLess'
  | 'front.votes'
  | 'front.profileEmptyPrimaryMessage'
  | 'front.profileEmptySecondaryMessage'
  | 'front.profileRingHintSelect'
  | 'front.profileRingHintMaxRings'
  | 'front.profileRingDelete'
  | 'front.profileRingNameLabel'
  | 'front.profileRingNameHelper'
  | 'front.profileRingDescriptionLabel'
  | 'front.profileRingLinkLabel'
  | 'front.profileRingLinkHelper'
  | 'front.profileRingLocationLabel'
  | 'front.profileRingLocationHelper'

  | 'profile.deleteContentTitle'
  | 'profile.deleteContentBody'
  | 'front.reactions'
  | 'front.deletePublicationTitle'
  | 'front.deletePublicationBody'
  | 'front.category'
  | 'front.presentationAddCarouselHint'
  | 'front.presentationTitleLabel'
  | 'front.presentationTitlePlaceholder'
  | 'front.presentationBodyLabel'
  | 'front.presentationBodyPlaceholder'
  | 'front.presentationLockedTitle'
  | 'front.presentationLockedBody'

  | 'front.selectSocialNetwork'
  | 'front.link'
  | 'front.linkMustBeFrom'

  | 'front.incorporate'
  | 'front.answerPlaceholder'
  | 'front.chooseCorrectOption'
  | 'front.addMoreSurveyOptions'
  | 'front.image'
  | 'front.text'
  | 'front.draftPlaceholder'
  | 'front.draftIntimacyPlaceholder'
  | 'front.intimaciesPublishBlocked'
  | 'front.homeSwipeTutorialHint'
  | 'front.channelLabel'

  | 'chat.tabYourChannel'
  | 'chat.tabChannel'
  | 'chat.tabJoined'
  | 'chat.tabYourGroups'
  | 'chat.tabGroups'
  | 'chat.postOnHomeToActivateChannel'
  | 'chat.noChannelsYet'
  | 'chat.createGroup'
  | 'chat.notJoinedAnyGroupYet'
  | 'chat.back'
  | 'chat.interactPlaceholder'
  | 'chat.timeExpired'
  | 'chat.remainingPrefix'
  | 'chat.hour'
  | 'chat.hours'
  | 'chat.minute'
  | 'chat.minutes'
  | 'chat.reply'
  | 'chat.limitWarningMessage'
  | 'chat.hostLimitedInteractions'
  | 'chat.waitHostReplyThread'
  | 'chat.waitCreatorReplyAgain'
  | 'chat.selectGroup'
  | 'chat.toPrefix'
  | 'chat.request'
  | 'chat.requestPending'
  | 'chat.requestAccepted'
  | 'chat.requestBlocked'
  | 'chat.members'
  | 'chat.messageHidden'
  | 'chat.messageVisible'
  | 'event.enableTasks'
  | 'event.assignTask'
  | 'event.activateRequests'
  | 'event.rewardTasks'
  | 'event.describeTask'
  | 'event.userPlaceholder'
  | 'event.amount'
  | 'event.type'
  | 'event.requiredSuffix'
  | 'event.name'
  | 'event.description'
  | 'event.dateAndTime'
  | 'event.dayMonthYear'
  | 'event.time'
  | 'event.durationTitle'
  | 'event.duration'
  | 'event.endsIn'
  | 'event.link'
  | 'event.selectSocialNetwork'
  | 'event.addLink'
  | 'event.location'
  | 'event.selectLocation'
  | 'event.searchType'
  | 'event.typeOption.social'
  | 'event.typeOption.cultural'
  | 'event.typeOption.artistic'
  | 'event.typeOption.business'
  | 'event.typeOption.economic'
  | 'event.typeOption.academic'
  | 'event.typeOption.sports'
  | 'event.typeOption.entertainment'
  | 'event.typeOption.political'
  | 'event.typeOption.religious'
  | 'event.typeOption.marketing'
  | 'event.typeOption.charity'
  | 'event.typeOption.audiovisual'
  | 'event.typeOption.scientific'
  | 'event.typeOption.technological'
  | 'event.typeOption.children'
  | 'event.typeOption.musical'
  | 'event.typeOption.automotive'
  | 'event.typeOption.fitness'
  | 'event.typeOption.gastronomic'
  | 'event.typeOption.work'

  | 'chat.lockedYourGroupsMessage'
  | 'chat.lockedJoinedGroupsMessage'
  | 'chat.hostOnlyEventHypeMessage'
  | 'chat.hostOnlyReadingHypeMessage'

  | 'groups.edit'
  | 'groups.delete'
  | 'groups.deleteConfirmTitle'
  | 'groups.deleteConfirmBody'
  | 'groups.limit'
  | 'groups.limited'
  | 'groups.expel'
  | 'groups.expelAndBlock'
  | 'groups.leaveGroup'
  | 'groups.leaveAndBlock'
  | 'groups.limitUsers'
  | 'groups.expelUsers'
  | 'groups.selectAll'
  | 'groups.deselect'
  | 'groups.noMembersToShow'

  | 'notifications.groupJoinRequestMessage'
  | 'notifications.ignore'
  | 'notifications.accept'
  | 'notifications.read'
  | 'notifications.unread'
  | 'notifications.empty'

  | 'config.title'
  | 'config.accountCenter'
  | 'config.blockedUsers'
  | 'config.devicePermissions'
  | 'config.helpCenter'
  | 'config.information'
  | 'config.yourAccountIn'

  | 'aboutKeinti.title'
  | 'aboutKeinti.moreAboutKeinti'
  | 'aboutKeinti.body'

  | 'devicePermissions.description'
  | 'devicePermissions.galleryTitle'
  | 'devicePermissions.galleryDescription'
  | 'devicePermissions.statusGranted'
  | 'devicePermissions.statusDenied'
  | 'devicePermissions.statusUnknown'
  | 'devicePermissions.iosHint'
  | 'devicePermissions.revokeConfirmTitle'
  | 'devicePermissions.revokeConfirmBody'
  | 'devicePermissions.reEnableConfirmTitle'
  | 'devicePermissions.reEnableConfirmBody'
  | 'devicePermissions.osRequestTitle'
  | 'devicePermissions.osRequestMessage'
  | 'devicePermissions.osRequestAskLater'
  | 'devicePermissions.osRequestDeny'
  | 'devicePermissions.osRequestAllow'

  | 'verifyKeinti.objectivesTitle'
  | 'verifyKeinti.objective1'
  | 'verifyKeinti.complete'
  | 'verifyKeinti.completed'
  | 'verifyKeinti.objective2'
  | 'verifyKeinti.objective3'
  | 'verifyKeinti.objective4'
  | 'verifyKeinti.objective5'
  | 'verifyKeinti.objective6'
  | 'verifyKeinti.benefitsTitle'
  | 'verifyKeinti.benefit1'
  | 'verifyKeinti.benefit2'
  | 'verifyKeinti.benefit3'
  | 'verifyKeinti.benefit4'
  | 'verifyKeinti.tabObjectives'
  | 'verifyKeinti.tabBenefits'
  | 'verifyKeinti.importantNotice'
  | 'verifyKeinti.importantNoticeBody'
  | 'verifyKeinti.verifyAction'
  | 'verifyKeinti.accountVerifiedLabel'

  | 'accountCenter.title'
  | 'accountCenter.personalData'
  | 'accountCenter.securityControl'
  | 'accountCenter.changePassword'
  | 'accountCenter.adminSelfies'
  | 'accountCenter.closeAccount'
  | 'accountCenter.deleteAccount'
  | 'accountCenter.deleteAccountConfirmTitle'
  | 'accountCenter.deleteAccountConfirmBody'
  | 'accountCenter.accountDeletionPolicyLink'

  | 'personalData.title'
  | 'personalData.description'
  | 'personalData.contactInfo'
  | 'personalData.birthDate'
  | 'personalData.gender'
  | 'personalData.nationality'

  | 'securityControl.passwordAndAuth'
  | 'securityControl.passwordAndAuthDescription'
  | 'securityControl.accountAuth'
  | 'securityControl.privacyPolicy'
  | 'securityControl.cookiesAdPolicy'
  | 'securityControl.termsOfUse'
  | 'securityControl.childSafetyStandards'
  | 'securityControl.accountDeletionPolicy'
  | 'securityControl.verifyYourKeinti'

  | 'accountAuth.description'
  | 'accountAuth.step1Title'
  | 'accountAuth.step1Body'
  | 'accountAuth.step1Hint'
  | 'accountAuth.step1Action'
  | 'accountAuth.step1Pending'
  | 'accountAuth.pendingReviewNotice'
  | 'accountAuth.step1Blocked'
  | 'accountAuth.refreshStatus'
  | 'accountAuth.selfieAccepted'
  | 'accountAuth.statusNotSubmitted'
  | 'accountAuth.statusPending'
  | 'accountAuth.statusAccepted'
  | 'accountAuth.statusFailed'
  | 'accountAuth.statusBlocked'
  | 'accountAuth.errorTitle'
  | 'accountAuth.selfieUploadError'
  | 'accountAuth.step2Title'
  | 'accountAuth.step2Body'
  | 'accountAuth.step2Locked'
  | 'accountAuth.step2Ready'
  | 'accountAuth.step2AlreadyEnabled'
  | 'accountAuth.generateTotp'
  | 'accountAuth.secretLabel'
  | 'accountAuth.secretHint'
  | 'accountAuth.codeLabel'
  | 'accountAuth.codePlaceholder'
  | 'accountAuth.verifyCode'
  | 'accountAuth.totpSetupError'
  | 'accountAuth.verifyError'
  | 'accountAuth.successTitle'
  | 'accountAuth.successBody'
  | 'accountAuth.completed'
  | 'accountAuth.badgeExpiresIn'

  | 'changePassword.requirements'
  | 'changePassword.currentPassword'
  | 'changePassword.currentPasswordInvalid'
  | 'changePassword.newPassword'
  | 'changePassword.repeatNewPassword'
  | 'changePassword.passwordsDontMatch'
  | 'changePassword.forgot'
  | 'changePassword.success'
  | 'changePassword.locked'
  | 'changePassword.attemptsRemaining'
  | 'changePassword.accountLocked'
  | 'changePassword.checkCompleted'
  | 'blockedUsers.title'
  | 'blockedUsers.empty'

  | 'adminSelfies.title'
  | 'adminSelfies.tabPending'
  | 'adminSelfies.tabBlocked'
  | 'adminSelfies.refresh'
  | 'adminSelfies.emptyPending'
  | 'adminSelfies.emptyBlocked'
  | 'adminSelfies.reasonPlaceholder'
  | 'adminSelfies.accept'
  | 'adminSelfies.reject'
  | 'adminSelfies.block'
  | 'adminSelfies.unblock'
  | 'blockedUsers.reasonPlaceholder'

  | 'toast.joinedChannelOf'
  | 'toast.published'
  | 'toast.publicationNotActiveHome'
  | 'toast.joinedGroupMessage'

  | 'carouselEditor.adjustImage'
  | 'carouselEditor.imageCount'
  | 'carouselEditor.processing'
  | 'carouselEditor.noPreview'
  | 'carouselEditor.editImage'
  | 'carouselEditor.errorTitle'
  | 'carouselEditor.cropErrorBody'
  | 'carouselEditor.ok';

const es: Record<TranslationKey, string> = {
  'language.label': 'Idioma',
  'language.spanish': 'Español',
  'language.english': 'Inglés',
  'language.french': 'Francés',
  'language.portuguese': 'Portugués',
  'language.german': 'Alemán',
  'language.italian': 'Italiano',
  'gender.male': 'Hombre',
  'gender.female': 'Mujer',
  'gender.unspecified': 'No especificar',
  'common.loading': 'Cargando...',
  'common.logout': 'Cerrar sesión',
  'common.notifications': 'Notificaciones',
  'common.keys': 'Llaves',
  'common.or': 'o',
  'common.unlock': 'Desbloquear',
  'common.view': 'Visualizar',
  'common.forgotPassword': '¿Olvidaste tu [[contraseña]]?',
  'common.cancel': 'Cancelar',
  'common.confirm': 'Confirmar',
  'common.delete': 'Eliminar',
  'common.report': 'Denunciar',
  'common.block': 'Bloquear',
  'common.check': 'Comprobar',
  'common.apply': 'Aplicar',
  'common.accept': 'Aceptar',
  'common.create': 'Crear',
  'keys.convert': 'Convertir',
  'keys.dailyGoals': 'Objetivos diarios',
  'keys.enterAmount': 'Introducir cantidad',
  'keys.convertMultipleOfTenHint': 'La cantidad debe ser un múltiplo de 10.',
  'keys.convertInsufficientWhiteKeys': 'No tienes suficientes llaves blancas para esta conversión.',
  'keys.convertRateHint': 'Cada 10 llaves blancas se convierten en 1 llave gradient.',
  'keys.redeem': 'Canjear',
  'keys.redeemSubtitle': 'Selecciona una de las opciones',
  'keys.redeemOption10': '10 euros',
  'keys.redeemOption20': '20 euros',
  'keys.redeemOption50': '50 euros',
  'keys.redeemOption100': '100 euros',
  'keys.redeemOption500': '500 euros',
  'keys.redeemInsufficientKeys': 'No tienes suficientes llaves.',
  'keys.redeemNote': 'Solo podrás canjear un cupón al día. Elige bien. El dinero canjeado se enviará a la cuenta de PayPal que proporciones en el paso posterior, después de pulsar el botón "Canjear".',
  'keys.lockedMessage': 'Autentica tu cuenta para acceder a tus objetivos diarios y llaves.',
  'keys.dailyGoal1': 'Por cada contenedor íntimo que abras, obtendrás 2 llaves blancas.',
  'keys.dailyGoal2': 'Consigue enlazar un hilo con un anfitrión en su canal.',
  'keys.dailyGoal3': 'Desbloquea las imágenes que compartan los anfitriones en sus canales a los que te unas y obtén 10 llaves blancas por cada una.',
  'keys.dailyGoal4': 'Publica tu perfil cada día.',
  'keys.dailyGoal5': 'Crea un evento en tu canal.',
  'keys.dailyGoal6': 'Comparte una imagen en tu canal.',
  'keys.dailyGoal7': 'Consigue que accedan 100 usuarios a tu canal: 1.000 llaves blancas.',
  'keys.dailyGoal8': 'Consigue que accedan 1.000 usuarios a tu canal: 10.000 llaves blancas.',
  'keys.dailyGoal9': 'Consigue que accedan 10.000 usuarios a tu canal: 100.000 llaves blancas.',

  'auth.sessionRequiredTitle': 'Sesión requerida',
  'auth.signInToBlockUsers': 'Inicia sesión para bloquear usuarios.',
  'auth.signInToReportUsers': 'Inicia sesión para denunciar usuarios.',

  'home.blockConfirmQuestion': '¿Quieres bloquear a {user}?',
  'home.blockReasonPlaceholder': 'Motivo del bloqueo (opcional)...',
  'home.reportSelectReason': 'Selecciona un motivo para denunciar a {user}.',

  'report.reason.sexualContent': 'Contenido sexual o desnudos',
  'report.reason.harassment': 'Acoso o bullying',
  'report.reason.offensiveLanguage': 'Lenguaje ofensivo',
  'report.reason.scam': 'Estafa o engaño',
  'report.reason.violence': 'Violencia o amenazas',
  'report.reason.spam': 'Spam',
  'report.reason.impersonation': 'Suplantación de identidad',
  'report.reason.illegalContent': 'Contenido ilegal',
  'report.reason.childSexualAbuse': 'Abuso/sexualización infantil',
  'report.reason.weaponsOrDrugsIncitement': 'Incitación al uso de armas/drogas',
  'report.reason.inappropriateConduct': 'Conducta inapropiada',
  'report.reason.other': 'Otros',

  'errors.unableToBlockUser': 'No se pudo bloquear al usuario',
  'errors.unableToSendReport': 'No se pudo enviar la denuncia',

  'login.email': 'Correo electrónico',
  'login.password': 'Contraseña',
  'login.emailPlaceholder': 'Ejemplo@correo.com',
  'login.passwordPlaceholder': 'Ingresa tu contraseña',
  'login.signIn': 'Iniciar Sesión',
  'login.fillAllFields': 'Por favor, completa todos los campos',
  'login.invalidCredentials': 'Credenciales incorrectas',
  'login.googleNotRegistered': 'Esta cuenta aún no está registrada. Regístrate antes para poder iniciar sesión con este correo electrónico',
  'login.googlePkceMissing': 'No se pudo completar el inicio de sesión con Google. Vuelve a intentarlo o regístrate en Keinti.',
  'login.noAccount': '¿No tienes una cuenta? ',
  'login.signUp': 'Regístrate',

  'login.resetTitle': 'Recuperar contraseña',
  'login.resetSend': 'Enviar',
  'login.resetVerify': 'Verificar',
  'login.resetChangePassword': 'Cambiar contraseña',
  'login.resetEmailNotRegistered': 'Este correo aún no está en uso en Keinti por ningún usuario. Regístrate.',
  'login.resetInvalidEmail': 'Correo inválido',
  'login.resetCodeSent': 'Te hemos enviado un código a tu correo.',
  'login.resetPasswordChanged': 'Contraseña actualizada. Ya puedes iniciar sesión.',
  'login.resetCodeLabel': 'Código',
  'login.resetNewPasswordLabel': 'Crear nueva contraseña',
  'login.resetRepeatNewPasswordLabel': 'Repetir nueva contraseña',

  'register.email': 'Correo electrónico',
  'register.username': '@usuario',
  'register.usernameAlreadyInUse': 'El "{username}" ya está siendo utilizado por otro usuario; prueba con otro.',
  'register.birthDate': 'Fecha de nacimiento',
  'register.birthDatePlaceholder': 'DD/MM/AAAA',
  'register.gender': 'Género',
  'register.selectGender': 'Selecciona tu género',
  'register.nationality': 'Nacionalidad',
  'register.password': 'Contraseña',
  'register.confirmPassword': 'Confirmar contraseña',
  'register.confirmPasswordPlaceholder': 'Confirma tu contraseña',
  'register.next': 'Siguiente',
  'register.signUp': 'Registrarse',
  'register.registering': 'Registrando...',
  'register.successMessage': 'Bienvenido, ya eres usuario de Keinti.',
  'register.privacyPrefix': 'Una vez te registres, quedarán aceptadas las ',
  'register.privacyPolicies': 'políticas de privacidad, las políticas de cookies y publicidad, y los términos y condiciones de uso',
  'register.privacyPolicyLink': 'políticas de privacidad',
  'register.cookiesAdsPolicyLink': 'políticas de cookies y publicidad',
  'register.termsConditionsLink': 'términos y condiciones de uso',
  'register.privacySeparator1': ', las ',
  'register.privacySeparator2': ', y los ',
  'register.privacySuffix': ' de nuestra aplicación Keinti. Para mayor seguridad, léelas antes de registrarte.',
  'register.errorTitle': 'Error al registrarse',
  'register.unableToComplete': 'No se pudo completar el registro.',
  'register.selectNationality': 'Selecciona tu nacionalidad',
  'register.searchCountry': 'Buscar país...',
  'register.noCountriesFound': 'No se encontraron países',

  'register.emailVerificationTitle': 'Verificación de email',
  'register.emailVerificationHint': 'Hemos enviado un código a',
  'register.emailVerificationLinkHint': 'Si recibes un enlace de confirmación en vez de un código, ábrelo y volverás automáticamente a Keinti para completar el registro.',
  'register.iConfirmedEmail': 'Ya confirmé el email',
  'register.confirmedCheckHint': 'Si ya abriste el enlace, vuelve a Keinti. Si no ocurre nada, espera unos segundos y pulsa de nuevo, o reenvía el email.',
  'register.code': 'Código',
  'register.codePlaceholder': 'Introduce el código',
  'register.timeRemaining': 'Tiempo restante',
  'register.verifyAndSignUp': 'Verificar y registrarse',
  'register.verifying': 'Verificando...',
  'register.resendCode': 'Reenviar email',
  'register.codeExpired': 'El código ha caducado. Solicita uno nuevo.',
  'register.codeIncorrect': 'Código incorrecto',
  'register.codeInvalidInline': 'El código introducido no es válido. Inténtelo de nuevo',
  'register.remainingAttempts': 'Intentos restantes',
  'register.remainingAttemptsInline': 'Faltan {count} intentos',
  'register.emailTemporarilyBlocked': 'Has agotado los intentos. Debes reiniciar el registro con otro correo.',
  'register.emailLockedInlineError': 'Este correo electrónico ha sido bloqueado temporalmente. Estamos revisando el correo electrónico. Envíenos una reclamación para poder reactivar su correo electrónico en Keinti.',
  'register.emailLockedRetryInline': 'Este correo electrónico ha sido bloqueado temporalmente. Prueba con otro o espera un tiempo hasta que se resuelva tu solicitud de rectificación.',
  'register.rectificationTitle': 'Rectificación',
  'register.rectificationPlaceholder': 'Describe tu rectificación (máx. 220 caracteres)',
  'register.rectificationSend': 'Enviar reclamación',
  'register.rectificationSent': 'Reclamación enviada. Revisaremos tu solicitud lo antes posible.',
  'register.tryLaterOrCheckEmailExists': 'Inténtalo más tarde o verifica que el email introducido existe',
  'register.mustVerifyEmailFirst': 'Debes verificar tu email antes de registrarte',
  'register.cancelledNotice': 'Proceso de registro anulado. En caso de querer registrar tu nueva cuenta en Keinti, vuelve a realizar el proceso de registro.',
  'register.emailAlreadyInUse': 'Este correo electrónico ya está en uso por otro usuario registrado en Keinti. Prueba con otro diferente.',
  'register.emailSendFailed': 'No hemos podido enviar el código al correo electrónico "{email}". Verifica que el correo electrónico sea válido.',
  'register.missingProfileData': 'Faltan datos del registro. Vuelve atrás y completa usuario, fecha, género y nacionalidad.',
  'register.signupExpiredNotice': 'El tiempo de verificación ha expirado. Tus datos de registro han sido eliminados. Puedes volver a intentarlo.',
  'register.signupExpiredTitle': 'Tiempo expirado',
  'register.emailLocked24h': 'Has agotado los {max} intentos de registro con este correo. No podrás volver a usarlo durante 48 horas.',
  'register.attemptsRemaining': 'Intentos restantes: {remaining} de {max}',
  'register.pendingConfirmation': 'Confirma tu email antes de que finalice el tiempo, aún está activo.',
  'register.emailLockedTemp': 'Has agotado los {max} intentos de registro con este correo. No podrás volver a usarlo durante 48 horas.',
  'register.emailLockedPermanent': 'Tu correo electrónico ha sido bloqueado permanentemente por exceder el límite de intentos de registro. Contacta con keintisoporte@gmail.com para solicitar el desbloqueo.',

  'validation.invalidDateFormat': 'Formato de fecha inválido',
  'validation.invalidDate': 'Fecha inválida',
  'validation.mustBeAdult': 'Debes ser mayor de 18 años',
  'validation.passwordMinLength': 'La contraseña debe tener al menos 10 caracteres',
  'validation.passwordMaxLength': 'La contraseña no puede superar 20 caracteres',
  'validation.passwordNeedsLowercase': 'Debe contener al menos 1 letra minúscula',
  'validation.passwordNeedsUppercase': 'Debe contener al menos 1 letra mayúscula',
  'validation.passwordNeedsLetter': 'Debe contener al menos 1 letra',
  'validation.passwordNeedsNumber': 'Debe contener al menos 1 número',
  'validation.passwordNeedsSpecial': 'Debe contener al menos 1 carácter especial',
  'validation.passwordsDontMatch': 'Las contraseñas no coinciden',

  'front.editProfilePhoto': 'Editar Foto Perfil',
  'front.yourSocialNetworks': 'Tus Redes Sociales',
  'front.userPlaceholder': 'Usuario',
  'front.selectCategory': 'Selecciona una categoría',
  'front.noPublicationsAvailable': 'No hay publicaciones disponibles',
  'front.forYou': 'Para ti',
  'front.publicationsCount': 'Publicaciones',
  'front.socialNetworksSingular': 'red social',
  'front.socialNetworksPlural': 'redes sociales',
  'front.publish': 'Publicar',
  'front.remove': 'Quitar (Publicado)',
  'front.yourProfile': 'Tu perfil',
  'front.yourPresentation': 'Tu presentación',
  'front.intimacies': 'Intimidades',
  'front.time': 'Tiempo',
  'front.loadingImages': 'Cargando imágenes...',
  'front.readMore': 'Leer más...',
  'front.readLess': 'Leer menos',
  'front.votes': 'votos',

  'front.profileEmptyPrimaryMessage': "Agrega 'Tu presentación' y al menos una 'Intimidad' para publicar tu perfil",
  'front.profileEmptySecondaryMessage': 'Al publicar tu perfil, otros usuarios podrán acceder a tu canal, donde podrás compartir contenido e interactuar con ellos',
  'front.profileRingHintSelect': 'Selecciona una parte de una de las imágenes del carrusel',
  'front.profileRingHintMaxRings': 'Máximo alcanzado: 5 aros',
  'front.profileRingDelete': 'Eliminar aro',
  'front.profileRingNameLabel': 'Nombre (máximo 38 caracteres)',
  'front.profileRingNameHelper': 'Nombre del producto, empresa, usuario, canal, post, contenido, etc',
  'front.profileRingDescriptionLabel': 'Descripción (máximo 280 caracteres)',
  'front.profileRingLinkLabel': 'Enlace',
  'front.profileRingLinkHelper': 'Añade un enlace a una red social para que los usuarios tengan acceso al producto, la empresa, el usuario, el canal, el contenido, etc.',
  'front.profileRingLocationLabel': 'Ubicación',
  'front.profileRingLocationHelper': 'Añade una ubicación',

  'profile.deleteContentTitle': 'Eliminar contenido',
  'profile.deleteContentBody': '¿Estás seguro de que quieres eliminar este contenido de tu perfil?',
  'front.reactions': 'Reacciones',
  'front.deletePublicationTitle': 'Quitar publicación',
  'front.deletePublicationBody': 'Tu perfil actualmente es público para otros usuarios en la «Home»; al quitarlo, solo podrás visualizarlo tú en «Tu perfil».',
  'front.category': 'Categoría',
  'front.presentationAddCarouselHint': 'Añade hasta 3 imágenes al carrusel',
  'front.presentationTitleLabel': 'Título',
  'front.presentationTitlePlaceholder': 'Escribe un título...',
  'front.presentationBodyLabel': 'Presentación',
  'front.presentationBodyPlaceholder': 'Escribe tu presentación...',
  'front.presentationLockedTitle': 'Edición bloqueada',
  'front.presentationLockedBody': 'Tu perfil actualmente está publicado.',

  'front.selectSocialNetwork': 'Selecciona una red social',
  'front.link': 'Enlace',
  'front.linkMustBeFrom': 'El enlace debe ser de',

  'front.incorporate': 'Incorporar',
  'front.answerPlaceholder': 'Respuesta...',
  'front.chooseCorrectOption': 'Elige la opción correcta',
  'front.addMoreSurveyOptions': 'Añade más opciones en la encuesta',
  'front.image': 'Imagen',
  'front.text': 'Texto',
  'front.draftPlaceholder': 'Redacta...',
  'front.draftIntimacyPlaceholder': 'Redacta alguna intimidad...',
  'front.intimaciesPublishBlocked': "Aún no puedes publicar intimidades. Primero crea 'Tu presentación'.",
  'front.homeSwipeTutorialHint': 'Cuando la barra indicadora de la parte superior de la pantalla esté activa, desliza hacia abajo para cambiar de publicación.',
  'front.channelLabel': 'Canal:',

  'chat.tabYourChannel': 'Tu canal',
  'chat.tabChannel': 'Canal',
  'chat.tabJoined': 'Unidos',
  'chat.tabYourGroups': 'Tus Grupos',
  'chat.tabGroups': 'Grupos',
  'chat.postOnHomeToActivateChannel': "Publica en la 'Home' para activar tu canal",
  'chat.noChannelsYet': 'No tienes canales aún',
  'chat.createGroup': 'Crea un grupo',
  'chat.notJoinedAnyGroupYet': 'Aún no te has unido a ningún grupo',
  'chat.back': 'Volver',
  'chat.interactPlaceholder': 'Interactúa...',
  'chat.timeExpired': 'Tiempo agotado',
  'chat.remainingPrefix': 'Falta',
  'chat.hour': 'hora',
  'chat.hours': 'horas',
  'chat.minute': 'minuto',
  'chat.minutes': 'minutos',
  'chat.reply': 'Responder',
  'chat.generalTab': 'General',
  'chat.yourThreadsTab': 'Hilos',
  'chat.recommendationsTab': 'Recomendaciones',
  'chat.eventsTab': 'Eventos',
  'chat.readingsTab': 'Lecturas',
  'chat.imagesTab': 'Imágenes',
  'chat.limitWarningMessage': 'Ya has interactuado en el canal de {user}. Espera la respuesta.',
  'chat.hostLimitedInteractions': '{user} te ha limitado las interacciones',
  'chat.waitHostReplyThread': 'Espera la respuesta del anfitrión para continuar con el hilo',
  'chat.waitCreatorReplyAgain': 'Espera la respuesta del anfitrión para poder responder de nuevo',
  'chat.signInToSendMessages': 'Inicia sesión para enviar mensajes.',
  'chat.unableToSendMessage': 'No se pudo enviar el mensaje.',
  'chat.connectionErrorSendingMessage': 'Error de conexión al enviar el mensaje.',
  'chat.addCaptionPlaceholder': 'Añade un pie de foto...',
  'chat.imageCaptionPlaceholder': 'Pie de imagen...',
  'reading.screenTitle': 'Crear lectura',
  'reading.titlePlaceholder': 'Título',
  'reading.subtitlePlaceholder': 'Subtítulo',
  'reading.leadPlaceholder': 'Entradilla...',
  'reading.dateLabel': 'Fecha',
  'reading.datePlaceholder': 'DD/MM/AAAA',
  'reading.chooseCategoryLabel': 'Elige una Categoría',
  'reading.categoryLabel': 'Categoría',
  'reading.searchCategoryPlaceholder': 'Buscar categoría',
  'reading.hypeCostLabel': 'Coste del hype',
  'reading.hypeCostInfo': 'Escoge la cantidad de llaves blancas que quieres que te done cada usuario del canal por esta lectura para impulsar tu perfil. Cuantas más llaves recibas, más se viralizará tu perfil y tu canal en la pantalla de Hype.',
  'reading.hypeCostAmountLabel': 'Cantidad',
  'reading.bodyPlaceholder': 'Cuerpo...',
  'reading.intertitlePlaceholder': 'Intertítulo',
  'reading.imageLimitReachedTitle': 'Límite alcanzado',
  'reading.imageLimitReachedBody': 'Solo puedes añadir hasta 3 imágenes en la lectura.',
  'reading.galleryPermissionTitle': 'Permiso de galería',
  'reading.galleryPermissionMessage': 'La aplicación necesita acceso a tu galería para seleccionar fotos.',
  'reading.galleryPermissionAskLater': 'Preguntar después',
  'reading.galleryPermissionRequiredTitle': 'Permiso requerido',
  'reading.galleryPermissionRequiredBody': 'Debes habilitar el acceso a Fotos desde Ajustes para poder seleccionar imágenes.',
  'reading.openSettings': 'Abrir ajustes',
  'reading.imagePickErrorTitle': 'Error',
  'reading.imagePickErrorBody': 'No se pudo seleccionar la imagen. Intenta de nuevo.',
  'reading.citeSelectionToastBody': 'Selecciona una parte del texto del cuerpo.',
  'reading.intertitleToastBody': 'Debes escribir texto en el cuerpo después de un intertítulo antes de añadir otro.',
  'reading.bodyMinSuffix': 'mín',
  'reading.addImage': 'Añadir imagen',
  'reading.maxImagesHint': 'Máximo 3',
  'reading.selectedCitationLabel': 'Cita seleccionada',
  'reading.citedUsersTitle': 'Usuarios citados',
  'reading.citedUsersInputLabel': '@usuarios',
  'reading.voiceNoteIntroTitle': 'Nota inicial',
  'reading.voiceNoteIntroDescription': 'Se reproducirá antes del título.',
  'reading.voiceNoteInlineTitle': 'Nota de voz en el cuerpo',
  'reading.voiceNoteInlineDescription': 'Se reproducirá entre bloques del cuerpo.',
  'reading.voiceNoteOutroTitle': 'Nota final',
  'reading.voiceNoteOutroDescription': 'Se reproducirá debajo del cuerpo.',
  'reading.voiceNoteRecordAction': 'Grabar',
  'reading.voiceNoteRerecordAction': 'Regrabar',
  'reading.voiceNoteRecordingStatus': 'Grabando',
  'chat.selectGroup': 'Selecciona un grupo',
  'chat.toPrefix': 'Para:',
  'chat.request': 'Solicitar',
  'chat.requestPending': 'En espera',
  'chat.requestAccepted': 'Aceptada',
  'chat.requestBlocked': 'Bloqueado',
  'chat.members': 'Miembros',
  'chat.messageHidden': 'Mensaje oculto',
  'chat.messageVisible': 'Mensaje visible',
  'event.enableTasks': 'Activar tareas',
  'event.assignTask': 'Asignar tarea',
  'event.activateRequests': 'Activar solicitudes',
  'event.rewardTasks': 'Recompensar tareas',
  'event.describeTask': 'Describe la tarea',
  'event.userPlaceholder': '@usuario',
  'event.amount': 'Cantidad',
  'event.type': 'Tipo de evento',
  'event.requiredSuffix': '(obligatorio)',
  'event.name': 'Nombre del evento',
  'event.description': 'Descripción del evento',
  'event.dateAndTime': 'Fecha y hora',
  'event.dayMonthYear': 'Día/Mes/Año',
  'event.time': 'Hora',
  'event.durationTitle': 'Duración del evento',
  'event.duration': 'Tiempo',
  'event.endsIn': 'Termina en',
  'event.link': 'Enlace',
  'event.selectSocialNetwork': 'Seleccionar red social',
  'event.addLink': 'Añadir enlace',
  'event.location': 'Ubicación',
  'event.selectLocation': 'Seleccionar ubicación',
  'event.searchType': 'Buscar tipo de evento',
  'event.typeOption.social': 'Social',
  'event.typeOption.cultural': 'Cultural',
  'event.typeOption.artistic': 'Artístico',
  'event.typeOption.business': 'Empresarial',
  'event.typeOption.economic': 'Económico',
  'event.typeOption.academic': 'Académico',
  'event.typeOption.sports': 'Deportivo',
  'event.typeOption.entertainment': 'Entretenimiento',
  'event.typeOption.political': 'Político',
  'event.typeOption.religious': 'Religioso',
  'event.typeOption.marketing': 'Marketing',
  'event.typeOption.charity': 'Solidario',
  'event.typeOption.audiovisual': 'Audiovisual',
  'event.typeOption.scientific': 'Científico',
  'event.typeOption.technological': 'Tecnológico',
  'event.typeOption.children': 'Infantil',
  'event.typeOption.musical': 'Musical',
  'event.typeOption.automotive': 'Automovilístico',
  'event.typeOption.fitness': 'Fitness',
  'event.typeOption.gastronomic': 'Gastronómico',
  'event.typeOption.work': 'Laboral',

  'chat.lockedYourGroupsMessage': 'Autentica tu cuenta para poder crear tus grupos',
  'chat.lockedJoinedGroupsMessage': 'Para conectarte a los chats de los grupos a los que te has unido, autentica tu cuenta',
  'chat.hostOnlyEventHypeMessage': 'Los usuarios que se unan a tu canal son los únicos que pueden hypear tu evento',
  'chat.hostOnlyReadingHypeMessage': 'Los usuarios que se unan a tu canal son los únicos que pueden hypear tu lectura',

  'groups.edit': 'Editar',
  'groups.delete': 'Eliminar',
  'groups.deleteConfirmTitle': 'Confirmar eliminación',
  'groups.deleteConfirmBody': '¿Estás seguro que quieres eliminar este grupo? Todos los usuarios que se encuentran en este grupo serán expulsados',
  'groups.limit': 'Limitar',
  'groups.limited': 'Limitado',
  'groups.expel': 'Expulsar',
  'groups.expelAndBlock': 'Expulsar y Bloquear',
  'groups.leaveGroup': 'Salir del grupo',
  'groups.leaveAndBlock': 'Salir y Bloquear',
  'groups.limitUsers': 'Limitar usuarios',
  'groups.expelUsers': 'Expulsar usuarios',
  'groups.selectAll': 'Seleccionar todos',
  'groups.deselect': 'Deseleccionar',
  'groups.noMembersToShow': 'No hay miembros para mostrar',

  'notifications.groupJoinRequestMessage': '{user} te ha enviado una solicitud para que te unas a su grupo {group}. ¿Quieres unirte?',
  'notifications.ignore': 'Ignorar',
  'notifications.accept': 'Aceptar',
  'notifications.read': 'Leído',
  'notifications.unread': 'No leído',
  'notifications.empty': 'No tienes notificaciones',

  'config.title': 'Configuración',
  'config.accountCenter': 'Centro de cuenta',
  'config.blockedUsers': 'Usuarios bloqueados',
  'config.devicePermissions': 'Permisos del dispositivo',
  'config.helpCenter': 'Centro de ayuda',
  'config.information': 'Acerca de Keinti',
  'config.yourAccountIn': 'Tu cuenta en',

  'aboutKeinti.title': 'Acerca de Keinti',
  'aboutKeinti.moreAboutKeinti': 'Más de Keinti',
  'aboutKeinti.body': `Para todos los usuarios de Keinti:

Keinti es una red social orientada al descubrimiento de perfiles y a la interacción directa entre personas. Nuestro propósito es ofrecer un entorno dinámico que favorezca la visibilidad de los usuarios, dentro de Keinti y como complemento a su presencia digital.

Cada usuario dispone de un perfil público compuesto por una Presentación y contenedores donde puede incorporar intimidades y aspectos personales. Esta estructura busca facilitar que otros miembros conozcan mejor su identidad, intereses y estilo. Desde la pantalla Home, la plataforma promueve la exploración continua de perfiles creados por personas de distintas partes del mundo.

Cuando un perfil resulte de interés, el usuario podrá unirse al canal general del anfitrión/publicador e interactuar mediante hilos de conversación. Adicionalmente, el publicador podrá invitar a los usuarios a unirse a sus grupos, con el fin de consolidar y ampliar su comunidad.

En cuanto al desarrollo, Keinti ha sido construido íntegramente con el apoyo de inteligencia artificial (ChatGPT, Claude Sonnet y Gemini) a través de Visual Studio Code. En Keinti se ha iniciado un proceso de búsqueda de colaboración para incorporar socios y perfiles especializados (desarrollo, diseño gráfico, entre otros), con el objetivo de reforzar de manera prioritaria la seguridad y la experiencia visual de la aplicación.

Para propuestas de colaboración, por favor contacte a: keintisoporte@gmail.com

Atentamente,
Equipo Keinti`,

  'verifyKeinti.objectivesTitle': 'Objetivos para conseguir Keinti verificado:',
  'verifyKeinti.objective1': '• Autentifica tu cuenta primero.',
  'verifyKeinti.complete': 'Completar',
  'verifyKeinti.completed': 'Completado',
  'verifyKeinti.objective2': '• Obtén 100.000 aperturas en tus publicaciones de tus intimidades',
  'verifyKeinti.objective3': '• Obtén en un mismo mes al menos 1.000 reproducciones de otros usuarios en las imágenes que compartas en los chats de "Tu canal"',
  'verifyKeinti.objective4': '• Haz que 100.000 usuarios se unan al chat de "Tu canal"',
  'verifyKeinti.objective5': '• Crea al menos un grupo en \'Tus grupos\' y haz que se unan a estos grupos al menos 200 miembros activos.',
  'verifyKeinti.objective6': '• Publica tu perfil al menos 40 veces en la “Home”.',
  'verifyKeinti.benefitsTitle': 'Beneficios por conseguir Keinti verificado:',
  'verifyKeinti.benefit1': '• Obtienes la insignia dorada.',
  'verifyKeinti.benefit2': '• Generas ingresos por cada usuario que acceda a los chats de "Tu canal."',
  'verifyKeinti.benefit3': '• Generas ingresos por cada 1.000 reproducciones de las imágenes que compartas por los chats de "Tu Canal"',
  'verifyKeinti.benefit4': '• Obtén otros privilegios futuros ofrecidos por Keinti en las próximas actualizaciones.',
  'verifyKeinti.tabObjectives': 'Objetivos',
  'verifyKeinti.tabBenefits': 'Beneficios',
  'verifyKeinti.importantNotice': 'Aviso importante',
  'verifyKeinti.importantNoticeBody': 'La monetización llegará pronto a Keinti. Verifica tu cuenta para obtener todos los beneficios cuando estén disponibles.',
  'verifyKeinti.verifyAction': 'Verificar',
  'verifyKeinti.accountVerifiedLabel': 'Cuenta verificada',

  'devicePermissions.description': 'Aquí puedes ver los permisos que has concedido a la app.',
  'devicePermissions.galleryTitle': 'Galería (fotos)',
  'devicePermissions.galleryDescription': 'Se usa cuando añades una imagen al avatar, la aplicas en tu presentación o la incorporas en intimidades.',
  'devicePermissions.statusGranted': 'Concedido',
  'devicePermissions.statusDenied': 'No concedido',
  'devicePermissions.statusUnknown': 'No disponible',
  'devicePermissions.iosHint': 'En iOS este permiso lo gestiona el sistema y se solicitará cuando selecciones una imagen.',
  'devicePermissions.revokeConfirmTitle': 'Quitar permisos',
  'devicePermissions.revokeConfirmBody': "¿Estás seguro de que quieres quitarle los permisos a 'Keinti' para que pueda acceder a tu galería?",
  'devicePermissions.reEnableConfirmTitle': 'Permiso de Galería',
  'devicePermissions.reEnableConfirmBody': '¿Quieres permitir que Keinti tenga acceso a tu galería?',
  'devicePermissions.osRequestTitle': 'Permiso de Galería',
  'devicePermissions.osRequestMessage': 'La aplicación necesita acceso a tu galería para seleccionar fotos.',
  'devicePermissions.osRequestAskLater': 'Preguntar después',
  'devicePermissions.osRequestDeny': 'Cancelar',
  'devicePermissions.osRequestAllow': 'Aceptar',

  'accountCenter.title': 'Centro de cuenta',
  'accountCenter.personalData': 'Datos personales',
  'accountCenter.securityControl': 'Control de Seguridad',
  'accountCenter.changePassword': 'Cambiar contraseña',
  'accountCenter.adminSelfies': 'Admin: revisar selfies',
  'accountCenter.closeAccount': 'Cerrar sesión',
  'accountCenter.deleteAccount': 'Eliminar cuenta',
  'accountCenter.deleteAccountConfirmTitle': 'Eliminar cuenta',
  'accountCenter.deleteAccountConfirmBody': '¿Seguro que quieres eliminar tu cuenta? Esta acción es irreversible y eliminará tus datos del sistema.',
  'accountCenter.accountDeletionPolicyLink': 'Ver política de eliminación de cuenta',

  'personalData.title': 'Datos personales',
  'personalData.description': 'Keinti usa esta información para verificar tu identidad y para mantener segura a la comunidad.',
  'personalData.contactInfo': 'Información de contacto',
  'personalData.birthDate': 'Fecha de nacimiento',
  'personalData.gender': 'Género',
  'personalData.nationality': 'Nacionalidad',

  'securityControl.passwordAndAuth': 'Contraseña y autenticación',
  'securityControl.passwordAndAuthDescription': 'En este apartado podrás autenticar tu perfil y cambiar tu contraseña',
  'securityControl.accountAuth': 'Autenticación de la cuenta',
  'securityControl.privacyPolicy': 'Política de privacidad',
  'securityControl.cookiesAdPolicy': 'Política de Cookies y Publicidad',
  'securityControl.termsOfUse': 'Términos y condiciones de uso',
  'securityControl.childSafetyStandards': 'Estándares de seguridad infantil',
  'securityControl.accountDeletionPolicy': 'Política de eliminación de cuenta',
  'securityControl.verifyYourKeinti': 'Verifica tu Keinti',

  'accountAuth.description': 'Para autenticar tu cuenta debes completar 2 pasos: selfie de verificación y código de Google Authenticator. El selfie se valida automáticamente y, solo si el sistema no puede decidirlo con seguridad, pasa a revisión adicional.',
  'accountAuth.step1Title': 'Paso 1: Selfie de verificación',
  'accountAuth.step1Body': 'Hazte un selfie para validar automáticamente que la imagen es apta para seguridad y anti-suplantación antes de activar el paso 2.',
  'accountAuth.step1Hint': 'Asegúrate de tener buena iluminación y encuadrar tu rostro.',
  'accountAuth.step1Action': 'Hacer selfie',
  'accountAuth.step1Pending': 'Revisión adicional',
  'accountAuth.pendingReviewNotice': 'Tu selfie sigue en revisión. Esta revisión puede tardar hasta 24 horas. Si, pasado ese tiempo, al pulsar "Actualizar estado" sigue apareciendo "Revisión adicional", contacta con keintisoporte@gmail.com.',
  'accountAuth.step1Blocked': 'Bloqueado',
  'accountAuth.refreshStatus': 'Actualizar estado',
  'accountAuth.selfieAccepted': 'Selfie aceptado',
  'accountAuth.statusNotSubmitted': 'Estado: pendiente de enviar.',
  'accountAuth.statusPending': 'Estado: revisión adicional requerida.',
  'accountAuth.statusAccepted': 'Estado: aceptado. Paso 2 disponible.',
  'accountAuth.statusFailed': 'Estado: fallido. Reintenta con mejor iluminación y encuadre.',
  'accountAuth.statusBlocked': 'Estado: bloqueado. No puedes reenviar selfies hasta que un administrador lo desbloquee.',
  'accountAuth.errorTitle': 'Error',
  'accountAuth.selfieUploadError': 'No se pudo enviar el selfie. Intenta de nuevo.',

  'accountAuth.step2Title': 'Paso 2: Google Authenticator',
  'accountAuth.step2Body': 'Configura Google Authenticator y verifica el código para finalizar la autenticación.',
  'accountAuth.step2Locked': 'Este paso se activará cuando el selfie sea aceptado.',
  'accountAuth.step2Ready': 'Listo para configurar Google Authenticator.',
  'accountAuth.step2AlreadyEnabled': 'Google Authenticator ya está activado.',
  'accountAuth.generateTotp': 'Generar configuración',
  'accountAuth.secretLabel': 'Clave secreta',
  'accountAuth.secretHint': 'Añade una cuenta en Google Authenticator con esta clave (manual). Luego introduce el código de 6 dígitos.',
  'accountAuth.codeLabel': 'Código',
  'accountAuth.codePlaceholder': '123456',
  'accountAuth.verifyCode': 'Verificar código',
  'accountAuth.totpSetupError': 'No se pudo generar la configuración.',
  'accountAuth.verifyError': 'No se pudo verificar el código.',
  'accountAuth.successTitle': 'Completado',
  'accountAuth.successBody': 'Tu cuenta ha sido autenticada y ya tienes la insignia.',
  'accountAuth.completed': 'Autenticación completada.',
  'accountAuth.badgeExpiresIn': 'La insignia se desactiva en:',

  'changePassword.requirements': 'La contraseña debe tener al menos 10 caracteres e incluir al menos un carácter especial (#!@_$€%)',
  'changePassword.currentPassword': 'Contraseña actual',
  'changePassword.currentPasswordInvalid': 'Contraseña incorrecta',
  'changePassword.newPassword': 'Contraseña nueva',
  'changePassword.repeatNewPassword': 'Repetir contraseña nueva',
  'changePassword.passwordsDontMatch': 'Las contraseñas no coinciden',
  'changePassword.forgot': '¿Olvidaste tu contraseña?',
  'changePassword.success': 'Contraseña actualizada',
  'changePassword.locked': 'Has alcanzado el máximo de intentos. Vuelve a intentarlo más tarde.',
  'changePassword.attemptsRemaining': 'Intentos restantes: {count}',
  'changePassword.accountLocked': 'Tu cuenta ha sido bloqueada. Se cerrará sesión automáticamente.',
  'changePassword.checkCompleted': 'Comprobación completada',

  'blockedUsers.title': 'Usuarios bloqueados',
  'blockedUsers.empty': 'No tienes usuarios bloqueados',

  'adminSelfies.title': 'Admin · Selfies',
  'adminSelfies.tabPending': 'Revisión adicional',
  'adminSelfies.tabBlocked': 'Bloqueados',
  'adminSelfies.refresh': 'Actualizar',
  'adminSelfies.emptyPending': 'No hay selfies en revisión adicional.',
  'adminSelfies.emptyBlocked': 'No hay usuarios bloqueados.',
  'adminSelfies.reasonPlaceholder': 'Motivo (opcional)',
  'adminSelfies.accept': 'Aceptar',
  'adminSelfies.reject': 'Rechazar',
  'adminSelfies.block': 'Bloquear',
  'adminSelfies.unblock': 'Desbloquear',
  'blockedUsers.reasonPlaceholder': 'Sin motivo',

  'toast.joinedChannelOf': 'Te has unido al canal de',
  'toast.published': 'Has publicado',
  'toast.publicationNotActiveHome': "La publicación ya no está activa en la 'Home'",
  'toast.joinedGroupMessage': 'Te has unido al grupo {group} de {user}',
  'toast.blocked': 'Bloqueado',
  'toast.reportSent': 'Denuncia enviada',

  'carouselEditor.adjustImage': 'Ajusta la imagen',
  'carouselEditor.imageCount': 'Imagen {current} de {total}',
  'carouselEditor.processing': 'Procesando...',
  'carouselEditor.noPreview': 'Sin vista previa',
  'carouselEditor.editImage': 'Editar imagen',
  'carouselEditor.errorTitle': 'Error',
  'carouselEditor.cropErrorBody': 'No se pudo recortar la imagen. Por favor, intenta de nuevo.',
  'carouselEditor.ok': 'OK',
};

const en: Record<TranslationKey, string> = {
  'language.label': 'Language',
  'language.spanish': 'Spanish',
  'language.english': 'English',
  'language.french': 'French',
  'language.portuguese': 'Portuguese',
  'language.german': 'German',
  'language.italian': 'Italian',
  'gender.male': 'Male',
  'gender.female': 'Female',
  'gender.unspecified': 'Prefer not to say',
  'common.loading': 'Loading...',
  'common.logout': 'Log out',
  'common.notifications': 'Notifications',
  'common.keys': 'Keys',
  'common.or': 'or',
  'common.unlock': 'Unblock',
  'common.view': 'View',
  'common.forgotPassword': 'Forgot your [[password]]?',
  'common.cancel': 'Cancel',
  'common.confirm': 'Confirm',
  'common.delete': 'Delete',
  'common.report': 'Report',
  'common.block': 'Block',
  'common.check': 'Check',
  'common.apply': 'Apply',
  'common.accept': 'Accept',
  'common.create': 'Create',
  'keys.convert': 'Convert',
  'keys.dailyGoals': 'Daily goals',
  'keys.enterAmount': 'Enter amount',
  'keys.convertMultipleOfTenHint': 'The amount must be a multiple of 10.',
  'keys.convertInsufficientWhiteKeys': 'You do not have enough white keys for this conversion.',
  'keys.convertRateHint': 'Every 10 white keys convert into 1 gradient key.',
  'keys.redeem': 'Redeem',
  'keys.redeemSubtitle': 'Select one of the options',
  'keys.redeemOption10': '10 euros',
  'keys.redeemOption20': '20 euros',
  'keys.redeemOption50': '50 euros',
  'keys.redeemOption100': '100 euros',
  'keys.redeemOption500': '500 euros',
  'keys.redeemInsufficientKeys': 'You do not have enough keys.',
  'keys.redeemNote': 'You can only redeem one coupon per day. Choose carefully. The redeemed money will be sent to the PayPal account you provide in the next step after tapping the "Redeem" button.',
  'keys.lockedMessage': 'Authenticate your account to access your daily goals and keys.',
  'keys.dailyGoal1': 'For each intimacy container you open, you will earn 2 white keys.',
  'keys.dailyGoal2': 'Manage to link a thread with a host in their channel.',
  'keys.dailyGoal3': 'Unlock the images shared by hosts in the channels you join and earn 10 white keys for each one.',
  'keys.dailyGoal4': 'Publish your profile every day.',
  'keys.dailyGoal5': 'Create an event in your channel.',
  'keys.dailyGoal6': 'Share an image in your channel.',
  'keys.dailyGoal7': 'Get 100 users to enter your channel: 1,000 white keys.',
  'keys.dailyGoal8': 'Get 1,000 users to enter your channel: 10,000 white keys.',
  'keys.dailyGoal9': 'Get 10,000 users to enter your channel: 100,000 white keys.',

  'auth.sessionRequiredTitle': 'Session required',
  'auth.signInToBlockUsers': 'Sign in to block users.',
  'auth.signInToReportUsers': 'Sign in to report users.',

  'home.blockConfirmQuestion': 'Do you want to block {user}?',
  'home.blockReasonPlaceholder': 'Block reason (optional)...',
  'home.reportSelectReason': 'Select a reason to report {user}.',

  'report.reason.sexualContent': 'Sexual content or nudity',
  'report.reason.harassment': 'Harassment or bullying',
  'report.reason.offensiveLanguage': 'Offensive language',
  'report.reason.scam': 'Scam or fraud',
  'report.reason.violence': 'Violence or threats',
  'report.reason.spam': 'Spam',
  'report.reason.impersonation': 'Impersonation',
  'report.reason.illegalContent': 'Illegal content',
  'report.reason.childSexualAbuse': 'Child sexual abuse/exploitation',
  'report.reason.weaponsOrDrugsIncitement': 'Incitement to use weapons/drugs',
  'report.reason.inappropriateConduct': 'Inappropriate conduct',
  'report.reason.other': 'Other',

  'errors.unableToBlockUser': 'Unable to block the user',
  'errors.unableToSendReport': 'Unable to send the report',

  'toast.joinedChannelOf': "You've joined the channel of",
  'toast.published': 'Posted',
  'toast.publicationNotActiveHome': 'The post is no longer active on Home',
  'toast.joinedGroupMessage': "You've joined group {group} from {user}",
  'toast.blocked': 'Blocked',
  'toast.reportSent': 'Report sent',

  'carouselEditor.adjustImage': 'Adjust image',
  'carouselEditor.imageCount': 'Image {current} of {total}',
  'carouselEditor.processing': 'Processing...',
  'carouselEditor.noPreview': 'No preview',
  'carouselEditor.editImage': 'Edit image',
  'carouselEditor.errorTitle': 'Error',
  'carouselEditor.cropErrorBody': 'Unable to crop the image. Please try again.',
  'carouselEditor.ok': 'OK',

  'login.email': 'Email',
  'login.password': 'Password',
  'login.emailPlaceholder': 'example@email.com',
  'login.passwordPlaceholder': 'Enter your password',
  'login.signIn': 'Sign in',
  'login.fillAllFields': 'Please fill in all fields',
  'login.invalidCredentials': 'Invalid credentials',
  'login.googleNotRegistered': 'This account is not registered yet. Please sign up first to be able to sign in with this email address.',
  'login.googlePkceMissing': 'Could not complete Google sign-in. Please try again.',
  'login.noAccount': "Don't have an account? ",
  'login.signUp': 'Sign up',

  'login.resetTitle': 'Reset password',
  'login.resetSend': 'Send',
  'login.resetVerify': 'Verify',
  'login.resetChangePassword': 'Change password',
  'login.resetEmailNotRegistered': 'This email is not yet used in Keinti by any user. Sign up.',
  'login.resetInvalidEmail': 'Invalid email',
  'login.resetCodeSent': 'We sent a code to your email.',
  'login.resetPasswordChanged': 'Password updated. You can sign in now.',
  'login.resetCodeLabel': 'Code',
  'login.resetNewPasswordLabel': 'Create new password',
  'login.resetRepeatNewPasswordLabel': 'Repeat new password',

  'register.email': 'Email',
  'register.username': '@username',
  'register.usernameAlreadyInUse': 'The "{username}" is already being used by another user; please try a different one.',
  'register.birthDate': 'Date of birth',
  'register.birthDatePlaceholder': 'DD/MM/YYYY',
  'register.gender': 'Gender',
  'register.selectGender': 'Select your gender',
  'register.nationality': 'Nationality',
  'register.password': 'Password',
  'register.confirmPassword': 'Confirm password',
  'register.confirmPasswordPlaceholder': 'Confirm your password',
  'register.next': 'Next',
  'register.signUp': 'Sign up',
  'register.registering': 'Signing up...',
  'register.successMessage': 'Welcome! You are now a Keinti user.',
  'register.privacyPrefix': 'By signing up, you accept the ',
  'register.privacyPolicies': 'privacy policy, cookies and ads policy, and the terms and conditions of use',
  'register.privacyPolicyLink': 'privacy policy',
  'register.cookiesAdsPolicyLink': 'cookies and ads policy',
  'register.termsConditionsLink': 'the terms and conditions of use',
  'register.privacySeparator1': ', ',
  'register.privacySeparator2': ', and ',
  'register.privacySuffix': ' of our Keinti app. For your safety, read them before signing up.',
  'register.errorTitle': 'Sign up error',
  'register.unableToComplete': 'Unable to complete sign up.',
  'register.selectNationality': 'Select your nationality',
  'register.searchCountry': 'Search country...',
  'register.noCountriesFound': 'No countries found',

  'register.emailVerificationTitle': 'Email verification',
  'register.emailVerificationHint': 'We sent a code to',
  'register.emailVerificationLinkHint': 'If you receive a confirmation link instead of a code, open it and you will return to Keinti automatically to complete signup.',
  'register.iConfirmedEmail': 'I already confirmed the email',
  'register.confirmedCheckHint': 'If you already opened the link, return to Keinti. If nothing happens, wait a few seconds and tap again, or resend the email.',
  'register.code': 'Code',
  'register.codePlaceholder': 'Enter the code',
  'register.timeRemaining': 'Time remaining',
  'register.verifyAndSignUp': 'Verify and sign up',
  'register.verifying': 'Verifying...',
  'register.resendCode': 'Resend email',
  'register.codeExpired': 'The code has expired. Request a new one.',
  'register.codeIncorrect': 'Incorrect code',
  'register.codeInvalidInline': 'The code you entered is not valid. Please try again.',
  'register.remainingAttempts': 'Remaining attempts',
  'register.remainingAttemptsInline': 'Remaining attempts: {count}',
  'register.emailTemporarilyBlocked': 'Too many attempts. Restart sign up with a different email.',
  'register.emailLockedInlineError': 'This email address has been temporarily blocked. We are reviewing it. Please send us a claim so we can reactivate your email address in Keinti.',
  'register.emailLockedRetryInline': 'This email address has been temporarily blocked. Try another one or wait until your rectification request is resolved.',
  'register.rectificationTitle': 'Rectification',
  'register.rectificationPlaceholder': 'Describe your rectification (max 220 characters)',
  'register.rectificationSend': 'Send claim',
  'register.rectificationSent': 'Claim sent. We will review your request as soon as possible.',
  'register.tryLaterOrCheckEmailExists': 'Try again later or check the email exists',
  'register.mustVerifyEmailFirst': 'You must verify your email before signing up',
  'register.cancelledNotice': 'Registration cancelled. If you want to create a new Keinti account, please start the sign up process again.',
  'register.emailAlreadyInUse': 'This email is already in use by another Keinti user. Please try a different one.',
  'register.emailSendFailed': 'We could not send the code to the email "{email}". Please verify the email address is valid.',
  'register.missingProfileData': 'Missing signup data. Go back and complete username, birth date, gender and nationality.',
  'register.signupExpiredNotice': 'Verification time expired. Your registration data has been cleared. You can try again.',
  'register.signupExpiredTitle': 'Time expired',
  'register.emailLocked24h': 'You have used all {max} signup attempts with this email. You cannot use it again for 48 hours.',
  'register.attemptsRemaining': 'Remaining attempts: {remaining} of {max}',
  'register.pendingConfirmation': 'Confirm your email before the timer runs out — it is still active.',
  'register.emailLockedTemp': 'You have used all {max} signup attempts with this email. You cannot use it again for 48 hours.',
  'register.emailLockedPermanent': 'Your email has been permanently blocked due to exceeding the signup attempt limit. Contact keintisoporte@gmail.com to request unblocking.',

  'validation.invalidDateFormat': 'Invalid date format',
  'validation.invalidDate': 'Invalid date',
  'validation.mustBeAdult': 'You must be at least 18 years old',
  'validation.passwordMinLength': 'Password must be at least 10 characters',
  'validation.passwordMaxLength': 'Password must be at most 20 characters',
  'validation.passwordNeedsLowercase': 'Must include at least 1 lowercase letter',
  'validation.passwordNeedsUppercase': 'Must include at least 1 uppercase letter',
  'validation.passwordNeedsLetter': 'Must include at least 1 letter',
  'validation.passwordNeedsNumber': 'Must include at least 1 number',
  'validation.passwordNeedsSpecial': 'Must include at least 1 special character',
  'validation.passwordsDontMatch': "Passwords don't match",

  'front.editProfilePhoto': 'Edit profile photo',
  'front.yourSocialNetworks': 'Your social networks',
  'front.userPlaceholder': 'User',
  'front.selectCategory': 'Select a category',
  'front.noPublicationsAvailable': 'No posts available',
  'front.forYou': 'For you',
  'front.publicationsCount': 'Posts',
  'front.socialNetworksSingular': 'social network',
  'front.socialNetworksPlural': 'social networks',
  'front.publish': 'Publish',
  'front.remove': 'Remove (Published)',
  'front.yourProfile': 'Your profile',
  'front.yourPresentation': 'Your presentation',
  'front.intimacies': 'Intimacies',
  'front.time': 'Time',
  'front.loadingImages': 'Loading images...',
  'front.readMore': 'Read more...',
  'front.readLess': 'Read less',
  'front.votes': 'votes',

  'front.profileEmptyPrimaryMessage': "Add 'Your presentation' and at least one 'Intimacy' to publish your profile",
  'front.profileEmptySecondaryMessage': 'By publishing your profile, other users will be able to access your channel, where you can share content and interact with them',
  'front.profileRingHintSelect': 'Select a part of one of the carousel images',
  'front.profileRingHintMaxRings': 'Max reached: 5 rings',
  'front.profileRingDelete': 'Delete ring',
  'front.profileRingNameLabel': 'Name (max 38 characters)',
  'front.profileRingNameHelper': 'Product, company, user, channel, content, etc',
  'front.profileRingDescriptionLabel': 'Description (max 280 characters)',
  'front.profileRingLinkLabel': 'Link',
  'front.profileRingLinkHelper': 'Add a social network link so users can access the product, company, user, channel, post, content, etc.',
  'front.profileRingLocationLabel': 'Location',
  'front.profileRingLocationHelper': 'Add a location',

  'profile.deleteContentTitle': 'Delete content',
  'profile.deleteContentBody': 'Are you sure you want to delete this content from your profile?',
  'front.reactions': 'Reactions',
  'front.deletePublicationTitle': 'Remove post',
  'front.deletePublicationBody': 'Your profile is currently public to other users on the “Home” page; if you remove it, only you will be able to view it in “Your Profile”.',
  'front.category': 'Category',
  'front.presentationAddCarouselHint': 'Add up to 3 images to the carousel',
  'front.presentationTitleLabel': 'Title',
  'front.presentationTitlePlaceholder': 'Write a title...',
  'front.presentationBodyLabel': 'Presentation',
  'front.presentationBodyPlaceholder': 'Write your presentation...',
  'front.presentationLockedTitle': 'Editing locked',
  'front.presentationLockedBody': 'Your profile is currently published.',

  'front.selectSocialNetwork': 'Select a social network',
  'front.link': 'Link',
  'front.linkMustBeFrom': 'Link must be from',

  'front.incorporate': 'Add',
  'front.answerPlaceholder': 'Answer...',
  'front.chooseCorrectOption': 'Choose the correct option',
  'front.addMoreSurveyOptions': 'Add more options to the survey',
  'front.image': 'Image',
  'front.text': 'Text',
  'front.draftPlaceholder': 'Write...',
  'front.draftIntimacyPlaceholder': 'Write an intimacy...',
  'front.intimaciesPublishBlocked': "You can't post intimacies yet. First create 'Your presentation'.",
  'front.homeSwipeTutorialHint': 'When the indicator bar at the top of the screen is active, swipe down to change the post.',
  'front.channelLabel': 'Channel:',

  'chat.tabYourChannel': 'Your channel',
  'chat.tabChannel': 'Channel',
  'chat.tabJoined': 'Joined',
  'chat.tabYourGroups': 'Your groups',
  'chat.tabGroups': 'Groups',
  'chat.postOnHomeToActivateChannel': 'Post on Home to activate your channel',
  'chat.noChannelsYet': "You don't have any channels yet",
  'chat.createGroup': 'Create a group',
  'chat.notJoinedAnyGroupYet': "You haven't joined any group yet",
  'chat.back': 'Back',
  'chat.interactPlaceholder': 'Interact...',
  'chat.timeExpired': 'Time expired',
  'chat.remainingPrefix': 'Remaining',
  'chat.hour': 'hour',
  'chat.hours': 'hours',
  'chat.minute': 'minute',
  'chat.minutes': 'minutes',
  'chat.reply': 'Reply',
  'chat.generalTab': 'General',
  'chat.yourThreadsTab': 'Threads',
  'chat.recommendationsTab': 'Recommendations',
  'chat.eventsTab': 'Events',
  'chat.readingsTab': 'Readings',
  'chat.imagesTab': 'Images',
  'chat.limitWarningMessage': "You've already interacted in the channel of {user}. Please wait for the reply.",
  'chat.hostLimitedInteractions': '{user} has limited your interactions',
  'chat.waitHostReplyThread': 'Wait for the host response to continue the thread',
  'chat.waitCreatorReplyAgain': 'Wait for the host response before replying again',
  'chat.selectGroup': 'Select a group',
  'chat.signInToSendMessages': 'Sign in to send messages.',
  'chat.unableToSendMessage': 'Unable to send the message.',
  'chat.connectionErrorSendingMessage': 'Connection error while sending the message.',
  'chat.addCaptionPlaceholder': 'Add a caption...',
  'chat.imageCaptionPlaceholder': 'Image caption...',
  'reading.screenTitle': 'Create reading',
  'reading.titlePlaceholder': 'Title',
  'reading.subtitlePlaceholder': 'Subtitle',
  'reading.leadPlaceholder': 'Lead...',
  'reading.dateLabel': 'Date',
  'reading.datePlaceholder': 'DD/MM/YYYY',
  'reading.chooseCategoryLabel': 'Choose a Category',
  'reading.categoryLabel': 'Category',
  'reading.searchCategoryPlaceholder': 'Search category',
  'reading.hypeCostLabel': 'Hype cost',
  'reading.hypeCostInfo': 'Choose how many white keys you want each channel user to donate for this reading to boost your profile. The more keys you receive, the more your profile and channel will spread on the Hype screen.',
  'reading.hypeCostAmountLabel': 'Amount',
  'reading.bodyPlaceholder': 'Body...',
  'reading.intertitlePlaceholder': 'Intertitle',
  'reading.imageLimitReachedTitle': 'Limit reached',
  'reading.imageLimitReachedBody': 'You can only add up to 3 images in the reading.',
  'reading.galleryPermissionTitle': 'Gallery permission',
  'reading.galleryPermissionMessage': 'The app needs access to your gallery to select photos.',
  'reading.galleryPermissionAskLater': 'Ask later',
  'reading.galleryPermissionRequiredTitle': 'Permission required',
  'reading.galleryPermissionRequiredBody': 'You must enable Photos access from Settings to select images.',
  'reading.openSettings': 'Open settings',
  'reading.imagePickErrorTitle': 'Error',
  'reading.imagePickErrorBody': 'The image could not be selected. Please try again.',
  'reading.citeSelectionToastBody': 'Select a portion of the body text.',
  'reading.intertitleToastBody': 'You must write body text after an intertitle before adding another one.',
  'reading.bodyMinSuffix': 'min',
  'reading.addImage': 'Add image',
  'reading.maxImagesHint': 'Maximum 3',
  'reading.selectedCitationLabel': 'Selected quote',
  'reading.citedUsersTitle': 'Cited users',
  'reading.citedUsersInputLabel': '@users',
  'reading.voiceNoteIntroTitle': 'Intro voice note',
  'reading.voiceNoteIntroDescription': 'It will play before the title.',
  'reading.voiceNoteInlineTitle': 'Voice note in body',
  'reading.voiceNoteInlineDescription': 'It will play between body blocks.',
  'reading.voiceNoteOutroTitle': 'Final voice note',
  'reading.voiceNoteOutroDescription': 'It will play below the body text.',
  'reading.voiceNoteRecordAction': 'Record',
  'reading.voiceNoteRerecordAction': 'Re-record',
  'reading.voiceNoteRecordingStatus': 'Recording',
  'chat.toPrefix': 'To:',
  'chat.request': 'Request',
  'chat.requestPending': 'Pending',
  'chat.requestAccepted': 'Accepted',
  'chat.requestBlocked': 'Blocked',
  'chat.members': 'Members',
  'chat.messageHidden': 'Message hidden',
  'chat.messageVisible': 'Message visible',
  'event.enableTasks': 'Enable tasks',
  'event.assignTask': 'Assign task',
  'event.activateRequests': 'Activate requests',
  'event.rewardTasks': 'Reward tasks',
  'event.describeTask': 'Describe the task',
  'event.userPlaceholder': '@user',
  'event.amount': 'Amount',
  'event.type': 'Event type',
  'event.requiredSuffix': '(required)',
  'event.name': 'Event name',
  'event.description': 'Event description',
  'event.dateAndTime': 'Date and time',
  'event.dayMonthYear': 'Day/Month/Year',
  'event.time': 'Time',
  'event.durationTitle': 'Event duration',
  'event.duration': 'Duration',
  'event.endsIn': 'Ends in',
  'event.link': 'Link',
  'event.selectSocialNetwork': 'Select social network',
  'event.addLink': 'Add link',
  'event.location': 'Location',
  'event.selectLocation': 'Select location',
  'event.searchType': 'Search event type',
  'event.typeOption.social': 'Social',
  'event.typeOption.cultural': 'Cultural',
  'event.typeOption.artistic': 'Artistic',
  'event.typeOption.business': 'Business',
  'event.typeOption.economic': 'Economic',
  'event.typeOption.academic': 'Academic',
  'event.typeOption.sports': 'Sports',
  'event.typeOption.entertainment': 'Entertainment',
  'event.typeOption.political': 'Political',
  'event.typeOption.religious': 'Religious',
  'event.typeOption.marketing': 'Marketing',
  'event.typeOption.charity': 'Charity',
  'event.typeOption.audiovisual': 'Audiovisual',
  'event.typeOption.scientific': 'Scientific',
  'event.typeOption.technological': 'Technological',
  'event.typeOption.children': 'Children',
  'event.typeOption.musical': 'Musical',
  'event.typeOption.automotive': 'Automotive',
  'event.typeOption.fitness': 'Fitness',
  'event.typeOption.gastronomic': 'Gastronomic',
  'event.typeOption.work': 'Work',

  'chat.lockedYourGroupsMessage': 'Authenticate your account to create your groups',
  'chat.lockedJoinedGroupsMessage': 'To connect to the chats of the groups you have joined, authenticate your account',
  'chat.hostOnlyEventHypeMessage': 'Users who join your channel are the only ones who can boost your event',
  'chat.hostOnlyReadingHypeMessage': 'Users who join your channel are the only ones who can boost your reading',

  'groups.edit': 'Edit',
  'groups.delete': 'Delete',
  'groups.deleteConfirmTitle': 'Confirm deletion',
  'groups.deleteConfirmBody': 'Are you sure you want to delete this group? All users in this group will be removed.',
  'groups.limit': 'Limit',
  'groups.limited': 'Limited',
  'groups.expel': 'Expel',
  'groups.expelAndBlock': 'Expel and block',
  'groups.leaveGroup': 'Leave group',
  'groups.leaveAndBlock': 'Leave and block',
  'groups.limitUsers': 'Limit users',
  'groups.expelUsers': 'Expel users',
  'groups.selectAll': 'Select all',
  'groups.deselect': 'Deselect',
  'groups.noMembersToShow': 'No members to display',

  'notifications.groupJoinRequestMessage': '{user} sent you a request to join their group {group}. Do you want to join?',
  'notifications.ignore': 'Ignore',
  'notifications.accept': 'Accept',
  'notifications.read': 'Read',
  'notifications.unread': 'Unread',
  'notifications.empty': "You don't have notifications",

  'config.title': 'Settings',
  'config.accountCenter': 'Account Center',
  'config.blockedUsers': 'Blocked users',
  'config.devicePermissions': 'Device permissions',
  'config.helpCenter': 'Help center',
  'config.information': 'About Keinti',
  'config.yourAccountIn': 'Your account on',

  'aboutKeinti.title': 'About Keinti',
  'aboutKeinti.moreAboutKeinti': 'More about Keinti',
  'aboutKeinti.body': `For all Keinti users:

Keinti is a social network focused on profile discovery and direct interaction between people. Our goal is to provide a dynamic environment that helps users increase their visibility—within Keinti and as a complement to their broader online presence.

Each user can build a public profile composed of a Presentation and containers where they may share personal insights and aspects of their identity. This structure is intended to help other members better understand their interests and style. From the Home screen, the platform promotes continuous exploration of profiles created by people around the world.

When a profile is of interest, users may join the host/publisher’s general channel and engage through conversation threads. Publishers may also invite users to join their groups in order to strengthen and grow their communities.

Regarding development, Keinti has been built entirely with the support of artificial intelligence (ChatGPT, Claude Sonnet, and Gemini) through Visual Studio Code. Keinti has initiated a collaboration search process to welcome partners and specialized contributors (development, graphic design, and other roles), with a primary focus on strengthening security and improving the product’s visual experience.

For collaboration proposals, please contact: keintisoporte@gmail.com

Sincerely,
Keinti Team`,

  'verifyKeinti.objectivesTitle': 'Targets to become Keinti Verified:',
  'verifyKeinti.objective1': '• Authenticate your account first.',
  'verifyKeinti.complete': 'Complete',
  'verifyKeinti.completed': 'Completed',
  'verifyKeinti.objective2': '• Get 100,000 openings on your intimacy posts',
  'verifyKeinti.objective3': '• In a single month, get at least 1,000 views from other users on the images you share in "Your channel" chats',
  'verifyKeinti.objective4': '• Get 100,000 users to join your "Your channel" chat',
  'verifyKeinti.objective5': '• Create at least one group in \'Your groups\' and have at least 200 active members join these groups.',
  'verifyKeinti.objective6': '• Publish your profile at least 40 times in the “Home”.',
  'verifyKeinti.benefitsTitle': 'Benefits of becoming Keinti Verified:',
  'verifyKeinti.benefit1': '• Get the golden badge.',
  'verifyKeinti.benefit2': '• Generate income for each user who accesses "Your channel" chats.',
  'verifyKeinti.benefit3': '• Generate income for every 1,000 views of the images you share in "Your Channel" chats',
  'verifyKeinti.benefit4': '• Get other future privileges offered by Keinti in upcoming updates.',
  'verifyKeinti.tabObjectives': 'Targets',
  'verifyKeinti.tabBenefits': 'Benefits',
  'verifyKeinti.importantNotice': 'Important notice',
  'verifyKeinti.importantNoticeBody': 'Monetization is coming soon to Keinti. Verify your account to get all the benefits when they become available.',
  'verifyKeinti.verifyAction': 'Verify',
  'verifyKeinti.accountVerifiedLabel': 'Account verified',

  'devicePermissions.description': 'Here you can see the permissions you have granted to the app.',
  'devicePermissions.galleryTitle': 'Gallery (photos)',
  'devicePermissions.galleryDescription': 'Used when you add an image to your avatar, apply it to your presentation, or include it in intimacies.',
  'devicePermissions.statusGranted': 'Granted',
  'devicePermissions.statusDenied': 'Not granted',
  'devicePermissions.statusUnknown': 'Unavailable',
  'devicePermissions.iosHint': 'On iOS, this permission is managed by the system and will be requested when you select an image.',
  'devicePermissions.revokeConfirmTitle': 'Remove permission',
  'devicePermissions.revokeConfirmBody': "Are you sure you want to remove Keinti's permission to access your photo gallery?",
  'devicePermissions.reEnableConfirmTitle': 'Gallery permission',
  'devicePermissions.reEnableConfirmBody': 'Do you want to allow Keinti to access your photo gallery?',
  'devicePermissions.osRequestTitle': 'Gallery permission',
  'devicePermissions.osRequestMessage': 'The app needs access to your gallery to select photos.',
  'devicePermissions.osRequestAskLater': 'Ask later',
  'devicePermissions.osRequestDeny': 'Cancel',
  'devicePermissions.osRequestAllow': 'Allow',

  'accountCenter.title': 'Account Center',
  'accountCenter.personalData': 'Personal data',
  'accountCenter.securityControl': 'Security control',
  'accountCenter.changePassword': 'Change password',
  'accountCenter.adminSelfies': 'Admin: review selfies',
  'accountCenter.closeAccount': 'Log out',
  'accountCenter.deleteAccount': 'Delete account',
  'accountCenter.deleteAccountConfirmTitle': 'Delete account',
  'accountCenter.deleteAccountConfirmBody': 'Are you sure you want to delete your account? This action is irreversible and will remove your data from the system.',
  'accountCenter.accountDeletionPolicyLink': 'View account deletion policy',

  'personalData.title': 'Personal data',
  'personalData.description': 'Keinti uses this information to verify your identity and to keep the community safe.',
  'personalData.contactInfo': 'Contact information',
  'personalData.birthDate': 'Date of birth',
  'personalData.gender': 'Gender',
  'personalData.nationality': 'Nationality',

  'securityControl.passwordAndAuth': 'Password and authentication',
  'securityControl.passwordAndAuthDescription': 'In this section you can authenticate your profile and change your password.',
  'securityControl.accountAuth': 'Account authentication',
  'securityControl.privacyPolicy': 'Privacy policy',
  'securityControl.cookiesAdPolicy': 'Cookies and advertising policy',
  'securityControl.termsOfUse': 'Terms of use',
  'securityControl.childSafetyStandards': 'Child safety standards',
  'securityControl.accountDeletionPolicy': 'Account deletion policy',
  'securityControl.verifyYourKeinti': 'Verify your Keinti',

  'accountAuth.description': 'To authenticate your account you must complete 2 steps: a verification selfie and a Google Authenticator code. The selfie is validated automatically and only falls back to additional review when the system cannot decide safely.',
  'accountAuth.step1Title': 'Step 1: Verification selfie',
  'accountAuth.step1Body': 'Take a selfie so the system can automatically validate that the image is suitable for security and anti-impersonation before enabling step 2.',
  'accountAuth.step1Hint': 'Make sure you have good lighting and frame your face.',
  'accountAuth.step1Action': 'Take selfie',
  'accountAuth.step1Pending': 'Additional review',
  'accountAuth.pendingReviewNotice': 'Your selfie is still under review. This review can take up to 24 hours. If, after that time, tapping "Refresh status" still shows "Additional review", contact keintisoporte@gmail.com.',
  'accountAuth.step1Blocked': 'Blocked',
  'accountAuth.refreshStatus': 'Refresh status',
  'accountAuth.selfieAccepted': 'Selfie accepted',
  'accountAuth.statusNotSubmitted': 'Status: not submitted yet.',
  'accountAuth.statusPending': 'Status: additional review required.',
  'accountAuth.statusAccepted': 'Status: accepted. Step 2 available.',
  'accountAuth.statusFailed': 'Status: failed. Retry with better lighting and framing.',
  'accountAuth.statusBlocked': 'Status: blocked. You cannot submit more selfies until an admin unblocks you.',
  'accountAuth.errorTitle': 'Error',
  'accountAuth.selfieUploadError': 'Could not upload the selfie. Please try again.',

  'accountAuth.step2Title': 'Step 2: Google Authenticator',
  'accountAuth.step2Body': 'Set up Google Authenticator and verify the code to complete authentication.',
  'accountAuth.step2Locked': 'This step will be enabled once the selfie is accepted.',
  'accountAuth.step2Ready': 'Ready to set up Google Authenticator.',
  'accountAuth.step2AlreadyEnabled': 'Google Authenticator is already enabled.',
  'accountAuth.generateTotp': 'Generate setup',
  'accountAuth.secretLabel': 'Secret key',
  'accountAuth.secretHint': 'Add an account in Google Authenticator using this key (manual). Then enter the 6-digit code.',
  'accountAuth.codeLabel': 'Code',
  'accountAuth.codePlaceholder': '123456',
  'accountAuth.verifyCode': 'Verify code',
  'accountAuth.totpSetupError': 'Could not generate the setup.',
  'accountAuth.verifyError': 'Could not verify the code.',
  'accountAuth.successTitle': 'Completed',
  'accountAuth.successBody': 'Your account has been authenticated and the badge is now active.',
  'accountAuth.completed': 'Authentication completed.',
  'accountAuth.badgeExpiresIn': 'Badge expires in:',

  'changePassword.requirements': 'Password must be at least 10 characters and include at least one special character (#!@_$€%).',
  'changePassword.currentPassword': 'Current password',
  'changePassword.currentPasswordInvalid': 'Incorrect password',
  'changePassword.newPassword': 'New password',
  'changePassword.repeatNewPassword': 'Repeat new password',
  'changePassword.passwordsDontMatch': "Passwords don't match",
  'changePassword.forgot': 'Forgot your password?',
  'changePassword.success': 'Password updated',
  'changePassword.locked': 'You have reached the maximum attempts. Please try again later.',
  'changePassword.attemptsRemaining': 'Attempts remaining: {count}',
  'changePassword.accountLocked': 'Your account has been locked. You will be logged out automatically.',
  'changePassword.checkCompleted': 'Check completed',

  'blockedUsers.title': 'Blocked users',
  'blockedUsers.empty': "You don't have blocked users",

  'adminSelfies.title': 'Admin · Selfies',
  'adminSelfies.tabPending': 'Additional review',
  'adminSelfies.tabBlocked': 'Blocked',
  'adminSelfies.refresh': 'Refresh',
  'adminSelfies.emptyPending': 'No selfies need additional review.',
  'adminSelfies.emptyBlocked': 'No blocked users.',
  'adminSelfies.reasonPlaceholder': 'Reason (optional)',
  'adminSelfies.accept': 'Accept',
  'adminSelfies.reject': 'Reject',
  'adminSelfies.block': 'Block',
  'adminSelfies.unblock': 'Unblock',
  'blockedUsers.reasonPlaceholder': 'No reason',
};

export const translations: Record<Language, Record<TranslationKey, string>> = {
  es,
  en,
  fr: {
    'language.label': 'Langue',
    'language.spanish': 'Espagnol',
    'language.english': 'Anglais',
    'language.french': 'Français',
    'language.portuguese': 'Portugais',
    'language.german': 'Allemand',
    'language.italian': 'Italien',
    'gender.male': 'Homme',
    'gender.female': 'Femme',
    'gender.unspecified': 'Préfère ne pas répondre',
    'common.loading': 'Chargement...',
    'common.logout': 'Se déconnecter',
    'common.notifications': 'Notifications',
    'common.keys': 'Clés',
    'common.or': 'ou',
    'common.unlock': 'Débloquer',
    'common.view': 'Voir',
    'common.forgotPassword': 'Mot de [[passe oublié]] ?',
    'common.cancel': 'Annuler',
    'common.confirm': 'Confirmer',
    'common.delete': 'Supprimer',
    'common.report': 'Signaler',
    'common.block': 'Bloquer',
    'common.check': 'Vérifier',
    'common.apply': 'Appliquer',
    'common.accept': 'Accepter',
    'common.create': 'Créer',
    'keys.convert': 'Convertir',
    'keys.dailyGoals': 'Objectifs quotidiens',
    'keys.enterAmount': 'Saisir le montant',
    'keys.convertMultipleOfTenHint': 'Le montant doit être un multiple de 10.',
    'keys.convertInsufficientWhiteKeys': 'Vous n’avez pas assez de clés blanches pour cette conversion.',
    'keys.convertRateHint': 'Chaque 10 clés blanches se convertissent en 1 clé gradient.',
    'keys.redeem': 'Échanger',
    'keys.redeemSubtitle': 'Sélectionnez l’une des options',
    'keys.redeemOption10': '10 euros',
    'keys.redeemOption20': '20 euros',
    'keys.redeemOption50': '50 euros',
    'keys.redeemOption100': '100 euros',
    'keys.redeemOption500': '500 euros',
    'keys.redeemInsufficientKeys': 'Vous n’avez pas assez de clés.',
    'keys.redeemNote': 'Vous ne pourrez échanger qu’un coupon par jour. Choisissez bien. L’argent échangé sera envoyé sur le compte PayPal que vous indiquerez à l’étape suivante après avoir appuyé sur le bouton « Échanger ».',
    'keys.lockedMessage': 'Authentifiez votre compte pour accéder à vos objectifs quotidiens et à vos clés.',
    'keys.dailyGoal1': 'Pour chaque conteneur d’intimité que vous ouvrez, vous obtenez 2 clés blanches.',
    'keys.dailyGoal2': 'Parvenez à lier un fil avec un hôte dans son canal.',
    'keys.dailyGoal3': 'Déverrouillez les images que les hôtes partagent dans les canaux que vous rejoignez et obtenez 10 clés blanches pour chacune.',
    'keys.dailyGoal4': 'Publiez votre profil chaque jour.',
    'keys.dailyGoal5': 'Créez un événement dans votre canal.',
    'keys.dailyGoal6': 'Partagez une image dans votre canal.',
    'keys.dailyGoal7': 'Faites entrer 100 utilisateurs dans votre canal : 1 000 clés blanches.',
    'keys.dailyGoal8': 'Faites entrer 1 000 utilisateurs dans votre canal : 10 000 clés blanches.',
    'keys.dailyGoal9': 'Faites entrer 10 000 utilisateurs dans votre canal : 100 000 clés blanches.',
    'auth.sessionRequiredTitle': 'Session requise',
    'auth.signInToBlockUsers': 'Connectez-vous pour bloquer des utilisateurs.',
    'auth.signInToReportUsers': 'Connectez-vous pour signaler des utilisateurs.',
    'home.blockConfirmQuestion': 'Voulez-vous bloquer {user} ?',
    'home.blockReasonPlaceholder': 'Motif du blocage (optionnel)...',
    'home.reportSelectReason': 'Sélectionnez un motif pour signaler {user}.',
    'report.reason.sexualContent': 'Contenu sexuel ou nudité',
    'report.reason.harassment': 'Harcèlement ou intimidation',
    'report.reason.offensiveLanguage': 'Langage offensant',
    'report.reason.scam': 'Arnaque ou fraude',
    'report.reason.violence': 'Violence ou menaces',
    'report.reason.spam': 'Spam',
    'report.reason.impersonation': 'Usurpation d’identité',
    'report.reason.illegalContent': 'Contenu illégal',
    'report.reason.childSexualAbuse': 'Abus ou exploitation sexuelle de mineurs',
    'report.reason.weaponsOrDrugsIncitement': 'Incitation à l’usage d’armes ou de drogues',
    'report.reason.inappropriateConduct': 'Comportement inapproprié',
    'report.reason.other': 'Autre',
    'toast.joinedChannelOf': 'Vous avez rejoint le canal de',
    'toast.published': 'Publié',
    'toast.publicationNotActiveHome': 'La publication n’est plus active sur Accueil',
    'toast.joinedGroupMessage': 'Vous avez rejoint le groupe {group} de {user}',
    'toast.blocked': 'Bloqué',
    'toast.reportSent': 'Signalement envoyé',
    'carouselEditor.adjustImage': 'Ajuster l’image',
    'carouselEditor.imageCount': 'Image {current} sur {total}',
    'carouselEditor.processing': 'Traitement...',
    'carouselEditor.noPreview': 'Aucun aperçu',
    'carouselEditor.editImage': 'Modifier l’image',
    'carouselEditor.errorTitle': 'Erreur',
    'carouselEditor.cropErrorBody': 'Impossible de recadrer l’image. Veuillez réessayer.',
    'carouselEditor.ok': 'OK',
    'errors.unableToBlockUser': 'Impossible de bloquer l’utilisateur',
    'errors.unableToSendReport': 'Impossible d’envoyer le signalement',
    'login.email': 'E-mail',
    'login.password': 'Mot de passe',
    'login.emailPlaceholder': 'exemple@email.com',
    'login.passwordPlaceholder': 'Entrez votre mot de passe',
    'login.signIn': 'Se connecter',
    'login.fillAllFields': 'Veuillez remplir tous les champs',
    'login.invalidCredentials': 'Identifiants invalides',
    'login.noAccount': 'Vous n’avez pas de compte ? ',
    'login.signUp': 'S’inscrire',
    'login.googleNotRegistered': 'Ce compte n’est pas encore enregistré. Inscrivez-vous d’abord pour pouvoir vous connecter avec cette adresse e-mail.',
    'login.googlePkceMissing': 'Impossible de finaliser la connexion Google. Veuillez réessayer.',
    'login.resetTitle': 'Réinitialiser le mot de passe',
    'login.resetSend': 'Envoyer',
    'login.resetVerify': 'Vérifier',
    'login.resetChangePassword': 'Changer le mot de passe',
    'login.resetEmailNotRegistered': 'Cette adresse e-mail n’est pas encore utilisée sur Keinti. Inscrivez-vous.',
    'login.resetInvalidEmail': 'E-mail invalide',
    'login.resetCodeSent': 'Nous avons envoyé un code à votre e-mail.',
    'login.resetPasswordChanged': 'Mot de passe mis à jour. Vous pouvez maintenant vous connecter.',
    'login.resetCodeLabel': 'Code',
    'login.resetNewPasswordLabel': 'Créer un nouveau mot de passe',
    'login.resetRepeatNewPasswordLabel': 'Répéter le nouveau mot de passe',
    'register.email': 'E-mail',
    'register.username': '@nomutilisateur',
    'register.usernameAlreadyInUse': '« {username} » est déjà utilisé par un autre utilisateur ; veuillez en essayer un autre.',
    'register.birthDate': 'Date de naissance',
    'register.birthDatePlaceholder': 'JJ/MM/AAAA',
    'register.gender': 'Genre',
    'register.selectGender': 'Sélectionnez votre genre',
    'register.nationality': 'Nationalité',
    'register.password': 'Mot de passe',
    'register.confirmPassword': 'Confirmer le mot de passe',
    'register.confirmPasswordPlaceholder': 'Confirmez votre mot de passe',
    'register.next': 'Suivant',
    'register.signUp': 'S’inscrire',
    'register.registering': 'Inscription...',
    'register.successMessage': 'Bienvenue ! Vous êtes désormais un utilisateur de Keinti.',
    'register.privacyPrefix': 'En vous inscrivant, vous acceptez la ',
    'register.privacyPolicies': 'politique de confidentialité, la politique de cookies et publicité, ainsi que les conditions d’utilisation',
    'register.privacyPolicyLink': 'politique de confidentialité',
    'register.cookiesAdsPolicyLink': 'politique de cookies et publicité',
    'register.termsConditionsLink': 'conditions d’utilisation',
    'register.privacySeparator1': ', ',
    'register.privacySeparator2': ', et ',
    'register.privacySuffix': ' de notre application Keinti. Pour votre sécurité, lisez-les avant de vous inscrire.',
    'register.errorTitle': 'Erreur d’inscription',
    'register.unableToComplete': 'Impossible de terminer l’inscription.',
    'register.selectNationality': 'Sélectionnez votre nationalité',
    'register.searchCountry': 'Rechercher un pays...',
    'register.noCountriesFound': 'Aucun pays trouvé',
    'register.emailVerificationTitle': 'Vérification de l’e-mail',
    'register.emailVerificationHint': 'Nous avons envoyé un code à',
    'register.emailVerificationLinkHint': 'Si vous recevez un lien de confirmation au lieu d’un code, ouvrez-le et vous reviendrez automatiquement dans Keinti pour terminer l’inscription.',
    'register.iConfirmedEmail': 'J’ai déjà confirmé l’e-mail',
    'register.confirmedCheckHint': 'Si vous avez déjà ouvert le lien, revenez dans Keinti. Si rien ne se passe, attendez quelques secondes puis appuyez de nouveau, ou renvoyez l’e-mail.',
    'register.code': 'Code',
    'register.codePlaceholder': 'Entrez le code',
    'register.timeRemaining': 'Temps restant',
    'register.verifyAndSignUp': 'Vérifier et s’inscrire',
    'register.verifying': 'Vérification...',
    'register.resendCode': 'Renvoyer l’e-mail',
    'register.codeExpired': 'Le code a expiré. Demandez-en un nouveau.',
    'register.codeIncorrect': 'Code incorrect',
    'register.codeInvalidInline': 'Le code saisi n’est pas valide. Veuillez réessayer.',
    'register.remainingAttempts': 'Tentatives restantes',
    'register.remainingAttemptsInline': 'Tentatives restantes : {count}',
    'register.emailTemporarilyBlocked': 'Trop de tentatives. Recommencez l’inscription avec une autre adresse e-mail.',
    'register.emailLockedInlineError': 'Cette adresse e-mail a été temporairement bloquée. Nous l’examinons. Veuillez nous envoyer une réclamation afin que nous puissions la réactiver dans Keinti.',
    'register.emailLockedRetryInline': 'Cette adresse e-mail a été temporairement bloquée. Essayez-en une autre ou attendez la résolution de votre demande de rectification.',
    'register.rectificationTitle': 'Rectification',
    'register.rectificationPlaceholder': 'Décrivez votre rectification (max. 220 caractères)',
    'register.rectificationSend': 'Envoyer la réclamation',
    'register.rectificationSent': 'Réclamation envoyée. Nous examinerons votre demande dès que possible.',
    'register.tryLaterOrCheckEmailExists': 'Réessayez plus tard ou vérifiez que l’e-mail existe',
    'register.mustVerifyEmailFirst': 'Vous devez vérifier votre e-mail avant de vous inscrire',
    'register.cancelledNotice': 'Inscription annulée. Si vous souhaitez créer un nouveau compte Keinti, veuillez recommencer la procédure d’inscription.',
    'register.emailAlreadyInUse': 'Cette adresse e-mail est déjà utilisée par un autre utilisateur Keinti. Veuillez en essayer une autre.',
    'register.emailSendFailed': 'Nous n’avons pas pu envoyer le code à l’adresse e-mail « {email} ». Veuillez vérifier que cette adresse est valide.',
    'register.missingProfileData': 'Données d’inscription manquantes. Revenez en arrière et complétez le nom d’utilisateur, la date de naissance, le genre et la nationalité.',
    'register.signupExpiredNotice': 'Le délai de vérification a expiré. Vos données d’inscription ont été effacées. Vous pouvez réessayer.',
    'register.signupExpiredTitle': 'Temps écoulé',
    'register.emailLocked24h': 'Vous avez utilisé les {max} tentatives d’inscription avec cet e-mail. Vous ne pourrez plus l’utiliser pendant 48 heures.',
    'register.attemptsRemaining': 'Tentatives restantes : {remaining} sur {max}',
    'register.pendingConfirmation': 'Confirmez votre e-mail avant que le minuteur n’expire : il est encore actif.',
    'register.emailLockedTemp': 'Vous avez utilisé les {max} tentatives d’inscription avec cet e-mail. Vous ne pourrez plus l’utiliser pendant 48 heures.',
    'register.emailLockedPermanent': 'Votre adresse e-mail a été bloquée définitivement pour dépassement de la limite de tentatives d’inscription. Contactez keintisoporte@gmail.com pour demander son déblocage.',
    'validation.invalidDateFormat': 'Format de date invalide',
    'validation.invalidDate': 'Date invalide',
    'validation.mustBeAdult': 'Vous devez avoir au moins 18 ans',
    'validation.passwordMinLength': 'Le mot de passe doit contenir au moins 10 caractères',
    'validation.passwordMaxLength': 'Le mot de passe doit contenir au maximum 20 caractères',
    'validation.passwordNeedsLowercase': 'Doit inclure au moins 1 lettre minuscule',
    'validation.passwordNeedsUppercase': 'Doit inclure au moins 1 lettre majuscule',
    'validation.passwordNeedsLetter': 'Doit inclure au moins 1 lettre',
    'validation.passwordNeedsNumber': 'Doit inclure au moins 1 chiffre',
    'validation.passwordNeedsSpecial': 'Doit inclure au moins 1 caractère spécial',
    'validation.passwordsDontMatch': 'Les mots de passe ne correspondent pas',
    'front.editProfilePhoto': 'Modifier la photo de profil',
    'front.yourSocialNetworks': 'Vos réseaux sociaux',
    'front.userPlaceholder': 'Utilisateur',
    'front.selectCategory': 'Sélectionnez une catégorie',
    'front.noPublicationsAvailable': 'Aucune publication disponible',
    'front.forYou': 'Pour toi',
    'front.publicationsCount': 'Publications',
    'front.socialNetworksSingular': 'réseau social',
    'front.socialNetworksPlural': 'réseaux sociaux',
    'front.publish': 'Publier',
    'front.remove': 'Retirer (publié)',
    'front.yourProfile': 'Votre profil',
    'front.yourPresentation': 'Votre présentation',
    'front.intimacies': 'Intimités',
    'front.time': 'Temps',
    'front.loadingImages': 'Chargement des images...',
    'front.readMore': 'Lire plus...',
    'front.readLess': 'Lire moins',
    'front.votes': 'votes',
    'front.profileEmptyPrimaryMessage': 'Ajoutez « Votre présentation » et au moins une « Intimité » pour publier votre profil',
    'front.profileEmptySecondaryMessage': 'En publiant votre profil, d’autres utilisateurs pourront accéder à votre canal, où vous pourrez partager du contenu et interagir avec eux',
    'front.profileRingHintSelect': 'Sélectionnez une partie d’une des images du carrousel',
    'front.profileRingHintMaxRings': 'Maximum atteint : 5 anneaux',
    'front.profileRingDelete': 'Supprimer l’anneau',
    'front.profileRingNameLabel': 'Nom (max. 38 caractères)',
    'front.profileRingNameHelper': 'Produit, entreprise, utilisateur, canal, contenu, etc.',
    'front.profileRingDescriptionLabel': 'Description (max. 280 caractères)',
    'front.profileRingLinkLabel': 'Lien',
    'front.profileRingLinkHelper': 'Ajoutez un lien de réseau social pour que les utilisateurs puissent accéder au produit, à l’entreprise, à l’utilisateur, au canal, à la publication, au contenu, etc.',
    'front.profileRingLocationLabel': 'Lieu',
    'front.profileRingLocationHelper': 'Ajouter un lieu',
    'profile.deleteContentTitle': 'Supprimer le contenu',
    'profile.deleteContentBody': 'Voulez-vous vraiment supprimer ce contenu de votre profil ?',
    'front.reactions': 'Réactions',
    'front.deletePublicationTitle': 'Retirer la publication',
    'front.deletePublicationBody': 'Votre profil est actuellement public pour les autres utilisateurs sur la page « Accueil » ; si vous le retirez, vous seul pourrez le voir dans « Votre profil ».',
    'front.category': 'Catégorie',
    'front.presentationAddCarouselHint': 'Ajoutez jusqu’à 3 images au carrousel',
    'front.presentationTitleLabel': 'Titre',
    'front.presentationTitlePlaceholder': 'Écrivez un titre...',
    'front.presentationBodyLabel': 'Présentation',
    'front.presentationBodyPlaceholder': 'Écrivez votre présentation...',
    'front.presentationLockedTitle': 'Modification verrouillée',
    'front.presentationLockedBody': 'Votre profil est actuellement publié.',
    'front.selectSocialNetwork': 'Sélectionnez un réseau social',
    'front.link': 'Lien',
    'front.linkMustBeFrom': 'Le lien doit provenir de',
    'front.incorporate': 'Ajouter',
    'front.answerPlaceholder': 'Réponse...',
    'front.chooseCorrectOption': 'Choisissez la bonne option',
    'front.addMoreSurveyOptions': 'Ajouter plus d’options au sondage',
    'front.image': 'Image',
    'front.text': 'Texte',
    'front.draftPlaceholder': 'Écrire...',
    'front.draftIntimacyPlaceholder': 'Écrivez une intimité...',
    'front.intimaciesPublishBlocked': 'Vous ne pouvez pas encore publier d’intimités. Créez d’abord « Votre présentation ».',
    'front.homeSwipeTutorialHint': 'Lorsque la barre indicatrice en haut de l’écran est active, balayez vers le bas pour changer de publication.',
    'front.channelLabel': 'Canal :',
    'chat.tabYourChannel': 'Votre canal',
    'chat.tabChannel': 'Canal',
    'chat.tabJoined': 'Rejoints',
    'chat.tabYourGroups': 'Vos groupes',
    'chat.tabGroups': 'Groupes',
    'chat.postOnHomeToActivateChannel': 'Publiez sur Accueil pour activer votre canal',
    'chat.noChannelsYet': 'Vous n’avez encore aucun canal',
    'chat.createGroup': 'Créer un groupe',
    'chat.notJoinedAnyGroupYet': 'Vous n’avez encore rejoint aucun groupe',
    'chat.back': 'Retour',
    'chat.interactPlaceholder': 'Interagir...',
    'chat.timeExpired': 'Temps écoulé',
    'chat.remainingPrefix': 'Reste',
    'chat.hour': 'heure',
    'chat.hours': 'heures',
    'chat.minute': 'minute',
    'chat.minutes': 'minutes',
    'chat.reply': 'Répondre',
    'chat.generalTab': 'Général',
    'chat.yourThreadsTab': 'Fils',
    'chat.recommendationsTab': 'Recommandations',
    'chat.eventsTab': 'Événements',
    'chat.readingsTab': 'Lectures',
    'chat.imagesTab': 'Images',
    'chat.limitWarningMessage': 'Vous avez déjà interagi dans le canal de {user}. Veuillez attendre la réponse.',
    'chat.hostLimitedInteractions': '{user} a limité vos interactions',
    'chat.waitHostReplyThread': 'Attendez la réponse de l’hôte pour continuer le fil',
    'chat.waitCreatorReplyAgain': 'Attendez la réponse de l’hôte avant de répondre à nouveau',
    'chat.signInToSendMessages': 'Connectez-vous pour envoyer des messages.',
    'chat.unableToSendMessage': 'Impossible d\'envoyer le message.',
    'chat.connectionErrorSendingMessage': 'Erreur de connexion lors de l\'envoi du message.',
    'chat.addCaptionPlaceholder': 'Ajoutez une légende...',
    'chat.imageCaptionPlaceholder': 'Légende de l\'image...',
    'reading.screenTitle': 'Créer une lecture',
    'reading.titlePlaceholder': 'Titre',
    'reading.subtitlePlaceholder': 'Sous-titre',
    'reading.leadPlaceholder': 'Chapô...',
    'reading.dateLabel': 'Date',
    'reading.datePlaceholder': 'JJ/MM/AAAA',
    'reading.chooseCategoryLabel': 'Choisissez une catégorie',
    'reading.categoryLabel': 'Catégorie',
    'reading.searchCategoryPlaceholder': 'Rechercher une catégorie',
    'reading.hypeCostLabel': 'Coût du Hype',
    'reading.hypeCostInfo': 'Choisissez le nombre de clés blanches que vous voulez que chaque utilisateur du canal vous donne pour cette lecture afin de booster votre profil. Plus vous recevez de clés, plus votre profil et votre canal se diffuseront sur l’écran Hype.',
    'reading.hypeCostAmountLabel': 'Quantité',
    'reading.bodyPlaceholder': 'Corps...',
    'reading.intertitlePlaceholder': 'Intertitre',
    'reading.imageLimitReachedTitle': 'Limite atteinte',
    'reading.imageLimitReachedBody': 'Vous pouvez ajouter jusqu’à 3 images dans la lecture.',
    'reading.galleryPermissionTitle': 'Autorisation de la galerie',
    'reading.galleryPermissionMessage': 'L’application a besoin d’accéder à votre galerie pour sélectionner des photos.',
    'reading.galleryPermissionAskLater': 'Demander plus tard',
    'reading.galleryPermissionRequiredTitle': 'Autorisation requise',
    'reading.galleryPermissionRequiredBody': 'Vous devez activer l’accès aux Photos dans Réglages pour pouvoir sélectionner des images.',
    'reading.openSettings': 'Ouvrir les réglages',
    'reading.imagePickErrorTitle': 'Erreur',
    'reading.imagePickErrorBody': 'Impossible de sélectionner l’image. Veuillez réessayer.',
    'reading.citeSelectionToastBody': 'Sélectionnez une partie du texte du corps.',
    'reading.intertitleToastBody': 'Vous devez écrire du texte dans le corps après un intertitre avant d’en ajouter un autre.',
    'reading.bodyMinSuffix': 'min',
    'reading.addImage': 'Ajouter une image',
    'reading.maxImagesHint': 'Maximum 3',
    'reading.selectedCitationLabel': 'Citation sélectionnée',
    'reading.citedUsersTitle': 'Utilisateurs cités',
    'reading.citedUsersInputLabel': '@utilisateurs',
    'reading.voiceNoteIntroTitle': 'Note d’introduction',
    'reading.voiceNoteIntroDescription': 'Elle sera jouée avant le titre.',
    'reading.voiceNoteInlineTitle': 'Note vocale dans le corps',
    'reading.voiceNoteInlineDescription': 'Elle sera jouée entre les blocs du corps.',
    'reading.voiceNoteOutroTitle': 'Note finale',
    'reading.voiceNoteOutroDescription': 'Elle sera jouée sous le corps du texte.',
    'reading.voiceNoteRecordAction': 'Enregistrer',
    'reading.voiceNoteRerecordAction': 'Réenregistrer',
    'reading.voiceNoteRecordingStatus': 'Enregistrement',
    'chat.selectGroup': 'Sélectionnez un groupe',
    'chat.toPrefix': 'À :',
    'chat.request': 'Demande',
    'chat.requestPending': 'En attente',
    'chat.requestAccepted': 'Acceptée',
    'chat.requestBlocked': 'Bloquée',
    'chat.members': 'Membres',
    'chat.messageHidden': 'Message masqué',
    'chat.messageVisible': 'Message visible',
    'event.enableTasks': 'Activer les tâches',
    'event.assignTask': 'Attribuer une tâche',
    'event.activateRequests': 'Activer les demandes',
    'event.rewardTasks': 'Récompenser les tâches',
    'event.describeTask': 'Décris la tâche',
    'event.userPlaceholder': '@utilisateur',
    'event.amount': 'Montant',
    'event.type': 'Type d’événement',
    'event.requiredSuffix': '(obligatoire)',
    'event.name': 'Nom de l’événement',
    'event.description': 'Description de l’événement',
    'event.dateAndTime': 'Date et heure',
    'event.dayMonthYear': 'Jour/Mois/Année',
    'event.time': 'Heure',
    'event.durationTitle': 'Durée de l’événement',
    'event.duration': 'Durée',
    'event.endsIn': 'Se termine dans',
    'event.link': 'Lien',
    'event.selectSocialNetwork': 'Sélectionner un réseau social',
    'event.addLink': 'Ajouter un lien',
    'event.location': 'Emplacement',
    'event.selectLocation': 'Sélectionner un emplacement',
    'event.searchType': 'Rechercher un type d’événement',
    'event.typeOption.social': 'Social',
    'event.typeOption.cultural': 'Culturel',
    'event.typeOption.artistic': 'Artistique',
    'event.typeOption.business': 'Professionnel',
    'event.typeOption.economic': 'Économique',
    'event.typeOption.academic': 'Académique',
    'event.typeOption.sports': 'Sportif',
    'event.typeOption.entertainment': 'Divertissement',
    'event.typeOption.political': 'Politique',
    'event.typeOption.religious': 'Religieux',
    'event.typeOption.marketing': 'Marketing',
    'event.typeOption.charity': 'Solidaire',
    'event.typeOption.audiovisual': 'Audiovisuel',
    'event.typeOption.scientific': 'Scientifique',
    'event.typeOption.technological': 'Technologique',
    'event.typeOption.children': 'Enfants',
    'event.typeOption.musical': 'Musical',
    'event.typeOption.automotive': 'Automobile',
    'event.typeOption.fitness': 'Fitness',
    'event.typeOption.gastronomic': 'Gastronomique',
    'event.typeOption.work': 'Professionnel',
    'chat.lockedYourGroupsMessage': 'Authentifiez votre compte pour créer vos groupes',
    'chat.lockedJoinedGroupsMessage': 'Pour accéder aux chats des groupes que vous avez rejoints, authentifiez votre compte',
    'chat.hostOnlyEventHypeMessage': 'Les utilisateurs qui rejoignent votre canal sont les seuls à pouvoir booster votre événement',
    'chat.hostOnlyReadingHypeMessage': 'Les utilisateurs qui rejoignent votre canal sont les seuls à pouvoir booster votre lecture',
    'groups.edit': 'Modifier',
    'groups.delete': 'Supprimer',
    'groups.deleteConfirmTitle': 'Confirmer la suppression',
    'groups.deleteConfirmBody': 'Voulez-vous vraiment supprimer ce groupe ? Tous les utilisateurs de ce groupe seront retirés.',
    'groups.limit': 'Limiter',
    'groups.limited': 'Limité',
    'groups.expel': 'Expulser',
    'groups.expelAndBlock': 'Expulser et bloquer',
    'groups.leaveGroup': 'Quitter le groupe',
    'groups.leaveAndBlock': 'Quitter et bloquer',
    'groups.limitUsers': 'Limiter des utilisateurs',
    'groups.expelUsers': 'Expulser des utilisateurs',
    'groups.selectAll': 'Tout sélectionner',
    'groups.deselect': 'Désélectionner',
    'groups.noMembersToShow': 'Aucun membre à afficher',
    'notifications.groupJoinRequestMessage': '{user} vous a envoyé une demande pour rejoindre son groupe {group}. Voulez-vous le rejoindre ?',
    'notifications.ignore': 'Ignorer',
    'notifications.accept': 'Accepter',
    'notifications.read': 'Lu',
    'notifications.unread': 'Non lu',
    'notifications.empty': 'Vous n’avez pas de notifications',
    'config.title': 'Paramètres',
    'config.accountCenter': 'Centre de compte',
    'config.blockedUsers': 'Utilisateurs bloqués',
    'config.devicePermissions': 'Autorisations de l’appareil',
    'config.helpCenter': 'Centre d’aide',
    'config.information': 'À propos de Keinti',
    'config.yourAccountIn': 'Votre compte sur',
    'aboutKeinti.title': 'À propos de Keinti',
    'aboutKeinti.moreAboutKeinti': 'En savoir plus sur Keinti',
    'aboutKeinti.body': `À tous les utilisateurs de Keinti :

Keinti est un réseau social axé sur la découverte de profils et l’interaction directe entre les personnes. Notre objectif est d’offrir un environnement dynamique qui aide les utilisateurs à accroître leur visibilité, au sein de Keinti et en complément de leur présence en ligne.

Chaque utilisateur peut créer un profil public composé d’une Présentation et d’espaces où il peut partager des réflexions personnelles et des aspects de son identité. Cette structure vise à aider les autres membres à mieux comprendre leurs intérêts et leur style. Depuis l’écran Accueil, la plateforme encourage une exploration continue des profils créés par des personnes du monde entier.

Lorsqu’un profil suscite de l’intérêt, les utilisateurs peuvent rejoindre le canal général de l’hôte ou de l’auteur et interagir par le biais de fils de conversation. Les auteurs peuvent également inviter des utilisateurs à rejoindre leurs groupes afin de renforcer et de développer leurs communautés.

Concernant le développement, Keinti a été entièrement construit avec le soutien de l’intelligence artificielle (ChatGPT, Claude Sonnet et Gemini) via Visual Studio Code. Keinti a entamé un processus de recherche de collaboration afin d’accueillir des partenaires et des contributeurs spécialisés (développement, design graphique et autres rôles), avec comme priorité le renforcement de la sécurité et l’amélioration de l’expérience visuelle du produit.

Pour les propositions de collaboration, veuillez contacter : keintisoporte@gmail.com

Cordialement,
L’équipe Keinti`,
    'verifyKeinti.objectivesTitle': 'Objectifs pour devenir Keinti Verified :',
    'verifyKeinti.objective1': '• Authentifiez d’abord votre compte.',
    'verifyKeinti.complete': 'Compléter',
    'verifyKeinti.completed': 'Terminé',
    'verifyKeinti.objective2': '• Obtenez 100 000 ouvertures sur vos publications d’intimités',
    'verifyKeinti.objective3': '• En un seul mois, obtenez au moins 1 000 vues d’autres utilisateurs sur les images que vous partagez dans les chats de « Votre canal »',
    'verifyKeinti.objective4': '• Faites rejoindre 100 000 utilisateurs au chat de « Votre canal »',
    'verifyKeinti.objective5': '• Créez au moins un groupe dans « Vos groupes » et faites en sorte qu’au moins 200 membres actifs rejoignent ces groupes.',
    'verifyKeinti.objective6': '• Publiez votre profil au moins 40 fois sur « Accueil ».',
    'verifyKeinti.benefitsTitle': 'Avantages de devenir Keinti Verified :',
    'verifyKeinti.benefit1': '• Obtenez le badge doré.',
    'verifyKeinti.benefit2': '• Générez des revenus pour chaque utilisateur qui accède aux chats de « Votre canal ».',
    'verifyKeinti.benefit3': '• Générez des revenus pour chaque tranche de 1 000 vues des images que vous partagez dans les chats de « Votre canal »',
    'verifyKeinti.benefit4': '• Obtenez d’autres privilèges futurs proposés par Keinti dans les prochaines mises à jour.',
    'verifyKeinti.tabObjectives': 'Objectifs',
    'verifyKeinti.tabBenefits': 'Avantages',
    'verifyKeinti.importantNotice': 'Avis important',
    'verifyKeinti.importantNoticeBody': 'La monétisation arrivera bientôt sur Keinti. Vérifiez votre compte pour obtenir tous les avantages lorsqu’ils seront disponibles.',
    'verifyKeinti.verifyAction': 'Vérifier',
    'verifyKeinti.accountVerifiedLabel': 'Compte vérifié',
    'devicePermissions.description': 'Ici, vous pouvez voir les autorisations que vous avez accordées à l’application.',
    'devicePermissions.galleryTitle': 'Galerie (photos)',
    'devicePermissions.galleryDescription': 'Utilisée lorsque vous ajoutez une image à votre avatar, à votre présentation ou à vos intimités.',
    'devicePermissions.statusGranted': 'Accordée',
    'devicePermissions.statusDenied': 'Non accordée',
    'devicePermissions.statusUnknown': 'Indisponible',
    'devicePermissions.iosHint': 'Sur iOS, cette autorisation est gérée par le système et sera demandée lorsque vous sélectionnerez une image.',
    'devicePermissions.revokeConfirmTitle': 'Retirer l’autorisation',
    'devicePermissions.revokeConfirmBody': 'Voulez-vous vraiment retirer à Keinti l’autorisation d’accéder à votre galerie photo ?',
    'devicePermissions.reEnableConfirmTitle': 'Autorisation de galerie',
    'devicePermissions.reEnableConfirmBody': 'Voulez-vous autoriser Keinti à accéder à votre galerie photo ?',
    'devicePermissions.osRequestTitle': 'Autorisation de galerie',
    'devicePermissions.osRequestMessage': 'L’application a besoin d’accéder à votre galerie pour sélectionner des photos.',
    'devicePermissions.osRequestAskLater': 'Plus tard',
    'devicePermissions.osRequestDeny': 'Annuler',
    'devicePermissions.osRequestAllow': 'Autoriser',
    'accountCenter.title': 'Centre de compte',
    'accountCenter.personalData': 'Données personnelles',
    'accountCenter.securityControl': 'Contrôle de sécurité',
    'accountCenter.changePassword': 'Changer le mot de passe',
    'accountCenter.adminSelfies': 'Admin : vérifier les selfies',
    'accountCenter.closeAccount': 'Se déconnecter',
    'accountCenter.deleteAccount': 'Supprimer le compte',
    'accountCenter.deleteAccountConfirmTitle': 'Supprimer le compte',
    'accountCenter.deleteAccountConfirmBody': 'Voulez-vous vraiment supprimer votre compte ? Cette action est irréversible et supprimera vos données du système.',
    'accountCenter.accountDeletionPolicyLink': 'Voir la politique de suppression de compte',
    'personalData.title': 'Données personnelles',
    'personalData.description': 'Keinti utilise ces informations pour vérifier votre identité et protéger la communauté.',
    'personalData.contactInfo': 'Coordonnées',
    'personalData.birthDate': 'Date de naissance',
    'personalData.gender': 'Genre',
    'personalData.nationality': 'Nationalité',
    'securityControl.passwordAndAuth': 'Mot de passe et authentification',
    'securityControl.passwordAndAuthDescription': 'Dans cette section, vous pouvez authentifier votre profil et changer votre mot de passe.',
    'securityControl.accountAuth': 'Authentification du compte',
    'securityControl.privacyPolicy': 'Politique de confidentialité',
    'securityControl.cookiesAdPolicy': 'Politique de cookies et publicité',
    'securityControl.termsOfUse': 'Conditions d’utilisation',
    'securityControl.childSafetyStandards': 'Normes de sécurité de l’enfance',
    'securityControl.accountDeletionPolicy': 'Politique de suppression de compte',
    'securityControl.verifyYourKeinti': 'Vérifiez votre Keinti',
    'accountAuth.description': 'Pour authentifier votre compte, vous devez compléter 2 étapes : un selfie de vérification et un code Google Authenticator. Le selfie est validé automatiquement et ne passe en examen complémentaire que si le système ne peut pas décider avec assez de certitude.',
    'accountAuth.step1Title': 'Étape 1 : selfie de vérification',
    'accountAuth.step1Body': 'Prenez un selfie pour que le système valide automatiquement que l’image est adaptée à la sécurité et à l’anti-usurpation avant d’activer l’étape 2.',
    'accountAuth.step1Hint': 'Assurez-vous d’avoir un bon éclairage et de bien cadrer votre visage.',
    'accountAuth.step1Action': 'Prendre un selfie',
    'accountAuth.step1Pending': 'Examen complémentaire',
    'accountAuth.pendingReviewNotice': 'Votre selfie est toujours en cours d’examen. Cette vérification peut prendre jusqu’à 24 heures. Si, après ce délai, en appuyant sur "Actualiser le statut", le statut reste "Examen complémentaire", contactez keintisoporte@gmail.com.',
    'accountAuth.step1Blocked': 'Bloqué',
    'accountAuth.refreshStatus': 'Actualiser le statut',
    'accountAuth.selfieAccepted': 'Selfie accepté',
    'accountAuth.statusNotSubmitted': 'Statut : pas encore envoyé.',
    'accountAuth.statusPending': 'Statut : examen complémentaire requis.',
    'accountAuth.statusAccepted': 'Statut : accepté. Étape 2 disponible.',
    'accountAuth.statusFailed': 'Statut : échec. Réessayez avec un meilleur éclairage et un meilleur cadrage.',
    'accountAuth.statusBlocked': 'Statut : bloqué. Vous ne pouvez plus envoyer de selfies tant qu’un administrateur ne vous a pas débloqué.',
    'accountAuth.errorTitle': 'Erreur',
    'accountAuth.selfieUploadError': 'Impossible d’envoyer le selfie. Veuillez réessayer.',
    'accountAuth.step2Title': 'Étape 2 : Google Authenticator',
    'accountAuth.step2Body': 'Configurez Google Authenticator et vérifiez le code pour terminer l’authentification.',
    'accountAuth.step2Locked': 'Cette étape sera activée une fois le selfie accepté.',
    'accountAuth.step2Ready': 'Prêt à configurer Google Authenticator.',
    'accountAuth.step2AlreadyEnabled': 'Google Authenticator est déjà activé.',
    'accountAuth.generateTotp': 'Générer la configuration',
    'accountAuth.secretLabel': 'Clé secrète',
    'accountAuth.secretHint': 'Ajoutez un compte dans Google Authenticator en utilisant cette clé (manuel). Ensuite, entrez le code à 6 chiffres.',
    'accountAuth.codeLabel': 'Code',
    'accountAuth.codePlaceholder': '123456',
    'accountAuth.verifyCode': 'Vérifier le code',
    'accountAuth.totpSetupError': 'Impossible de générer la configuration.',
    'accountAuth.verifyError': 'Impossible de vérifier le code.',
    'accountAuth.successTitle': 'Terminé',
    'accountAuth.successBody': 'Votre compte a été authentifié et le badge est maintenant actif.',
    'accountAuth.completed': 'Authentification terminée.',
    'accountAuth.badgeExpiresIn': 'Le badge expire dans :',
    'changePassword.requirements': 'Le mot de passe doit contenir au moins 10 caractères et inclure au moins un caractère spécial (#!@_$€%).',
    'changePassword.currentPassword': 'Mot de passe actuel',
    'changePassword.currentPasswordInvalid': 'Mot de passe incorrect',
    'changePassword.newPassword': 'Nouveau mot de passe',
    'changePassword.repeatNewPassword': 'Répéter le nouveau mot de passe',
    'changePassword.passwordsDontMatch': 'Les mots de passe ne correspondent pas',
    'changePassword.forgot': 'Mot de passe oublié ?',
    'changePassword.success': 'Mot de passe mis à jour',
    'changePassword.locked': 'Vous avez atteint le nombre maximal de tentatives. Veuillez réessayer plus tard.',
    'changePassword.attemptsRemaining': 'Tentatives restantes : {count}',
    'changePassword.accountLocked': 'Votre compte a été verrouillé. Vous serez déconnecté automatiquement.',
    'changePassword.checkCompleted': 'Vérification terminée',
    'blockedUsers.title': 'Utilisateurs bloqués',
    'blockedUsers.empty': 'Vous n’avez pas d’utilisateurs bloqués',
    'adminSelfies.title': 'Admin · Selfies',
    'adminSelfies.tabPending': 'Examen complémentaire',
    'adminSelfies.tabBlocked': 'Bloqués',
    'adminSelfies.refresh': 'Actualiser',
    'adminSelfies.emptyPending': 'Aucun selfie en examen complémentaire.',
    'adminSelfies.emptyBlocked': 'Aucun utilisateur bloqué.',
    'adminSelfies.reasonPlaceholder': 'Motif (optionnel)',
    'adminSelfies.accept': 'Accepter',
    'adminSelfies.reject': 'Rejeter',
    'adminSelfies.block': 'Bloquer',
    'adminSelfies.unblock': 'Débloquer',
    'blockedUsers.reasonPlaceholder': 'Aucun motif',
  },
  pt: {
    'language.label': 'Idioma',
    'language.spanish': 'Espanhol',
    'language.english': 'Inglês',
    'language.french': 'Francês',
    'language.portuguese': 'Português',
    'language.german': 'Alemão',
    'language.italian': 'Italiano',
    'gender.male': 'Homem',
    'gender.female': 'Mulher',
    'gender.unspecified': 'Prefiro não informar',
    'common.loading': 'Carregando...',
    'common.logout': 'Sair',
    'common.notifications': 'Notificações',
    'common.keys': 'Chaves',
    'common.or': 'ou',
    'common.unlock': 'Desbloquear',
    'common.view': 'Visualizar',
    'common.forgotPassword': 'Esqueceu sua [[senha]]?',
    'common.cancel': 'Cancelar',
    'common.confirm': 'Confirmar',
    'common.delete': 'Excluir',
    'common.report': 'Denunciar',
    'common.block': 'Bloquear',
    'common.check': 'Verificar',
    'common.apply': 'Aplicar',
    'common.accept': 'Aceitar',
    'common.create': 'Criar',
    'keys.convert': 'Converter',
    'keys.dailyGoals': 'Objetivos diários',
    'keys.enterAmount': 'Inserir quantidade',
    'keys.convertMultipleOfTenHint': 'A quantidade deve ser um múltiplo de 10.',
    'keys.convertInsufficientWhiteKeys': 'Você não tem chaves brancas suficientes para esta conversão.',
    'keys.convertRateHint': 'Cada 10 chaves brancas se convertem em 1 chave gradient.',
    'keys.redeem': 'Resgatar',
    'keys.redeemSubtitle': 'Selecione uma das opções',
    'keys.redeemOption10': '10 euros',
    'keys.redeemOption20': '20 euros',
    'keys.redeemOption50': '50 euros',
    'keys.redeemOption100': '100 euros',
    'keys.redeemOption500': '500 euros',
    'keys.redeemInsufficientKeys': 'Você não tem chaves suficientes.',
    'keys.redeemNote': 'Você só poderá resgatar um cupom por dia. Escolha bem. O dinheiro resgatado será enviado para a conta do PayPal que você informar na etapa seguinte, depois de tocar no botão "Resgatar".',
    'keys.lockedMessage': 'Autentique sua conta para acessar seus objetivos diários e chaves.',
    'keys.dailyGoal1': 'Para cada contêiner de intimidade que você abrir, ganhará 2 chaves brancas.',
    'keys.dailyGoal2': 'Consiga vincular um tópico com um anfitrião no seu canal.',
    'keys.dailyGoal3': 'Desbloqueie as imagens que os anfitriões compartilharem nos canais aos quais você entrar e ganhe 10 chaves brancas por cada uma.',
    'keys.dailyGoal4': 'Publique seu perfil todos os dias.',
    'keys.dailyGoal5': 'Crie um evento no seu canal.',
    'keys.dailyGoal6': 'Compartilhe uma imagem no seu canal.',
    'keys.dailyGoal7': 'Faça 100 usuários entrarem no seu canal: 1.000 chaves brancas.',
    'keys.dailyGoal8': 'Faça 1.000 usuários entrarem no seu canal: 10.000 chaves brancas.',
    'keys.dailyGoal9': 'Faça 10.000 usuários entrarem no seu canal: 100.000 chaves brancas.',
    'auth.sessionRequiredTitle': 'Sessão obrigatória',
    'auth.signInToBlockUsers': 'Entre para bloquear usuários.',
    'auth.signInToReportUsers': 'Entre para denunciar usuários.',
    'home.blockConfirmQuestion': 'Deseja bloquear {user}?',
    'home.blockReasonPlaceholder': 'Motivo do bloqueio (opcional)...',
    'home.reportSelectReason': 'Selecione um motivo para denunciar {user}.',
    'report.reason.sexualContent': 'Conteúdo sexual ou nudez',
    'report.reason.harassment': 'Assédio ou bullying',
    'report.reason.offensiveLanguage': 'Linguagem ofensiva',
    'report.reason.scam': 'Golpe ou fraude',
    'report.reason.violence': 'Violência ou ameaças',
    'report.reason.spam': 'Spam',
    'report.reason.impersonation': 'Falsidade ideológica',
    'report.reason.illegalContent': 'Conteúdo ilegal',
    'report.reason.childSexualAbuse': 'Abuso ou exploração sexual de menores',
    'report.reason.weaponsOrDrugsIncitement': 'Incitação ao uso de armas ou drogas',
    'report.reason.inappropriateConduct': 'Conduta inadequada',
    'report.reason.other': 'Outro',
    'errors.unableToBlockUser': 'Não foi possível bloquear o usuário',
    'errors.unableToSendReport': 'Não foi possível enviar a denúncia',
    'login.email': 'E-mail',
    'login.password': 'Senha',
    'login.emailPlaceholder': 'exemplo@email.com',
    'login.passwordPlaceholder': 'Digite sua senha',
    'login.signIn': 'Entrar',
    'login.fillAllFields': 'Preencha todos os campos',
    'login.invalidCredentials': 'Credenciais inválidas',
    'login.googleNotRegistered': 'Esta conta ainda não está registrada. Cadastre-se primeiro para poder entrar com este e-mail.',
    'login.googlePkceMissing': 'Não foi possível concluir o acesso com Google. Tente novamente.',
    'login.noAccount': 'Não tem conta? ',
    'login.signUp': 'Cadastre-se',
    'login.resetTitle': 'Recuperar senha',
    'login.resetSend': 'Enviar',
    'login.resetVerify': 'Verificar',
    'login.resetChangePassword': 'Alterar senha',
    'login.resetEmailNotRegistered': 'Este e-mail ainda não está em uso no Keinti. Cadastre-se.',
    'login.resetInvalidEmail': 'E-mail inválido',
    'login.resetCodeSent': 'Enviamos um código para seu e-mail.',
    'login.resetPasswordChanged': 'Senha atualizada. Agora você pode entrar.',
    'login.resetCodeLabel': 'Código',
    'login.resetNewPasswordLabel': 'Criar nova senha',
    'login.resetRepeatNewPasswordLabel': 'Repetir nova senha',
    'register.email': 'E-mail',
    'register.username': '@usuário',
    'register.usernameAlreadyInUse': '"{username}" já está sendo usado por outro usuário; tente outro.',
    'register.birthDate': 'Data de nascimento',
    'register.birthDatePlaceholder': 'DD/MM/AAAA',
    'register.gender': 'Gênero',
    'register.selectGender': 'Selecione seu gênero',
    'register.nationality': 'Nacionalidade',
    'register.password': 'Senha',
    'register.confirmPassword': 'Confirmar senha',
    'register.confirmPasswordPlaceholder': 'Confirme sua senha',
    'register.next': 'Próximo',
    'register.signUp': 'Cadastrar',
    'register.registering': 'Cadastrando...',
    'register.successMessage': 'Bem-vindo! Agora você é usuário do Keinti.',
    'register.privacyPrefix': 'Ao se cadastrar, você aceita a ',
    'register.privacyPolicies': 'política de privacidade, política de cookies e publicidade, e os termos e condições de uso',
    'register.privacyPolicyLink': 'política de privacidade',
    'register.cookiesAdsPolicyLink': 'política de cookies e publicidade',
    'register.termsConditionsLink': 'termos e condições de uso',
    'register.privacySeparator1': ', a ',
    'register.privacySeparator2': ', e os ',
    'register.privacySuffix': ' do nosso app Keinti. Para sua segurança, leia-os antes de se cadastrar.',
    'register.errorTitle': 'Erro ao cadastrar',
    'register.unableToComplete': 'Não foi possível concluir o cadastro.',
    'register.selectNationality': 'Selecione sua nacionalidade',
    'register.searchCountry': 'Buscar país...',
    'register.noCountriesFound': 'Nenhum país encontrado',
    'register.emailVerificationTitle': 'Verificação de e-mail',
    'register.emailVerificationHint': 'Enviamos um código para',
    'register.emailVerificationLinkHint': 'Se receber um link de confirmação em vez de um código, abra-o e retornará automaticamente ao Keinti para concluir o cadastro.',
    'register.iConfirmedEmail': 'Já confirmei o e-mail',
    'register.confirmedCheckHint': 'Se já abriu o link, volte ao Keinti. Se nada acontecer, aguarde alguns segundos e toque novamente, ou reenvie o e-mail.',
    'register.code': 'Código',
    'register.codePlaceholder': 'Digite o código',
    'register.timeRemaining': 'Tempo restante',
    'register.verifyAndSignUp': 'Verificar e cadastrar',
    'register.verifying': 'Verificando...',
    'register.resendCode': 'Reenviar e-mail',
    'register.codeExpired': 'O código expirou. Solicite um novo.',
    'register.codeIncorrect': 'Código incorreto',
    'register.codeInvalidInline': 'O código digitado não é válido. Tente novamente.',
    'register.remainingAttempts': 'Tentativas restantes',
    'register.remainingAttemptsInline': 'Tentativas restantes: {count}',
    'register.emailTemporarilyBlocked': 'Muitas tentativas. Reinicie o cadastro com outro e-mail.',
    'register.emailLockedInlineError': 'Este e-mail foi bloqueado temporariamente. Estamos analisando. Envie-nos uma reclamação para que possamos reativar seu e-mail no Keinti.',
    'register.emailLockedRetryInline': 'Este e-mail foi bloqueado temporariamente. Tente outro ou aguarde a resolução de sua solicitação de retificação.',
    'register.rectificationTitle': 'Retificação',
    'register.rectificationPlaceholder': 'Descreva sua retificação (máx. 220 caracteres)',
    'register.rectificationSend': 'Enviar reclamação',
    'register.rectificationSent': 'Reclamação enviada. Analisaremos sua solicitação em breve.',
    'register.tryLaterOrCheckEmailExists': 'Tente mais tarde ou verifique se o e-mail existe',
    'register.mustVerifyEmailFirst': 'Você deve verificar seu e-mail antes de se cadastrar',
    'register.cancelledNotice': 'Cadastro cancelado. Se quiser criar uma nova conta Keinti, comece o processo novamente.',
    'register.emailAlreadyInUse': 'Este e-mail já está em uso por outro usuário Keinti. Tente outro.',
    'register.emailSendFailed': 'Não conseguimos enviar o código para o e-mail "{email}". Verifique se está correto.',
    'register.missingProfileData': 'Faltam dados do cadastro. Volte e complete usuário, data, gênero e nacionalidade.',
    'register.signupExpiredNotice': 'O prazo de verificação expirou. Seus dados foram apagados. Tente novamente.',
    'register.signupExpiredTitle': 'Tempo expirado',
    'register.emailLocked24h': 'Você usou todas as {max} tentativas com este e-mail. Não poderá usá-lo novamente por 48 horas.',
    'register.attemptsRemaining': 'Tentativas restantes: {remaining} de {max}',
    'register.pendingConfirmation': 'Confirme seu e-mail antes do tempo acabar – ainda está ativo.',
    'register.emailLockedTemp': 'Você usou todas as {max} tentativas com este e-mail. Não poderá usá-lo novamente por 48 horas.',
    'register.emailLockedPermanent': 'Seu e-mail foi bloqueado permanentemente por exceder o limite de tentativas. Contate keintisoporte@gmail.com para solicitar desbloqueio.',
    'validation.invalidDateFormat': 'Formato de data inválido',
    'validation.invalidDate': 'Data inválida',
    'validation.mustBeAdult': 'Você deve ter pelo menos 18 anos',
    'validation.passwordMinLength': 'A senha deve ter no mínimo 10 caracteres',
    'validation.passwordMaxLength': 'A senha não pode ter mais de 20 caracteres',
    'validation.passwordNeedsLowercase': 'Deve incluir pelo menos 1 letra minúscula',
    'validation.passwordNeedsUppercase': 'Deve incluir pelo menos 1 letra maiúscula',
    'validation.passwordNeedsLetter': 'Deve incluir pelo menos 1 letra',
    'validation.passwordNeedsNumber': 'Deve incluir pelo menos 1 número',
    'validation.passwordNeedsSpecial': 'Deve incluir pelo menos 1 caractere especial',
    'validation.passwordsDontMatch': 'As senhas não correspondem',
    'front.editProfilePhoto': 'Editar foto do perfil',
    'front.yourSocialNetworks': 'Suas redes sociais',
    'front.userPlaceholder': 'Usuário',
    'front.selectCategory': 'Selecione uma categoria',
    'front.noPublicationsAvailable': 'Nenhuma publicação disponível',
    'front.forYou': 'Para você',
    'front.publicationsCount': 'Publicações',
    'front.socialNetworksSingular': 'rede social',
    'front.socialNetworksPlural': 'redes sociais',
    'front.publish': 'Publicar',
    'front.remove': 'Remover (Publicado)',
    'front.yourProfile': 'Seu perfil',
    'front.yourPresentation': 'Sua apresentação',
    'front.intimacies': 'Intimidades',
    'front.time': 'Tempo',
    'front.loadingImages': 'Carregando imagens...',
    'front.readMore': 'Ler mais...',
    'front.readLess': 'Ler menos',
    'front.votes': 'votos',
    'front.profileEmptyPrimaryMessage': 'Adicione "Sua apresentação" e pelo menos uma "Intimidade" para publicar seu perfil',
    'front.profileEmptySecondaryMessage': 'Ao publicar seu perfil, outros usuários poderão acessar seu canal, onde você poderá compartilhar conteúdo e interagir com eles',
    'front.profileRingHintSelect': 'Selecione uma parte de uma das imagens do carrossel',
    'front.profileRingHintMaxRings': 'Máximo atingido: 5 aros',
    'front.profileRingDelete': 'Deletar aro',
    'front.profileRingNameLabel': 'Nome (máx. 38 caracteres)',
    'front.profileRingNameHelper': 'Produto, empresa, usuário, canal, conteúdo etc.',
    'front.profileRingDescriptionLabel': 'Descrição (máx. 280 caracteres)',
    'front.profileRingLinkLabel': 'Link',
    'front.profileRingLinkHelper': 'Adicione um link de rede social para que os usuários possam acessar o produto, empresa, usuário, canal, publicação, conteúdo etc.',
    'front.profileRingLocationLabel': 'Localização',
    'front.profileRingLocationHelper': 'Adicione uma localização',
    'profile.deleteContentTitle': 'Deletar conteúdo',
    'profile.deleteContentBody': 'Tem certeza de que deseja deletar este conteúdo do seu perfil?',
    'front.reactions': 'Reações',
    'front.deletePublicationTitle': 'Remover publicação',
    'front.deletePublicationBody': 'Seu perfil está público para outros usuários na Home; ao remover, apenas você poderá vê-lo em "Seu perfil".',
    'front.category': 'Categoria',
    'front.presentationAddCarouselHint': 'Adicione até 3 imagens ao carrossel',
    'front.presentationTitleLabel': 'Título',
    'front.presentationTitlePlaceholder': 'Escreva um título...',
    'front.presentationBodyLabel': 'Apresentação',
    'front.presentationBodyPlaceholder': 'Escreva sua apresentação...',
    'front.presentationLockedTitle': 'Edição bloqueada',
    'front.presentationLockedBody': 'Seu perfil está publicado no momento.',
    'front.selectSocialNetwork': 'Selecione uma rede social',
    'front.link': 'Link',
    'front.linkMustBeFrom': 'O link deve ser de',
    'front.incorporate': 'Adicionar',
    'front.answerPlaceholder': 'Resposta...',
    'front.chooseCorrectOption': 'Escolha a opção correta',
    'front.addMoreSurveyOptions': 'Adicione mais opções à pesquisa',
    'front.image': 'Imagem',
    'front.text': 'Texto',
    'front.draftPlaceholder': 'Rascunhe...',
    'front.draftIntimacyPlaceholder': 'Rascunhe uma intimidade...',
    'front.intimaciesPublishBlocked': 'Você ainda não pode publicar intimidades. Primeiro crie "Sua apresentação".',
    'front.homeSwipeTutorialHint': 'Quando a barra indicadora na parte superior da tela estiver ativa, deslize para baixo para mudar de publicação.',
    'front.channelLabel': 'Canal:',
    'chat.tabYourChannel': 'Seu canal',
    'chat.tabChannel': 'Canal',
    'chat.tabJoined': 'Entrados',
    'chat.tabYourGroups': 'Seus grupos',
    'chat.tabGroups': 'Grupos',
    'chat.postOnHomeToActivateChannel': 'Publique na Home para ativar seu canal',
    'chat.noChannelsYet': 'Você não tem canais ainda',
    'chat.createGroup': 'Criar um grupo',
    'chat.notJoinedAnyGroupYet': 'Você ainda não entrou em nenhum grupo',
    'chat.back': 'Voltar',
    'chat.interactPlaceholder': 'Interaja...',
    'chat.timeExpired': 'Tempo expirado',
    'chat.remainingPrefix': 'Falta',
    'chat.hour': 'hora',
    'chat.hours': 'horas',
    'chat.minute': 'minuto',
    'chat.minutes': 'minutos',
    'chat.reply': 'Responder',
    'chat.limitWarningMessage': 'Você já interagiu no canal de {user}. Aguarde a resposta.',
    'chat.generalTab': 'Geral',
    'chat.yourThreadsTab': 'Tópicos',
    'chat.recommendationsTab': 'Recomendações',
    'chat.eventsTab': 'Eventos',
    'chat.readingsTab': 'Leituras',
    'chat.imagesTab': 'Imagens',
    'chat.hostLimitedInteractions': '{user} limitou suas interações',
    'chat.waitHostReplyThread': 'Aguarde a resposta do anfitrião para continuar a conversa',
    'chat.waitCreatorReplyAgain': 'Aguarde a resposta do anfitrião antes de responder novamente',
    'chat.selectGroup': 'Selecione um grupo',
    'chat.signInToSendMessages': 'Inicie sessão para enviar mensagens.',
    'chat.unableToSendMessage': 'Não foi possível enviar a mensagem.',
    'chat.connectionErrorSendingMessage': 'Erro de conexão ao enviar a mensagem.',
    'chat.addCaptionPlaceholder': 'Adicione uma legenda...',
    'chat.imageCaptionPlaceholder': 'Legenda da imagem...',
    'reading.screenTitle': 'Criar leitura',
    'reading.titlePlaceholder': 'Título',
    'reading.subtitlePlaceholder': 'Subtítulo',
    'reading.leadPlaceholder': 'Entradilla...',
    'reading.dateLabel': 'Data',
    'reading.datePlaceholder': 'DD/MM/AAAA',
    'reading.chooseCategoryLabel': 'Escolha uma categoria',
    'reading.categoryLabel': 'Categoria',
    'reading.searchCategoryPlaceholder': 'Pesquisar categoria',
    'reading.hypeCostLabel': 'Custo do Hype',
    'reading.hypeCostInfo': 'Escolha a quantidade de chaves brancas que deseja que cada usuário do canal doe por esta leitura para impulsionar o seu perfil. Quanto mais chaves você receber, mais o seu perfil e o seu canal serão impulsionados na tela de Hype.',
    'reading.hypeCostAmountLabel': 'Quantidade',
    'reading.bodyPlaceholder': 'Corpo...',
    'reading.intertitlePlaceholder': 'Intertítulo',
    'reading.imageLimitReachedTitle': 'Limite atingido',
    'reading.imageLimitReachedBody': 'Você só pode adicionar até 3 imagens na leitura.',
    'reading.galleryPermissionTitle': 'Permissão da galeria',
    'reading.galleryPermissionMessage': 'O aplicativo precisa de acesso à sua galeria para selecionar fotos.',
    'reading.galleryPermissionAskLater': 'Perguntar depois',
    'reading.galleryPermissionRequiredTitle': 'Permissão necessária',
    'reading.galleryPermissionRequiredBody': 'Você deve ativar o acesso a Fotos nos Ajustes para poder selecionar imagens.',
    'reading.openSettings': 'Abrir ajustes',
    'reading.imagePickErrorTitle': 'Erro',
    'reading.imagePickErrorBody': 'Não foi possível selecionar a imagem. Tente novamente.',
    'reading.citeSelectionToastBody': 'Selecione uma parte do texto do corpo.',
    'reading.intertitleToastBody': 'Você deve escrever texto no corpo depois de um intertítulo antes de adicionar outro.',
    'reading.bodyMinSuffix': 'min',
    'reading.addImage': 'Adicionar imagem',
    'reading.maxImagesHint': 'Máximo 3',
    'reading.selectedCitationLabel': 'Citação selecionada',
    'reading.citedUsersTitle': 'Usuários citados',
    'reading.citedUsersInputLabel': '@usuários',
    'reading.voiceNoteIntroTitle': 'Nota inicial',
    'reading.voiceNoteIntroDescription': 'Será reproduzida antes do título.',
    'reading.voiceNoteInlineTitle': 'Nota de voz no corpo',
    'reading.voiceNoteInlineDescription': 'Será reproduzida entre blocos do corpo.',
    'reading.voiceNoteOutroTitle': 'Nota final',
    'reading.voiceNoteOutroDescription': 'Será reproduzida abaixo do corpo.',
    'reading.voiceNoteRecordAction': 'Gravar',
    'reading.voiceNoteRerecordAction': 'Regravar',
    'reading.voiceNoteRecordingStatus': 'Gravando',
    'chat.toPrefix': 'Para:',
    'chat.request': 'Solicitar',
    'chat.requestPending': 'Pendente',
    'chat.requestAccepted': 'Aceita',
    'chat.requestBlocked': 'Bloqueada',
    'chat.members': 'Membros',
    'chat.messageHidden': 'Mensagem oculta',
    'chat.messageVisible': 'Mensagem visível',
    'event.enableTasks': 'Ativar tarefas',
    'event.assignTask': 'Atribuir tarefa',
    'event.activateRequests': 'Ativar pedidos',
    'event.rewardTasks': 'Recompensar tarefas',
    'event.describeTask': 'Descreva a tarefa',
    'event.userPlaceholder': '@usuario',
    'event.amount': 'Quantidade',
    'event.type': 'Tipo de evento',
    'event.requiredSuffix': '(obrigatório)',
    'event.name': 'Nome do evento',
    'event.description': 'Descrição do evento',
    'event.dateAndTime': 'Data e hora',
    'event.dayMonthYear': 'Dia/Mês/Ano',
    'event.time': 'Hora',
    'event.durationTitle': 'Duração do evento',
    'event.duration': 'Duração',
    'event.endsIn': 'Termina em',
    'event.link': 'Link',
    'event.selectSocialNetwork': 'Selecionar rede social',
    'event.addLink': 'Adicionar link',
    'event.location': 'Localização',
    'event.selectLocation': 'Selecionar localização',
    'event.searchType': 'Buscar tipo de evento',
    'event.typeOption.social': 'Social',
    'event.typeOption.cultural': 'Cultural',
    'event.typeOption.artistic': 'Artístico',
    'event.typeOption.business': 'Empresarial',
    'event.typeOption.economic': 'Econômico',
    'event.typeOption.academic': 'Acadêmico',
    'event.typeOption.sports': 'Desportivo',
    'event.typeOption.entertainment': 'Entretenimento',
    'event.typeOption.political': 'Político',
    'event.typeOption.religious': 'Religioso',
    'event.typeOption.marketing': 'Marketing',
    'event.typeOption.charity': 'Solidário',
    'event.typeOption.audiovisual': 'Audiovisual',
    'event.typeOption.scientific': 'Científico',
    'event.typeOption.technological': 'Tecnológico',
    'event.typeOption.children': 'Infantil',
    'event.typeOption.musical': 'Musical',
    'event.typeOption.automotive': 'Automobilístico',
    'event.typeOption.fitness': 'Fitness',
    'event.typeOption.gastronomic': 'Gastronômico',
    'event.typeOption.work': 'Laboral',
    'chat.lockedYourGroupsMessage': 'Autentique sua conta para criar seus grupos',
    'chat.lockedJoinedGroupsMessage': 'Para acessar os chats dos grupos em que entrou, autentique sua conta',
    'chat.hostOnlyEventHypeMessage': 'Os utilizadores que entrarem no teu canal são os únicos que podem impulsionar o teu evento',
    'chat.hostOnlyReadingHypeMessage': 'Os utilizadores que entrarem no teu canal são os únicos que podem impulsionar a tua leitura',
    'groups.edit': 'Editar',
    'groups.delete': 'Deletar',
    'groups.deleteConfirmTitle': 'Confirmar exclusão',
    'groups.deleteConfirmBody': 'Tem certeza de que deseja deletar este grupo? Todos os usuários serão removidos.',
    'groups.limit': 'Limitar',
    'groups.limited': 'Limitado',
    'groups.expel': 'Expulsar',
    'groups.expelAndBlock': 'Expulsar e bloquear',
    'groups.leaveGroup': 'Sair do grupo',
    'groups.leaveAndBlock': 'Sair e bloquear',
    'groups.limitUsers': 'Limitar usuários',
    'groups.expelUsers': 'Expulsar usuários',
    'groups.selectAll': 'Selecionar todos',
    'groups.deselect': 'Desselecionar',
    'groups.noMembersToShow': 'Nenhum membro para mostrar',
    'notifications.groupJoinRequestMessage': '{user} enviou uma solicitação para você entrar no grupo {group}. Deseja entrar?',
    'notifications.ignore': 'Ignorar',
    'notifications.accept': 'Aceitar',
    'notifications.read': 'Lido',
    'notifications.unread': 'Não lido',
    'notifications.empty': 'Você não tem notificações',
    'config.title': 'Configurações',
    'config.accountCenter': 'Central da conta',
    'config.blockedUsers': 'Usuários bloqueados',
    'config.devicePermissions': 'Permissões do dispositivo',
    'config.helpCenter': 'Central de ajuda',
    'config.information': 'Sobre o Keinti',
    'config.yourAccountIn': 'Sua conta em',
    'aboutKeinti.title': 'Sobre o Keinti',
    'aboutKeinti.moreAboutKeinti': 'Mais sobre o Keinti',
    'aboutKeinti.body': `Para todos os usuários do Keinti:

O Keinti é uma rede social focada na descoberta de perfis e na interação direta entre pessoas. Nosso objetivo é oferecer um ambiente dinâmico que ajude os usuários a aumentar sua visibilidade, dentro do Keinti e como complemento à sua presença online.

Cada usuário pode criar um perfil público composto por uma Apresentação e espaços onde pode compartilhar reflexões pessoais e aspectos de sua identidade. Essa estrutura ajuda outros membros a compreender melhor seus interesses e estilo. Na tela Home, a plataforma promove a exploração contínua de perfis criados por pessoas do mundo todo.

Quando um perfil despertar interesse, os usuários podem entrar no canal geral do anfitrião/publicador e interagir por meio de conversas em thread. Os publicadores também podem convidar usuários para entrar em seus grupos, a fim de fortalecer e expandir suas comunidades.

Quanto ao desenvolvimento, o Keinti foi totalmente construído com o apoio de inteligência artificial (ChatGPT, Claude Sonnet e Gemini) através do Visual Studio Code. O Keinti iniciou um processo de busca de colaboração para acolher parceiros e colaboradores especializados (desenvolvimento, design gráfico e outros papéis), com foco principal no fortalecimento da segurança e na melhoria da experiência visual do produto.

Para propostas de colaboração, entre em contato: keintisoporte@gmail.com

Atenciosamente,
Equipe Keinti`,
    'verifyKeinti.objectivesTitle': 'Objetivos para conquistar o Keinti Verificado:',
    'verifyKeinti.objective1': '• Autentique sua conta primeiro.',
    'verifyKeinti.complete': 'Completar',
    'verifyKeinti.completed': 'Concluído',
    'verifyKeinti.objective2': '• Obtenha 100.000 aberturas nas suas publicações de intimidades',
    'verifyKeinti.objective3': '• Em um único mês, obtenha pelo menos 1.000 visualizações de outros usuários nas imagens que compartilha nos chats de "Seu canal"',
    'verifyKeinti.objective4': '• Faça 100.000 usuários entrarem no chat de "Seu canal"',
    'verifyKeinti.objective5': '• Crie pelo menos um grupo em "Seus grupos" e faça com que pelo menos 200 membros ativos entrem nesses grupos.',
    'verifyKeinti.objective6': '• Publique seu perfil pelo menos 40 vezes na Home.',
    'verifyKeinti.benefitsTitle': 'Benefícios ao conquistar o Keinti Verificado:',
    'verifyKeinti.benefit1': '• Obtenha a badge dourada.',
    'verifyKeinti.benefit2': '• Gere renda para cada usuário que acesse os chats de "Seu canal".',
    'verifyKeinti.benefit3': '• Gere renda a cada 1.000 visualizações das imagens que compartilha nos chats de "Seu canal"',
    'verifyKeinti.benefit4': '• Obtenha outros privilégios futuros oferecidos pelo Keinti em próximas atualizações.',
    'verifyKeinti.tabObjectives': 'Objetivos',
    'verifyKeinti.tabBenefits': 'Benefícios',
    'verifyKeinti.importantNotice': 'Aviso importante',
    'verifyKeinti.importantNoticeBody': 'A monetização chegará em breve ao Keinti. Verifique sua conta para obter todos os benefícios quando estiverem disponíveis.',
    'verifyKeinti.verifyAction': 'Verificar',
    'verifyKeinti.accountVerifiedLabel': 'Conta verificada',
    'devicePermissions.description': 'Aqui você pode ver as permissões que concedeu ao app.',
    'devicePermissions.galleryTitle': 'Galeria (fotos)',
    'devicePermissions.galleryDescription': 'Usada quando você adiciona uma imagem ao seu avatar, à sua apresentação ou às intimidades.',
    'devicePermissions.statusGranted': 'Concedida',
    'devicePermissions.statusDenied': 'Não concedida',
    'devicePermissions.statusUnknown': 'Indisponível',
    'devicePermissions.iosHint': 'No iOS essa permissão é gerenciada pelo sistema e será solicitada ao selecionar uma imagem.',
    'devicePermissions.revokeConfirmTitle': 'Remover permissão',
    'devicePermissions.revokeConfirmBody': 'Tem certeza de que deseja remover a permissão do Keinti de acessar sua galeria?',
    'devicePermissions.reEnableConfirmTitle': 'Permissão da galeria',
    'devicePermissions.reEnableConfirmBody': 'Deseja permitir que o Keinti acesse sua galeria?',
    'devicePermissions.osRequestTitle': 'Permissão da galeria',
    'devicePermissions.osRequestMessage': 'O app precisa de acesso à sua galeria para selecionar fotos.',
    'devicePermissions.osRequestAskLater': 'Perguntar depois',
    'devicePermissions.osRequestDeny': 'Cancelar',
    'devicePermissions.osRequestAllow': 'Permitir',
    'accountCenter.title': 'Central da conta',
    'accountCenter.personalData': 'Dados pessoais',
    'accountCenter.securityControl': 'Controle de segurança',
    'accountCenter.changePassword': 'Alterar senha',
    'accountCenter.adminSelfies': 'Admin: revisar selfies',
    'accountCenter.closeAccount': 'Sair',
    'accountCenter.deleteAccount': 'Excluir conta',
    'accountCenter.deleteAccountConfirmTitle': 'Excluir conta',
    'accountCenter.deleteAccountConfirmBody': 'Tem certeza de que deseja excluir sua conta? Esta ação é irreversível e removerá seus dados do sistema.',
    'accountCenter.accountDeletionPolicyLink': 'Ver política de exclusão de conta',
    'personalData.title': 'Dados pessoais',
    'personalData.description': 'O Keinti usa essas informações para verificar sua identidade e proteger a comunidade.',
    'personalData.contactInfo': 'Informações de contato',
    'personalData.birthDate': 'Data de nascimento',
    'personalData.gender': 'Gênero',
    'personalData.nationality': 'Nacionalidade',
    'securityControl.passwordAndAuth': 'Senha e autenticação',
    'securityControl.passwordAndAuthDescription': 'Nesta seção você pode autenticar seu perfil e alterar sua senha.',
    'securityControl.accountAuth': 'Autenticação da conta',
    'securityControl.privacyPolicy': 'Política de privacidade',
    'securityControl.cookiesAdPolicy': 'Política de cookies e publicidade',
    'securityControl.termsOfUse': 'Termos de uso',
    'securityControl.childSafetyStandards': 'Padrões de segurança infantil',
    'securityControl.accountDeletionPolicy': 'Política de exclusão de conta',
    'securityControl.verifyYourKeinti': 'Verifique seu Keinti',
    'accountAuth.description': 'Para autenticar sua conta você deve completar 2 etapas: selfie de verificação e código do Google Authenticator. A selfie é validada automaticamente e só vai para análise adicional quando o sistema não consegue decidir com segurança.',
    'accountAuth.step1Title': 'Etapa 1: selfie de verificação',
    'accountAuth.step1Body': 'Tire uma selfie para que o sistema valide automaticamente que a imagem é adequada para segurança e anti-impersonação antes de liberar a etapa 2.',
    'accountAuth.step1Hint': 'Certifique-se de ter boa iluminação e enquadrar seu rosto.',
    'accountAuth.step1Action': 'Tirar selfie',
    'accountAuth.step1Pending': 'Análise adicional',
    'accountAuth.pendingReviewNotice': 'Sua selfie ainda está em análise. Essa revisão pode levar até 24 horas. Se, após esse período, ao tocar em "Atualizar status", ela continuar como "Análise adicional", entre em contato com keintisoporte@gmail.com.',
    'accountAuth.step1Blocked': 'Bloqueado',
    'accountAuth.refreshStatus': 'Atualizar status',
    'accountAuth.selfieAccepted': 'Selfie aceita',
    'accountAuth.statusNotSubmitted': 'Status: ainda não enviada.',
    'accountAuth.statusPending': 'Status: análise adicional necessária.',
    'accountAuth.statusAccepted': 'Status: aceita. Etapa 2 disponível.',
    'accountAuth.statusFailed': 'Status: falhou. Tente novamente com melhor iluminação e enquadramento.',
    'accountAuth.statusBlocked': 'Status: bloqueado. Você não pode mais enviar selfies até que um administrador o desbloqueie.',
    'accountAuth.errorTitle': 'Erro',
    'accountAuth.selfieUploadError': 'Não foi possível enviar a selfie. Tente novamente.',
    'accountAuth.step2Title': 'Etapa 2: Google Authenticator',
    'accountAuth.step2Body': 'Configure o Google Authenticator e verifique o código para concluir a autenticação.',
    'accountAuth.step2Locked': 'Esta etapa será ativada quando a selfie for aceita.',
    'accountAuth.step2Ready': 'Pronto para configurar o Google Authenticator.',
    'accountAuth.step2AlreadyEnabled': 'Google Authenticator já está ativado.',
    'accountAuth.generateTotp': 'Gerar configuração',
    'accountAuth.secretLabel': 'Chave secreta',
    'accountAuth.secretHint': 'Adicione uma conta no Google Authenticator usando esta chave (manual). Depois digite o código de 6 dígitos.',
    'accountAuth.codeLabel': 'Código',
    'accountAuth.codePlaceholder': '123456',
    'accountAuth.verifyCode': 'Verificar código',
    'accountAuth.totpSetupError': 'Não foi possível gerar a configuração.',
    'accountAuth.verifyError': 'Não foi possível verificar o código.',
    'accountAuth.successTitle': 'Concluído',
    'accountAuth.successBody': 'Sua conta foi autenticada e a badge agora está ativa.',
    'accountAuth.completed': 'Autenticação concluída.',
    'accountAuth.badgeExpiresIn': 'A badge expira em:',
    'changePassword.requirements': 'A senha deve ter no mínimo 10 caracteres e incluir pelo menos um caractere especial (#!@_$€%).',
    'changePassword.currentPassword': 'Senha atual',
    'changePassword.currentPasswordInvalid': 'Senha incorreta',
    'changePassword.newPassword': 'Nova senha',
    'changePassword.repeatNewPassword': 'Repetir nova senha',
    'changePassword.passwordsDontMatch': 'As senhas não correspondem',
    'changePassword.forgot': 'Esqueceu sua senha?',
    'changePassword.success': 'Senha atualizada',
    'changePassword.locked': 'Você atingiu o máximo de tentativas. Tente novamente mais tarde.',
    'changePassword.attemptsRemaining': 'Tentativas restantes: {count}',
    'changePassword.accountLocked': 'Sua conta foi bloqueada. Você será desconectado automaticamente.',
    'changePassword.checkCompleted': 'Verificação concluída',
    'blockedUsers.title': 'Usuários bloqueados',
    'blockedUsers.empty': 'Você não tem usuários bloqueados',
    'adminSelfies.title': 'Admin · Selfies',
    'adminSelfies.tabPending': 'Análise adicional',
    'adminSelfies.tabBlocked': 'Bloqueados',
    'adminSelfies.refresh': 'Atualizar',
    'adminSelfies.emptyPending': 'Nenhuma selfie em análise adicional.',
    'adminSelfies.emptyBlocked': 'Nenhum usuário bloqueado.',
    'adminSelfies.reasonPlaceholder': 'Motivo (opcional)',
    'adminSelfies.accept': 'Aceitar',
    'adminSelfies.reject': 'Rejeitar',
    'adminSelfies.block': 'Bloquear',
    'adminSelfies.unblock': 'Desbloquear',
    'blockedUsers.reasonPlaceholder': 'Sem motivo',
    'toast.joinedChannelOf': 'Você entrou no canal de',
    'toast.published': 'Publicado',
    'toast.publicationNotActiveHome': 'A publicação não está mais ativa na Home',
    'toast.joinedGroupMessage': 'Você entrou no grupo {group} de {user}',
    'toast.blocked': 'Bloqueado',
    'toast.reportSent': 'Denúncia enviada',
    'carouselEditor.adjustImage': 'Ajustar imagem',
    'carouselEditor.imageCount': 'Imagem {current} de {total}',
    'carouselEditor.processing': 'Processando...',
    'carouselEditor.noPreview': 'Sem pré-visualização',
    'carouselEditor.editImage': 'Editar imagem',
    'carouselEditor.errorTitle': 'Erro',
    'carouselEditor.cropErrorBody': 'Não foi possível recortar a imagem. Tente novamente.',
    'carouselEditor.ok': 'OK',
  },
  de: {
    'language.label': 'Sprache',
    'language.spanish': 'Spanisch',
    'language.english': 'Englisch',
    'language.french': 'Französisch',
    'language.portuguese': 'Portugiesisch',
    'language.german': 'Deutsch',
    'language.italian': 'Italienisch',
    'gender.male': 'Mann',
    'gender.female': 'Frau',
    'gender.unspecified': 'Keine Angabe',
    'common.loading': 'Laden...',
    'common.logout': 'Abmelden',
    'common.notifications': 'Benachrichtigungen',
    'common.keys': 'Schlüssel',
    'common.or': 'oder',
    'common.unlock': 'Entsperren',
    'common.view': 'Anzeigen',
    'common.forgotPassword': '[[Passwort]] vergessen?',
    'common.cancel': 'Abbrechen',
    'common.confirm': 'Bestätigen',
    'common.delete': 'Löschen',
    'common.report': 'Melden',
    'common.block': 'Blockieren',
    'common.check': 'Prüfen',
    'common.apply': 'Anwenden',
    'common.accept': 'Akzeptieren',
    'common.create': 'Erstellen',
    'keys.convert': 'Umwandeln',
    'keys.dailyGoals': 'Tägliche Ziele',
    'keys.enterAmount': 'Betrag eingeben',
    'keys.convertMultipleOfTenHint': 'Der Betrag muss ein Vielfaches von 10 sein.',
    'keys.convertInsufficientWhiteKeys': 'Du hast nicht genug weiße Schlüssel für diese Umwandlung.',
    'keys.convertRateHint': 'Je 10 weiße Schlüssel werden in 1 Gradient-Schlüssel umgewandelt.',
    'keys.redeem': 'Einlösen',
    'keys.redeemSubtitle': 'Wähle eine der Optionen',
    'keys.redeemOption10': '10 Euro',
    'keys.redeemOption20': '20 Euro',
    'keys.redeemOption50': '50 Euro',
    'keys.redeemOption100': '100 Euro',
    'keys.redeemOption500': '500 Euro',
    'keys.redeemInsufficientKeys': 'Du hast nicht genug Schlüssel.',
    'keys.redeemNote': 'Du kannst nur einen Gutschein pro Tag einlösen. Wähle sorgfältig. Das eingelöste Geld wird an das PayPal-Konto gesendet, das du im nächsten Schritt angibst, nachdem du auf die Schaltfläche „Einlösen“ getippt hast.',
    'keys.lockedMessage': 'Authentifiziere dein Konto, um auf deine täglichen Ziele und Schlüssel zuzugreifen.',
    'keys.dailyGoal1': 'Für jeden Intimitäten-Container, den du öffnest, erhältst du 2 weiße Schlüssel.',
    'keys.dailyGoal2': 'Schaffe es, einen Thread mit einem Gastgeber in seinem Kanal zu verknüpfen.',
    'keys.dailyGoal3': 'Schalte die Bilder frei, die Gastgeber in ihren Kanälen teilen, denen du beitrittst, und erhalte 10 weiße Schlüssel für jedes davon.',
    'keys.dailyGoal4': 'Veröffentliche dein Profil jeden Tag.',
    'keys.dailyGoal5': 'Erstelle ein Event in deinem Kanal.',
    'keys.dailyGoal6': 'Teile ein Bild in deinem Kanal.',
    'keys.dailyGoal7': 'Sorge dafür, dass 100 Benutzer deinen Kanal betreten: 1.000 weiße Schlüssel.',
    'keys.dailyGoal8': 'Sorge dafür, dass 1.000 Benutzer deinen Kanal betreten: 10.000 weiße Schlüssel.',
    'keys.dailyGoal9': 'Sorge dafür, dass 10.000 Benutzer deinen Kanal betreten: 100.000 weiße Schlüssel.',
    'auth.sessionRequiredTitle': 'Sitzung erforderlich',
    'auth.signInToBlockUsers': 'Melde dich an, um Benutzer zu blockieren.',
    'auth.signInToReportUsers': 'Melde dich an, um Benutzer zu melden.',
    'home.blockConfirmQuestion': 'Möchtest du {user} blockieren?',
    'home.blockReasonPlaceholder': 'Grund der Blockierung (optional)...',
    'home.reportSelectReason': 'Wähle einen Grund, um {user} zu melden.',
    'report.reason.sexualContent': 'Sexuelle Inhalte oder Nacktheit',
    'report.reason.harassment': 'Belästigung oder Mobbing',
    'report.reason.offensiveLanguage': 'Beleidigende Sprache',
    'report.reason.scam': 'Betrug oder Täuschung',
    'report.reason.violence': 'Gewalt oder Drohungen',
    'report.reason.spam': 'Spam',
    'report.reason.impersonation': 'Identitätsdiebstahl',
    'report.reason.illegalContent': 'Illegale Inhalte',
    'report.reason.childSexualAbuse': 'Sexueller Missbrauch/Ausbeutung von Kindern',
    'report.reason.weaponsOrDrugsIncitement': 'Anstiftung zum Gebrauch von Waffen/Drogen',
    'report.reason.inappropriateConduct': 'Unangemessenes Verhalten',
    'report.reason.other': 'Sonstiges',
    'errors.unableToBlockUser': 'Benutzer konnte nicht blockiert werden',
    'errors.unableToSendReport': 'Meldung konnte nicht gesendet werden',
    'toast.joinedChannelOf': 'Du bist dem Kanal beigetreten von',
    'toast.published': 'Veröffentlicht',
    'toast.publicationNotActiveHome': 'Der Beitrag ist nicht mehr auf der Startseite aktiv',
    'toast.joinedGroupMessage': 'Du bist der Gruppe {group} von {user} beigetreten',
    'toast.blocked': 'Blockiert',
    'toast.reportSent': 'Meldung gesendet',
    'carouselEditor.adjustImage': 'Bild anpassen',
    'carouselEditor.imageCount': 'Bild {current} von {total}',
    'carouselEditor.processing': 'Verarbeitung...',
    'carouselEditor.noPreview': 'Keine Vorschau',
    'carouselEditor.editImage': 'Bild bearbeiten',
    'carouselEditor.errorTitle': 'Fehler',
    'carouselEditor.cropErrorBody': 'Das Bild konnte nicht zugeschnitten werden. Bitte versuche es erneut.',
    'carouselEditor.ok': 'OK',
    'login.email': 'E-Mail',
    'login.password': 'Passwort',
    'login.emailPlaceholder': 'beispiel@email.com',
    'login.passwordPlaceholder': 'Gib dein Passwort ein',
    'login.signIn': 'Anmelden',
    'login.fillAllFields': 'Bitte fülle alle Felder aus',
    'login.invalidCredentials': 'Ungültige Anmeldedaten',
    'login.googleNotRegistered': 'Dieses Konto ist noch nicht registriert. Registriere dich zuerst, um dich mit dieser E-Mail anmelden zu können.',
    'login.googlePkceMissing': 'Die Google-Anmeldung konnte nicht abgeschlossen werden. Bitte versuche es erneut.',
    'login.noAccount': 'Noch kein Konto? ',
    'login.signUp': 'Registrieren',
    'login.resetTitle': 'Passwort zurücksetzen',
    'login.resetSend': 'Senden',
    'login.resetVerify': 'Bestätigen',
    'login.resetChangePassword': 'Passwort ändern',
    'login.resetEmailNotRegistered': 'Diese E-Mail wird noch nicht bei Keinti verwendet. Registriere dich.',
    'login.resetInvalidEmail': 'Ungültige E-Mail',
    'login.resetCodeSent': 'Wir haben einen Code an deine E-Mail gesendet.',
    'login.resetPasswordChanged': 'Passwort aktualisiert. Du kannst dich jetzt anmelden.',
    'login.resetCodeLabel': 'Code',
    'login.resetNewPasswordLabel': 'Neues Passwort erstellen',
    'login.resetRepeatNewPasswordLabel': 'Neues Passwort wiederholen',
    'register.email': 'E-Mail',
    'register.username': '@Benutzername',
    'register.usernameAlreadyInUse': '"{username}" wird bereits von einem anderen Benutzer verwendet; bitte versuche einen anderen.',
    'register.birthDate': 'Geburtsdatum',
    'register.birthDatePlaceholder': 'TT/MM/JJJJ',
    'register.gender': 'Geschlecht',
    'register.selectGender': 'Wähle dein Geschlecht',
    'register.nationality': 'Nationalität',
    'register.password': 'Passwort',
    'register.confirmPassword': 'Passwort bestätigen',
    'register.confirmPasswordPlaceholder': 'Bestätige dein Passwort',
    'register.next': 'Weiter',
    'register.signUp': 'Registrieren',
    'register.registering': 'Registrierung...',
    'register.successMessage': 'Willkommen! Du bist jetzt ein Keinti-Benutzer.',
    'register.privacyPrefix': 'Mit der Registrierung akzeptierst du die ',
    'register.privacyPolicies': 'Datenschutzrichtlinie, die Cookie- und Werberichtlinie sowie die Nutzungsbedingungen',
    'register.privacyPolicyLink': 'Datenschutzrichtlinie',
    'register.cookiesAdsPolicyLink': 'Cookie- und Werberichtlinie',
    'register.termsConditionsLink': 'Nutzungsbedingungen',
    'register.privacySeparator1': ', die ',
    'register.privacySeparator2': ' sowie die ',
    'register.privacySuffix': ' unserer Keinti-App. Zu deiner Sicherheit lies sie bitte vor der Registrierung.',
    'register.errorTitle': 'Registrierungsfehler',
    'register.unableToComplete': 'Registrierung konnte nicht abgeschlossen werden.',
    'register.selectNationality': 'Wähle deine Nationalität',
    'register.searchCountry': 'Land suchen...',
    'register.noCountriesFound': 'Keine Länder gefunden',
    'register.emailVerificationTitle': 'E-Mail-Verifizierung',
    'register.emailVerificationHint': 'Wir haben einen Code gesendet an',
    'register.emailVerificationLinkHint': 'Wenn du einen Bestätigungslink statt eines Codes erhältst, öffne ihn und du wirst automatisch zu Keinti zurückgeleitet.',
    'register.iConfirmedEmail': 'Ich habe die E-Mail bereits bestätigt',
    'register.confirmedCheckHint': 'Wenn du den Link bereits geöffnet hast, kehre zu Keinti zurück. Falls nichts passiert, warte einige Sekunden und tippe erneut, oder sende die E-Mail erneut.',
    'register.code': 'Code',
    'register.codePlaceholder': 'Code eingeben',
    'register.timeRemaining': 'Verbleibende Zeit',
    'register.verifyAndSignUp': 'Verifizieren und registrieren',
    'register.verifying': 'Verifizierung...',
    'register.resendCode': 'E-Mail erneut senden',
    'register.codeExpired': 'Der Code ist abgelaufen. Fordere einen neuen an.',
    'register.codeIncorrect': 'Falscher Code',
    'register.codeInvalidInline': 'Der eingegebene Code ist ungültig. Bitte versuche es erneut.',
    'register.remainingAttempts': 'Verbleibende Versuche',
    'register.remainingAttemptsInline': 'Verbleibende Versuche: {count}',
    'register.emailTemporarilyBlocked': 'Zu viele Versuche. Starte die Registrierung mit einer anderen E-Mail neu.',
    'register.emailLockedInlineError': 'Diese E-Mail-Adresse wurde vorübergehend gesperrt. Wir überprüfen sie. Bitte sende uns eine Beschwerde, damit wir deine E-Mail bei Keinti reaktivieren können.',
    'register.emailLockedRetryInline': 'Diese E-Mail-Adresse wurde vorübergehend gesperrt. Versuche eine andere oder warte auf die Lösung deiner Berichtigungsanfrage.',
    'register.rectificationTitle': 'Berichtigung',
    'register.rectificationPlaceholder': 'Beschreibe deine Berichtigung (max. 220 Zeichen)',
    'register.rectificationSend': 'Beschwerde senden',
    'register.rectificationSent': 'Beschwerde gesendet. Wir werden deine Anfrage so schnell wie möglich prüfen.',
    'register.tryLaterOrCheckEmailExists': 'Versuche es später erneut oder prüfe, ob die E-Mail existiert',
    'register.mustVerifyEmailFirst': 'Du musst deine E-Mail verifizieren, bevor du dich registrierst',
    'register.cancelledNotice': 'Registrierung abgebrochen. Wenn du ein neues Keinti-Konto erstellen möchtest, starte den Registrierungsprozess erneut.',
    'register.emailAlreadyInUse': 'Diese E-Mail wird bereits von einem anderen Keinti-Benutzer verwendet. Bitte versuche eine andere.',
    'register.emailSendFailed': 'Wir konnten den Code nicht an die E-Mail "{email}" senden. Bitte überprüfe, ob die E-Mail-Adresse gültig ist.',
    'register.missingProfileData': 'Fehlende Registrierungsdaten. Gehe zurück und vervollständige Benutzername, Geburtsdatum, Geschlecht und Nationalität.',
    'register.signupExpiredNotice': 'Die Verifizierungszeit ist abgelaufen. Deine Registrierungsdaten wurden gelöscht. Du kannst es erneut versuchen.',
    'register.signupExpiredTitle': 'Zeit abgelaufen',
    'register.emailLocked24h': 'Du hast alle {max} Registrierungsversuche mit dieser E-Mail verbraucht. Du kannst sie 48 Stunden lang nicht erneut verwenden.',
    'register.attemptsRemaining': 'Verbleibende Versuche: {remaining} von {max}',
    'register.pendingConfirmation': 'Bestätige deine E-Mail, bevor der Timer abläuft – er ist noch aktiv.',
    'register.emailLockedTemp': 'Du hast alle {max} Registrierungsversuche mit dieser E-Mail verbraucht. Du kannst sie 48 Stunden lang nicht erneut verwenden.',
    'register.emailLockedPermanent': 'Deine E-Mail wurde dauerhaft gesperrt, da das Limit der Registrierungsversuche überschritten wurde. Kontaktiere keintisoporte@gmail.com, um die Entsperrung anzufordern.',
    'validation.invalidDateFormat': 'Ungültiges Datumsformat',
    'validation.invalidDate': 'Ungültiges Datum',
    'validation.mustBeAdult': 'Du musst mindestens 18 Jahre alt sein',
    'validation.passwordMinLength': 'Das Passwort muss mindestens 10 Zeichen haben',
    'validation.passwordMaxLength': 'Das Passwort darf maximal 20 Zeichen haben',
    'validation.passwordNeedsLowercase': 'Muss mindestens 1 Kleinbuchstaben enthalten',
    'validation.passwordNeedsUppercase': 'Muss mindestens 1 Großbuchstaben enthalten',
    'validation.passwordNeedsLetter': 'Muss mindestens 1 Buchstaben enthalten',
    'validation.passwordNeedsNumber': 'Muss mindestens 1 Zahl enthalten',
    'validation.passwordNeedsSpecial': 'Muss mindestens 1 Sonderzeichen enthalten',
    'validation.passwordsDontMatch': 'Die Passwörter stimmen nicht überein',
    'front.editProfilePhoto': 'Profilfoto bearbeiten',
    'front.yourSocialNetworks': 'Deine sozialen Netzwerke',
    'front.userPlaceholder': 'Benutzer',
    'front.selectCategory': 'Wähle eine Kategorie',
    'front.noPublicationsAvailable': 'Keine Beiträge verfügbar',
    'front.forYou': 'Für dich',
    'front.publicationsCount': 'Beiträge',
    'front.socialNetworksSingular': 'soziales Netzwerk',
    'front.socialNetworksPlural': 'soziale Netzwerke',
    'front.publish': 'Veröffentlichen',
    'front.remove': 'Entfernen (Veröffentlicht)',
    'front.yourProfile': 'Dein Profil',
    'front.yourPresentation': 'Deine Vorstellung',
    'front.intimacies': 'Intimitäten',
    'front.time': 'Zeit',
    'front.loadingImages': 'Bilder werden geladen...',
    'front.readMore': 'Mehr lesen...',
    'front.readLess': 'Weniger lesen',
    'front.votes': 'Stimmen',
    'front.profileEmptyPrimaryMessage': 'Füge „Deine Vorstellung" und mindestens eine „Intimität" hinzu, um dein Profil zu veröffentlichen',
    'front.profileEmptySecondaryMessage': 'Wenn du dein Profil veröffentlichst, können andere Benutzer auf deinen Kanal zugreifen, wo du Inhalte teilen und mit ihnen interagieren kannst',
    'front.profileRingHintSelect': 'Wähle einen Teil eines der Karussellbilder',
    'front.profileRingHintMaxRings': 'Maximum erreicht: 5 Ringe',
    'front.profileRingDelete': 'Ring löschen',
    'front.profileRingNameLabel': 'Name (max. 38 Zeichen)',
    'front.profileRingNameHelper': 'Produkt, Unternehmen, Benutzer, Kanal, Inhalt usw.',
    'front.profileRingDescriptionLabel': 'Beschreibung (max. 280 Zeichen)',
    'front.profileRingLinkLabel': 'Link',
    'front.profileRingLinkHelper': 'Füge einen Social-Media-Link hinzu, damit Benutzer auf das Produkt, Unternehmen, den Benutzer, Kanal, Beitrag, Inhalt usw. zugreifen können.',
    'front.profileRingLocationLabel': 'Standort',
    'front.profileRingLocationHelper': 'Einen Standort hinzufügen',
    'profile.deleteContentTitle': 'Inhalt löschen',
    'profile.deleteContentBody': 'Bist du sicher, dass du diesen Inhalt von deinem Profil löschen möchtest?',
    'front.reactions': 'Reaktionen',
    'front.deletePublicationTitle': 'Beitrag entfernen',
    'front.deletePublicationBody': 'Dein Profil ist derzeit auf der „Startseite" öffentlich sichtbar; wenn du es entfernst, kannst nur du es unter „Dein Profil" sehen.',
    'front.category': 'Kategorie',
    'front.presentationAddCarouselHint': 'Füge bis zu 3 Bilder zum Karussell hinzu',
    'front.presentationTitleLabel': 'Titel',
    'front.presentationTitlePlaceholder': 'Schreibe einen Titel...',
    'front.presentationBodyLabel': 'Vorstellung',
    'front.presentationBodyPlaceholder': 'Schreibe deine Vorstellung...',
    'front.presentationLockedTitle': 'Bearbeitung gesperrt',
    'front.presentationLockedBody': 'Dein Profil ist derzeit veröffentlicht.',
    'front.selectSocialNetwork': 'Wähle ein soziales Netzwerk',
    'front.link': 'Link',
    'front.linkMustBeFrom': 'Der Link muss von',
    'front.incorporate': 'Hinzufügen',
    'front.answerPlaceholder': 'Antwort...',
    'front.chooseCorrectOption': 'Wähle die richtige Option',
    'front.addMoreSurveyOptions': 'Füge mehr Optionen zur Umfrage hinzu',
    'front.image': 'Bild',
    'front.text': 'Text',
    'front.draftPlaceholder': 'Schreibe...',
    'front.draftIntimacyPlaceholder': 'Schreibe eine Intimität...',
    'front.intimaciesPublishBlocked': 'Du kannst noch keine Intimitäten veröffentlichen. Erstelle zuerst „Deine Vorstellung".',
    'front.homeSwipeTutorialHint': 'Wenn die Anzeigeleiste oben auf dem Bildschirm aktiv ist, wische nach unten, um den Beitrag zu wechseln.',
    'front.channelLabel': 'Kanal:',
    'chat.tabYourChannel': 'Dein Kanal',
    'chat.tabChannel': 'Kanal',
    'chat.tabJoined': 'Beigetreten',
    'chat.tabYourGroups': 'Deine Gruppen',
    'chat.tabGroups': 'Gruppen',
    'chat.postOnHomeToActivateChannel': 'Veröffentliche auf der Startseite, um deinen Kanal zu aktivieren',
    'chat.noChannelsYet': 'Du hast noch keine Kanäle',
    'chat.createGroup': 'Gruppe erstellen',
    'chat.notJoinedAnyGroupYet': 'Du bist noch keiner Gruppe beigetreten',
    'chat.back': 'Zurück',
    'chat.interactPlaceholder': 'Interagiere...',
    'chat.timeExpired': 'Zeit abgelaufen',
    'chat.remainingPrefix': 'Noch',
    'chat.hour': 'Stunde',
    'chat.hours': 'Stunden',
    'chat.minute': 'Minute',
    'chat.minutes': 'Minuten',
    'chat.reply': 'Antworten',
    'chat.limitWarningMessage': 'Du hast bereits im Kanal von {user} interagiert. Bitte warte auf die Antwort.',
    'chat.generalTab': 'Allgemein',
    'chat.yourThreadsTab': 'Threads',
    'chat.recommendationsTab': 'Empfehlungen',
    'chat.eventsTab': 'Ereignisse',
    'chat.readingsTab': 'Lektüren',
    'chat.imagesTab': 'Bilder',
    'chat.hostLimitedInteractions': '{user} hat deine Interaktionen eingeschränkt',
    'chat.waitHostReplyThread': 'Warte auf die Antwort des Hosts, um den Thread fortzusetzen',
    'chat.waitCreatorReplyAgain': 'Warte auf die Antwort des Hosts, bevor du erneut antwortest',
    'chat.selectGroup': 'Wähle eine Gruppe',
    'chat.toPrefix': 'An:',
    'chat.request': 'Anfragen',
    'chat.requestPending': 'Ausstehend',
    'chat.requestAccepted': 'Akzeptiert',
    'chat.requestBlocked': 'Blockiert',
    'chat.members': 'Mitglieder',
    'chat.messageHidden': 'Nachricht verborgen',
    'chat.messageVisible': 'Nachricht sichtbar',
    'event.enableTasks': 'Aufgaben aktivieren',
    'event.assignTask': 'Aufgabe zuweisen',
    'event.activateRequests': 'Anfragen aktivieren',
    'event.rewardTasks': 'Aufgaben belohnen',
    'event.describeTask': 'Beschreibe die Aufgabe',
    'event.userPlaceholder': '@benutzer',
    'event.amount': 'Betrag',
    'event.type': 'Ereignistyp',
    'event.requiredSuffix': '(erforderlich)',
    'event.name': 'Name des Ereignisses',
    'event.description': 'Beschreibung des Ereignisses',
    'event.dateAndTime': 'Datum und Uhrzeit',
    'event.dayMonthYear': 'Tag/Monat/Jahr',
    'event.time': 'Uhrzeit',
    'event.durationTitle': 'Dauer des Ereignisses',
    'event.duration': 'Dauer',
    'event.endsIn': 'Endet in',
    'event.link': 'Link',
    'event.selectSocialNetwork': 'Soziales Netzwerk auswählen',
    'event.addLink': 'Link hinzufügen',
    'event.location': 'Standort',
    'event.selectLocation': 'Standort auswählen',
    'event.searchType': 'Ereignistyp suchen',
    'event.typeOption.social': 'Sozial',
    'event.typeOption.cultural': 'Kulturell',
    'event.typeOption.artistic': 'Künstlerisch',
    'event.typeOption.business': 'Unternehmerisch',
    'event.typeOption.economic': 'Wirtschaftlich',
    'event.typeOption.academic': 'Akademisch',
    'event.typeOption.sports': 'Sportlich',
    'event.typeOption.entertainment': 'Unterhaltung',
    'event.typeOption.political': 'Politisch',
    'event.typeOption.religious': 'Religiös',
    'event.typeOption.marketing': 'Marketing',
    'event.typeOption.charity': 'Gemeinnützig',
    'event.typeOption.audiovisual': 'Audiovisuell',
    'event.typeOption.scientific': 'Wissenschaftlich',
    'event.typeOption.technological': 'Technologisch',
    'event.typeOption.children': 'Kinder',
    'event.typeOption.musical': 'Musikalisch',
    'event.typeOption.automotive': 'Automobil',
    'event.typeOption.fitness': 'Fitness',
    'event.typeOption.gastronomic': 'Gastronomisch',
    'event.typeOption.work': 'Beruflich',
    'chat.signInToSendMessages': 'Melde dich an, um Nachrichten zu senden.',
    'chat.unableToSendMessage': 'Die Nachricht konnte nicht gesendet werden.',
    'chat.connectionErrorSendingMessage': 'Verbindungsfehler beim Senden der Nachricht.',
    'chat.addCaptionPlaceholder': 'Füge eine Bildunterschrift hinzu...',
    'chat.imageCaptionPlaceholder': 'Bildunterschrift...',
    'reading.screenTitle': 'Lesebeitrag erstellen',
    'reading.titlePlaceholder': 'Titel',
    'reading.subtitlePlaceholder': 'Untertitel',
    'reading.leadPlaceholder': 'Vorspann...',
    'reading.dateLabel': 'Datum',
    'reading.datePlaceholder': 'TT/MM/JJJJ',
    'reading.chooseCategoryLabel': 'Wähle eine Kategorie',
    'reading.categoryLabel': 'Kategorie',
    'reading.searchCategoryPlaceholder': 'Kategorie suchen',
    'reading.hypeCostLabel': 'Hype-Kosten',
    'reading.hypeCostInfo': 'Wähle die Anzahl weißer Schlüssel, die dir jeder Nutzer des Kanals für diese Lesung spenden soll, um dein Profil zu pushen. Je mehr Schlüssel du erhältst, desto stärker werden dein Profil und dein Kanal im Hype-Bildschirm verbreitet.',
    'reading.hypeCostAmountLabel': 'Menge',
    'reading.bodyPlaceholder': 'Text...',
    'reading.intertitlePlaceholder': 'Zwischentitel',
    'reading.imageLimitReachedTitle': 'Limit erreicht',
    'reading.imageLimitReachedBody': 'Du kannst dem Lesebeitrag nur bis zu 3 Bilder hinzufügen.',
    'reading.galleryPermissionTitle': 'Galerieberechtigung',
    'reading.galleryPermissionMessage': 'Die App benötigt Zugriff auf deine Galerie, um Fotos auszuwählen.',
    'reading.galleryPermissionAskLater': 'Später fragen',
    'reading.galleryPermissionRequiredTitle': 'Berechtigung erforderlich',
    'reading.galleryPermissionRequiredBody': 'Du musst den Zugriff auf Fotos in den Einstellungen aktivieren, um Bilder auswählen zu können.',
    'reading.openSettings': 'Einstellungen öffnen',
    'reading.imagePickErrorTitle': 'Fehler',
    'reading.imagePickErrorBody': 'Das Bild konnte nicht ausgewählt werden. Bitte versuche es erneut.',
    'reading.citeSelectionToastBody': 'Wähle einen Teil des Haupttexts aus.',
    'reading.intertitleToastBody': 'Du musst Text im Hauptteil nach einem Zwischentitel schreiben, bevor du einen weiteren hinzufügst.',
    'reading.bodyMinSuffix': 'min',
    'reading.addImage': 'Bild hinzufügen',
    'reading.maxImagesHint': 'Maximal 3',
    'reading.selectedCitationLabel': 'Ausgewähltes Zitat',
    'reading.citedUsersTitle': 'Zitierte Nutzer',
    'reading.citedUsersInputLabel': '@nutzer',
    'reading.voiceNoteIntroTitle': 'Einleitende Sprachnotiz',
    'reading.voiceNoteIntroDescription': 'Wird vor dem Titel abgespielt.',
    'reading.voiceNoteInlineTitle': 'Sprachnotiz im Hauptteil',
    'reading.voiceNoteInlineDescription': 'Wird zwischen den Textblöcken abgespielt.',
    'reading.voiceNoteOutroTitle': 'Abschließende Sprachnotiz',
    'reading.voiceNoteOutroDescription': 'Wird unter dem Hauptteil abgespielt.',
    'reading.voiceNoteRecordAction': 'Aufnehmen',
    'reading.voiceNoteRerecordAction': 'Neu aufnehmen',
    'reading.voiceNoteRecordingStatus': 'Aufnahme',
    'chat.lockedYourGroupsMessage': 'Authentifiziere dein Konto, um deine Gruppen zu erstellen',
    'chat.lockedJoinedGroupsMessage': 'Um auf die Chats der Gruppen zuzugreifen, denen du beigetreten bist, authentifiziere dein Konto',
    'chat.hostOnlyEventHypeMessage': 'Nutzer, die deinem Kanal beitreten, sind die Einzigen, die dein Event pushen können',
    'chat.hostOnlyReadingHypeMessage': 'Nutzer, die deinem Kanal beitreten, sind die Einzigen, die deine Lesung pushen können',
    'groups.edit': 'Bearbeiten',
    'groups.delete': 'Löschen',
    'groups.deleteConfirmTitle': 'Löschung bestätigen',
    'groups.deleteConfirmBody': 'Bist du sicher, dass du diese Gruppe löschen möchtest? Alle Benutzer in dieser Gruppe werden entfernt.',
    'groups.limit': 'Einschränken',
    'groups.limited': 'Eingeschränkt',
    'groups.expel': 'Ausschließen',
    'groups.expelAndBlock': 'Ausschließen und blockieren',
    'groups.leaveGroup': 'Gruppe verlassen',
    'groups.leaveAndBlock': 'Verlassen und blockieren',
    'groups.limitUsers': 'Benutzer einschränken',
    'groups.expelUsers': 'Benutzer ausschließen',
    'groups.selectAll': 'Alle auswählen',
    'groups.deselect': 'Abwählen',
    'groups.noMembersToShow': 'Keine Mitglieder vorhanden',
    'notifications.groupJoinRequestMessage': '{user} hat dir eine Anfrage gesendet, seiner Gruppe {group} beizutreten. Möchtest du beitreten?',
    'notifications.ignore': 'Ignorieren',
    'notifications.accept': 'Akzeptieren',
    'notifications.read': 'Gelesen',
    'notifications.unread': 'Ungelesen',
    'notifications.empty': 'Du hast keine Benachrichtigungen',
    'config.title': 'Einstellungen',
    'config.accountCenter': 'Kontozentrum',
    'config.blockedUsers': 'Blockierte Benutzer',
    'config.devicePermissions': 'Geräteberechtigungen',
    'config.helpCenter': 'Hilfezentrum',
    'config.information': 'Über Keinti',
    'config.yourAccountIn': 'Dein Konto bei',
    'aboutKeinti.title': 'Über Keinti',
    'aboutKeinti.moreAboutKeinti': 'Mehr über Keinti',
    'aboutKeinti.body': `Für alle Keinti-Benutzer:

Keinti ist ein soziales Netzwerk, das sich auf die Entdeckung von Profilen und die direkte Interaktion zwischen Menschen konzentriert. Unser Ziel ist es, eine dynamische Umgebung zu bieten, die den Benutzern hilft, ihre Sichtbarkeit zu erhöhen – innerhalb von Keinti und als Ergänzung zu ihrer breiteren Online-Präsenz.

Jeder Benutzer kann ein öffentliches Profil erstellen, das aus einer Vorstellung und Containern besteht, in denen persönliche Einblicke und Aspekte der eigenen Identität geteilt werden können. Diese Struktur soll anderen Mitgliedern helfen, Interessen und Stil besser zu verstehen. Vom Startbildschirm aus fördert die Plattform die kontinuierliche Erkundung von Profilen, die von Menschen aus der ganzen Welt erstellt wurden.

Wenn ein Profil interessant ist, können Benutzer dem allgemeinen Kanal des Gastgebers/Veröffentlichers beitreten und über Gesprächsthreads interagieren. Veröffentlicher können Benutzer auch einladen, ihren Gruppen beizutreten, um ihre Gemeinschaften zu stärken und zu vergrößern.

Was die Entwicklung betrifft, wurde Keinti vollständig mit Unterstützung künstlicher Intelligenz (ChatGPT, Claude Sonnet und Gemini) über Visual Studio Code erstellt. Keinti hat einen Prozess zur Suche nach Zusammenarbeit gestartet, um Partner und spezialisierte Mitwirkende (Entwicklung, Grafikdesign und andere Rollen) willkommen zu heißen, mit dem Schwerpunkt auf der Stärkung der Sicherheit und der Verbesserung der visuellen Erfahrung des Produkts.

Für Kooperationsvorschläge kontaktiere bitte: keintisoporte@gmail.com

Mit freundlichen Grüßen,
Das Keinti-Team`,
    'verifyKeinti.objectivesTitle': 'Ziele für Keinti Verifiziert:',
    'verifyKeinti.objective1': '• Authentifiziere zuerst dein Konto.',
    'verifyKeinti.complete': 'Abschließen',
    'verifyKeinti.completed': 'Abgeschlossen',
    'verifyKeinti.objective2': '• Erhalte 100.000 Aufrufe deiner Intimitäten-Beiträge',
    'verifyKeinti.objective3': '• Erhalte in einem einzelnen Monat mindestens 1.000 Aufrufe von anderen Benutzern für die Bilder, die du in „Dein Kanal"-Chats teilst',
    'verifyKeinti.objective4': '• Bringe 100.000 Benutzer dazu, deinem „Dein Kanal"-Chat beizutreten',
    'verifyKeinti.objective5': '• Erstelle mindestens eine Gruppe in „Deine Gruppen" und bringe mindestens 200 aktive Mitglieder dazu, diesen Gruppen beizutreten.',
    'verifyKeinti.objective6': '• Veröffentliche dein Profil mindestens 40 Mal auf der „Startseite".',
    'verifyKeinti.benefitsTitle': 'Vorteile von Keinti Verifiziert:',
    'verifyKeinti.benefit1': '• Erhalte das goldene Abzeichen.',
    'verifyKeinti.benefit2': '• Generiere Einnahmen für jeden Benutzer, der auf „Dein Kanal"-Chats zugreift.',
    'verifyKeinti.benefit3': '• Generiere Einnahmen für je 1.000 Aufrufe der Bilder, die du in „Dein Kanal"-Chats teilst',
    'verifyKeinti.benefit4': '• Erhalte weitere zukünftige Vorteile, die Keinti in kommenden Updates anbietet.',
    'verifyKeinti.tabObjectives': 'Ziele',
    'verifyKeinti.tabBenefits': 'Vorteile',
    'verifyKeinti.importantNotice': 'Wichtiger Hinweis',
    'verifyKeinti.importantNoticeBody': 'Die Monetarisierung kommt bald zu Keinti. Verifiziere dein Konto, um alle Vorteile zu erhalten, sobald sie verfügbar sind.',
    'verifyKeinti.verifyAction': 'Verifizieren',
    'verifyKeinti.accountVerifiedLabel': 'Konto verifiziert',
    'devicePermissions.description': 'Hier kannst du die Berechtigungen sehen, die du der App erteilt hast.',
    'devicePermissions.galleryTitle': 'Galerie (Fotos)',
    'devicePermissions.galleryDescription': 'Wird verwendet, wenn du ein Bild zu deinem Avatar, deiner Vorstellung oder deinen Intimitäten hinzufügst.',
    'devicePermissions.statusGranted': 'Erteilt',
    'devicePermissions.statusDenied': 'Nicht erteilt',
    'devicePermissions.statusUnknown': 'Nicht verfügbar',
    'devicePermissions.iosHint': 'Unter iOS wird diese Berechtigung vom System verwaltet und beim Auswählen eines Bildes angefordert.',
    'devicePermissions.revokeConfirmTitle': 'Berechtigung entfernen',
    'devicePermissions.revokeConfirmBody': 'Bist du sicher, dass du Keinti die Berechtigung zum Zugriff auf deine Fotogalerie entziehen möchtest?',
    'devicePermissions.reEnableConfirmTitle': 'Galerie-Berechtigung',
    'devicePermissions.reEnableConfirmBody': 'Möchtest du Keinti den Zugriff auf deine Fotogalerie erlauben?',
    'devicePermissions.osRequestTitle': 'Galerie-Berechtigung',
    'devicePermissions.osRequestMessage': 'Die App benötigt Zugriff auf deine Galerie, um Fotos auszuwählen.',
    'devicePermissions.osRequestAskLater': 'Später fragen',
    'devicePermissions.osRequestDeny': 'Abbrechen',
    'devicePermissions.osRequestAllow': 'Erlauben',
    'accountCenter.title': 'Kontozentrum',
    'accountCenter.personalData': 'Persönliche Daten',
    'accountCenter.securityControl': 'Sicherheitskontrolle',
    'accountCenter.changePassword': 'Passwort ändern',
    'accountCenter.adminSelfies': 'Admin: Selfies prüfen',
    'accountCenter.closeAccount': 'Abmelden',
    'accountCenter.deleteAccount': 'Konto löschen',
    'accountCenter.deleteAccountConfirmTitle': 'Konto löschen',
    'accountCenter.deleteAccountConfirmBody': 'Bist du sicher, dass du dein Konto löschen möchtest? Diese Aktion ist unwiderruflich und entfernt deine Daten aus dem System.',
    'accountCenter.accountDeletionPolicyLink': 'Kontolöschungsrichtlinie ansehen',
    'personalData.title': 'Persönliche Daten',
    'personalData.description': 'Keinti verwendet diese Informationen, um deine Identität zu überprüfen und die Community zu schützen.',
    'personalData.contactInfo': 'Kontaktinformationen',
    'personalData.birthDate': 'Geburtsdatum',
    'personalData.gender': 'Geschlecht',
    'personalData.nationality': 'Nationalität',
    'securityControl.passwordAndAuth': 'Passwort und Authentifizierung',
    'securityControl.passwordAndAuthDescription': 'In diesem Bereich kannst du dein Profil authentifizieren und dein Passwort ändern.',
    'securityControl.accountAuth': 'Kontoauthentifizierung',
    'securityControl.privacyPolicy': 'Datenschutzrichtlinie',
    'securityControl.cookiesAdPolicy': 'Cookie- und Werberichtlinie',
    'securityControl.termsOfUse': 'Nutzungsbedingungen',
    'securityControl.childSafetyStandards': 'Kinderschutzstandards',
    'securityControl.accountDeletionPolicy': 'Kontolöschungsrichtlinie',
    'securityControl.verifyYourKeinti': 'Verifiziere dein Keinti',
    'accountAuth.description': 'Um dein Konto zu authentifizieren, musst du 2 Schritte abschließen: ein Verifizierungs-Selfie und einen Google-Authenticator-Code. Die Selfie-Überprüfung kann bis zu 24 Stunden dauern.',
    'accountAuth.step1Title': 'Schritt 1: Verifizierungs-Selfie',
    'accountAuth.step1Body': 'Mache ein Selfie, damit das Team überprüfen kann, dass du eine echte und volljährige Person bist.',
    'accountAuth.step1Hint': 'Achte auf gute Beleuchtung und rahme dein Gesicht ein.',
    'accountAuth.step1Action': 'Selfie machen',
    'accountAuth.step1Pending': 'In Prüfung…',
    'accountAuth.pendingReviewNotice': 'Dein Selfie wird noch geprüft. Diese Prüfung kann bis zu 24 Stunden dauern. Wenn nach dieser Zeit beim Tippen auf "Status aktualisieren" weiterhin "In Prüfung…" angezeigt wird, kontaktiere keintisoporte@gmail.com.',
    'accountAuth.step1Blocked': 'Blockiert',
    'accountAuth.refreshStatus': 'Status aktualisieren',
    'accountAuth.selfieAccepted': 'Selfie akzeptiert',
    'accountAuth.statusNotSubmitted': 'Status: noch nicht eingereicht.',
    'accountAuth.statusPending': 'Status: in Prüfung (bis zu 24 Stunden).',
    'accountAuth.statusAccepted': 'Status: akzeptiert. Schritt 2 verfügbar.',
    'accountAuth.statusFailed': 'Status: fehlgeschlagen. Versuche es mit besserer Beleuchtung und besserem Rahmen erneut.',
    'accountAuth.statusBlocked': 'Status: blockiert. Du kannst keine weiteren Selfies einreichen, bis ein Administrator dich entsperrt.',
    'accountAuth.errorTitle': 'Fehler',
    'accountAuth.selfieUploadError': 'Das Selfie konnte nicht hochgeladen werden. Bitte versuche es erneut.',
    'accountAuth.step2Title': 'Schritt 2: Google Authenticator',
    'accountAuth.step2Body': 'Richte Google Authenticator ein und verifiziere den Code, um die Authentifizierung abzuschließen.',
    'accountAuth.step2Locked': 'Dieser Schritt wird aktiviert, sobald das Selfie akzeptiert wurde.',
    'accountAuth.step2Ready': 'Bereit für die Einrichtung von Google Authenticator.',
    'accountAuth.step2AlreadyEnabled': 'Google Authenticator ist bereits aktiviert.',
    'accountAuth.generateTotp': 'Einrichtung generieren',
    'accountAuth.secretLabel': 'Geheimer Schlüssel',
    'accountAuth.secretHint': 'Füge ein Konto in Google Authenticator mit diesem Schlüssel hinzu (manuell). Gib dann den 6-stelligen Code ein.',
    'accountAuth.codeLabel': 'Code',
    'accountAuth.codePlaceholder': '123456',
    'accountAuth.verifyCode': 'Code verifizieren',
    'accountAuth.totpSetupError': 'Die Einrichtung konnte nicht generiert werden.',
    'accountAuth.verifyError': 'Der Code konnte nicht verifiziert werden.',
    'accountAuth.successTitle': 'Abgeschlossen',
    'accountAuth.successBody': 'Dein Konto wurde authentifiziert und das Abzeichen ist jetzt aktiv.',
    'accountAuth.completed': 'Authentifizierung abgeschlossen.',
    'accountAuth.badgeExpiresIn': 'Abzeichen läuft ab in:',
    'changePassword.requirements': 'Das Passwort muss mindestens 10 Zeichen haben und mindestens ein Sonderzeichen enthalten (#!@_$€%).',
    'changePassword.currentPassword': 'Aktuelles Passwort',
    'changePassword.currentPasswordInvalid': 'Falsches Passwort',
    'changePassword.newPassword': 'Neues Passwort',
    'changePassword.repeatNewPassword': 'Neues Passwort wiederholen',
    'changePassword.passwordsDontMatch': 'Die Passwörter stimmen nicht überein',
    'changePassword.forgot': 'Passwort vergessen?',
    'changePassword.success': 'Passwort aktualisiert',
    'changePassword.locked': 'Du hast die maximale Anzahl an Versuchen erreicht. Bitte versuche es später erneut.',
    'changePassword.attemptsRemaining': 'Verbleibende Versuche: {count}',
    'changePassword.accountLocked': 'Dein Konto wurde gesperrt. Du wirst automatisch abgemeldet.',
    'changePassword.checkCompleted': 'Prüfung abgeschlossen',
    'blockedUsers.title': 'Blockierte Benutzer',
    'blockedUsers.empty': 'Du hast keine blockierten Benutzer',
    'adminSelfies.title': 'Admin · Selfies',
    'adminSelfies.tabPending': 'Ausstehend',
    'adminSelfies.tabBlocked': 'Blockiert',
    'adminSelfies.refresh': 'Aktualisieren',
    'adminSelfies.emptyPending': 'Keine ausstehenden Selfies.',
    'adminSelfies.emptyBlocked': 'Keine blockierten Benutzer.',
    'adminSelfies.reasonPlaceholder': 'Grund (optional)',
    'adminSelfies.accept': 'Akzeptieren',
    'adminSelfies.reject': 'Ablehnen',
    'adminSelfies.block': 'Blockieren',
    'adminSelfies.unblock': 'Entsperren',
    'blockedUsers.reasonPlaceholder': 'Kein Grund',
  },
  it: {
    'language.label': 'Lingua',
    'language.spanish': 'Spagnolo',
    'language.english': 'Inglese',
    'language.french': 'Francese',
    'language.portuguese': 'Portoghese',
    'language.german': 'Tedesco',
    'language.italian': 'Italiano',
    'gender.male': 'Uomo',
    'gender.female': 'Donna',
    'gender.unspecified': 'Preferisco non specificare',
    'common.loading': 'Caricamento...',
    'common.logout': 'Esci',
    'common.notifications': 'Notifiche',
    'common.keys': 'Chiavi',
    'common.or': 'o',
    'common.unlock': 'Sblocca',
    'common.view': 'Visualizza',
    'common.forgotPassword': 'Hai dimenticato la [[password]]?',
    'common.cancel': 'Annulla',
    'common.confirm': 'Conferma',
    'common.delete': 'Elimina',
    'common.report': 'Segnala',
    'common.block': 'Blocca',
    'common.check': 'Verifica',
    'common.apply': 'Applica',
    'common.accept': 'Accetta',
    'common.create': 'Crea',
    'keys.convert': 'Converti',
    'keys.dailyGoals': 'Obiettivi giornalieri',
    'keys.enterAmount': 'Inserisci quantità',
    'keys.convertMultipleOfTenHint': 'La quantità deve essere un multiplo di 10.',
    'keys.convertInsufficientWhiteKeys': 'Non hai abbastanza chiavi bianche per questa conversione.',
    'keys.convertRateHint': 'Ogni 10 chiavi bianche si convertono in 1 chiave gradient.',
    'keys.redeem': 'Riscatta',
    'keys.redeemSubtitle': 'Seleziona una delle opzioni',
    'keys.redeemOption10': '10 euro',
    'keys.redeemOption20': '20 euro',
    'keys.redeemOption50': '50 euro',
    'keys.redeemOption100': '100 euro',
    'keys.redeemOption500': '500 euro',
    'keys.redeemInsufficientKeys': 'Non hai abbastanza chiavi.',
    'keys.redeemNote': 'Potrai riscattare solo un coupon al giorno. Scegli bene. Il denaro riscattato verrà inviato al conto PayPal che indicherai nel passaggio successivo dopo aver toccato il pulsante "Riscatta".',
    'keys.lockedMessage': 'Autentica il tuo account per accedere ai tuoi obiettivi giornalieri e alle chiavi.',
    'keys.dailyGoal1': 'Per ogni contenitore di intimità che apri, otterrai 2 chiavi bianche.',
    'keys.dailyGoal2': 'Riesci a collegare un thread con un host nel suo canale.',
    'keys.dailyGoal3': 'Sblocca le immagini che gli host condividono nei canali a cui ti unisci e ottieni 10 chiavi bianche per ciascuna.',
    'keys.dailyGoal4': 'Pubblica il tuo profilo ogni giorno.',
    'keys.dailyGoal5': 'Crea un evento nel tuo canale.',
    'keys.dailyGoal6': 'Condividi un’immagine nel tuo canale.',
    'keys.dailyGoal7': 'Fai entrare 100 utenti nel tuo canale: 1.000 chiavi bianche.',
    'keys.dailyGoal8': 'Fai entrare 1.000 utenti nel tuo canale: 10.000 chiavi bianche.',
    'keys.dailyGoal9': 'Fai entrare 10.000 utenti nel tuo canale: 100.000 chiavi bianche.',
    'auth.sessionRequiredTitle': 'Sessione richiesta',
    'auth.signInToBlockUsers': 'Accedi per bloccare gli utenti.',
    'auth.signInToReportUsers': 'Accedi per segnalare gli utenti.',
    'home.blockConfirmQuestion': 'Vuoi bloccare {user}?',
    'home.blockReasonPlaceholder': 'Motivo del blocco (facoltativo)...',
    'home.reportSelectReason': 'Seleziona un motivo per segnalare {user}.',
    'report.reason.sexualContent': 'Contenuto sessuale o nudità',
    'report.reason.harassment': 'Molestie o bullismo',
    'report.reason.offensiveLanguage': 'Linguaggio offensivo',
    'report.reason.scam': 'Truffa o frode',
    'report.reason.violence': 'Violenza o minacce',
    'report.reason.spam': 'Spam',
    'report.reason.impersonation': 'Furto d\'identità',
    'report.reason.illegalContent': 'Contenuto illegale',
    'report.reason.childSexualAbuse': 'Abuso/sfruttamento sessuale di minori',
    'report.reason.weaponsOrDrugsIncitement': 'Istigazione all\'uso di armi/droghe',
    'report.reason.inappropriateConduct': 'Comportamento inappropriato',
    'report.reason.other': 'Altro',
    'errors.unableToBlockUser': 'Impossibile bloccare l\'utente',
    'errors.unableToSendReport': 'Impossibile inviare la segnalazione',
    'toast.joinedChannelOf': 'Ti sei unito al canale di',
    'toast.published': 'Pubblicato',
    'toast.publicationNotActiveHome': 'Il post non è più attivo nella Home',
    'toast.joinedGroupMessage': 'Ti sei unito al gruppo {group} di {user}',
    'toast.blocked': 'Bloccato',
    'toast.reportSent': 'Segnalazione inviata',
    'carouselEditor.adjustImage': 'Regola l\'immagine',
    'carouselEditor.imageCount': 'Immagine {current} di {total}',
    'carouselEditor.processing': 'Elaborazione...',
    'carouselEditor.noPreview': 'Nessuna anteprima',
    'carouselEditor.editImage': 'Modifica immagine',
    'carouselEditor.errorTitle': 'Errore',
    'carouselEditor.cropErrorBody': 'Impossibile ritagliare l\'immagine. Riprova.',
    'carouselEditor.ok': 'OK',
    'login.email': 'E-mail',
    'login.password': 'Password',
    'login.emailPlaceholder': 'esempio@email.com',
    'login.passwordPlaceholder': 'Inserisci la tua password',
    'login.signIn': 'Accedi',
    'login.fillAllFields': 'Compila tutti i campi',
    'login.invalidCredentials': 'Credenziali non valide',
    'login.googleNotRegistered': 'Questo account non è ancora registrato. Registrati prima per poter accedere con questa e-mail.',
    'login.googlePkceMissing': 'Impossibile completare l\'accesso con Google. Riprova.',
    'login.noAccount': 'Non hai un account? ',
    'login.signUp': 'Registrati',
    'login.resetTitle': 'Recupera password',
    'login.resetSend': 'Invia',
    'login.resetVerify': 'Verifica',
    'login.resetChangePassword': 'Cambia password',
    'login.resetEmailNotRegistered': 'Questa e-mail non è ancora utilizzata su Keinti. Registrati.',
    'login.resetInvalidEmail': 'E-mail non valida',
    'login.resetCodeSent': 'Abbiamo inviato un codice alla tua e-mail.',
    'login.resetPasswordChanged': 'Password aggiornata. Ora puoi accedere.',
    'login.resetCodeLabel': 'Codice',
    'login.resetNewPasswordLabel': 'Crea nuova password',
    'login.resetRepeatNewPasswordLabel': 'Ripeti nuova password',
    'register.email': 'E-mail',
    'register.username': '@nomeutente',
    'register.usernameAlreadyInUse': '"{username}" è già in uso da un altro utente; prova con un altro.',
    'register.birthDate': 'Data di nascita',
    'register.birthDatePlaceholder': 'GG/MM/AAAA',
    'register.gender': 'Genere',
    'register.selectGender': 'Seleziona il tuo genere',
    'register.nationality': 'Nazionalità',
    'register.password': 'Password',
    'register.confirmPassword': 'Conferma password',
    'register.confirmPasswordPlaceholder': 'Conferma la tua password',
    'register.next': 'Avanti',
    'register.signUp': 'Registrati',
    'register.registering': 'Registrazione...',
    'register.successMessage': 'Benvenuto! Ora sei un utente Keinti.',
    'register.privacyPrefix': 'Registrandoti, accetti la ',
    'register.privacyPolicies': 'informativa sulla privacy, la politica sui cookie e pubblicità, e i termini e condizioni d\'uso',
    'register.privacyPolicyLink': 'informativa sulla privacy',
    'register.cookiesAdsPolicyLink': 'politica sui cookie e pubblicità',
    'register.termsConditionsLink': 'termini e condizioni d\'uso',
    'register.privacySeparator1': ', la ',
    'register.privacySeparator2': ', e i ',
    'register.privacySuffix': ' della nostra app Keinti. Per la tua sicurezza, leggili prima di registrarti.',
    'register.errorTitle': 'Errore di registrazione',
    'register.unableToComplete': 'Impossibile completare la registrazione.',
    'register.selectNationality': 'Seleziona la tua nazionalità',
    'register.searchCountry': 'Cerca paese...',
    'register.noCountriesFound': 'Nessun paese trovato',
    'register.emailVerificationTitle': 'Verifica e-mail',
    'register.emailVerificationHint': 'Abbiamo inviato un codice a',
    'register.emailVerificationLinkHint': 'Se ricevi un link di conferma invece di un codice, aprilo e tornerai automaticamente su Keinti per completare la registrazione.',
    'register.iConfirmedEmail': 'Ho già confermato l\'e-mail',
    'register.confirmedCheckHint': 'Se hai già aperto il link, torna su Keinti. Se non succede nulla, attendi qualche secondo e tocca di nuovo, oppure reinvia l\'e-mail.',
    'register.code': 'Codice',
    'register.codePlaceholder': 'Inserisci il codice',
    'register.timeRemaining': 'Tempo rimanente',
    'register.verifyAndSignUp': 'Verifica e registrati',
    'register.verifying': 'Verifica in corso...',
    'register.resendCode': 'Reinvia e-mail',
    'register.codeExpired': 'Il codice è scaduto. Richiedine uno nuovo.',
    'register.codeIncorrect': 'Codice errato',
    'register.codeInvalidInline': 'Il codice inserito non è valido. Riprova.',
    'register.remainingAttempts': 'Tentativi rimanenti',
    'register.remainingAttemptsInline': 'Tentativi rimanenti: {count}',
    'register.emailTemporarilyBlocked': 'Troppi tentativi. Riavvia la registrazione con un\'altra e-mail.',
    'register.emailLockedInlineError': 'Questo indirizzo e-mail è stato temporaneamente bloccato. Lo stiamo esaminando. Inviaci un reclamo per poter riattivare la tua e-mail su Keinti.',
    'register.emailLockedRetryInline': 'Questo indirizzo e-mail è stato temporaneamente bloccato. Prova con un altro o attendi la risoluzione della tua richiesta di rettifica.',
    'register.rectificationTitle': 'Rettifica',
    'register.rectificationPlaceholder': 'Descrivi la tua rettifica (max. 220 caratteri)',
    'register.rectificationSend': 'Invia reclamo',
    'register.rectificationSent': 'Reclamo inviato. Esamineremo la tua richiesta il prima possibile.',
    'register.tryLaterOrCheckEmailExists': 'Riprova più tardi o verifica che l\'e-mail esista',
    'register.mustVerifyEmailFirst': 'Devi verificare la tua e-mail prima di registrarti',
    'register.cancelledNotice': 'Registrazione annullata. Se desideri creare un nuovo account Keinti, ricomincia la procedura di registrazione.',
    'register.emailAlreadyInUse': 'Questa e-mail è già in uso da un altro utente Keinti. Prova con un\'altra.',
    'register.emailSendFailed': 'Non siamo riusciti a inviare il codice all\'e-mail "{email}". Verifica che l\'indirizzo sia valido.',
    'register.missingProfileData': 'Dati di registrazione mancanti. Torna indietro e completa nome utente, data di nascita, genere e nazionalità.',
    'register.signupExpiredNotice': 'Il tempo di verifica è scaduto. I tuoi dati di registrazione sono stati cancellati. Puoi riprovare.',
    'register.signupExpiredTitle': 'Tempo scaduto',
    'register.emailLocked24h': 'Hai esaurito tutti i {max} tentativi di registrazione con questa e-mail. Non potrai riutilizzarla per 48 ore.',
    'register.attemptsRemaining': 'Tentativi rimanenti: {remaining} di {max}',
    'register.pendingConfirmation': 'Conferma la tua e-mail prima che scada il timer – è ancora attivo.',
    'register.emailLockedTemp': 'Hai esaurito tutti i {max} tentativi di registrazione con questa e-mail. Non potrai riutilizzarla per 48 ore.',
    'register.emailLockedPermanent': 'La tua e-mail è stata bloccata permanentemente per aver superato il limite di tentativi di registrazione. Contatta keintisoporte@gmail.com per richiedere lo sblocco.',
    'validation.invalidDateFormat': 'Formato data non valido',
    'validation.invalidDate': 'Data non valida',
    'validation.mustBeAdult': 'Devi avere almeno 18 anni',
    'validation.passwordMinLength': 'La password deve avere almeno 10 caratteri',
    'validation.passwordMaxLength': 'La password non può superare i 20 caratteri',
    'validation.passwordNeedsLowercase': 'Deve contenere almeno 1 lettera minuscola',
    'validation.passwordNeedsUppercase': 'Deve contenere almeno 1 lettera maiuscola',
    'validation.passwordNeedsLetter': 'Deve contenere almeno 1 lettera',
    'validation.passwordNeedsNumber': 'Deve contenere almeno 1 numero',
    'validation.passwordNeedsSpecial': 'Deve contenere almeno 1 carattere speciale',
    'validation.passwordsDontMatch': 'Le password non corrispondono',
    'front.editProfilePhoto': 'Modifica foto profilo',
    'front.yourSocialNetworks': 'I tuoi social network',
    'front.userPlaceholder': 'Utente',
    'front.selectCategory': 'Seleziona una categoria',
    'front.noPublicationsAvailable': 'Nessun post disponibile',
    'front.forYou': 'Per te',
    'front.publicationsCount': 'Pubblicazioni',
    'front.socialNetworksSingular': 'social network',
    'front.socialNetworksPlural': 'social network',
    'front.publish': 'Pubblica',
    'front.remove': 'Rimuovi (Pubblicato)',
    'front.yourProfile': 'Il tuo profilo',
    'front.yourPresentation': 'La tua presentazione',
    'front.intimacies': 'Intimità',
    'front.time': 'Tempo',
    'front.loadingImages': 'Caricamento immagini...',
    'front.readMore': 'Leggi di più...',
    'front.readLess': 'Leggi meno',
    'front.votes': 'voti',
    'front.profileEmptyPrimaryMessage': 'Aggiungi "La tua presentazione" e almeno un\' "Intimità" per pubblicare il tuo profilo',
    'front.profileEmptySecondaryMessage': 'Pubblicando il tuo profilo, altri utenti potranno accedere al tuo canale, dove potrai condividere contenuti e interagire con loro',
    'front.profileRingHintSelect': 'Seleziona una parte di una delle immagini del carosello',
    'front.profileRingHintMaxRings': 'Massimo raggiunto: 5 anelli',
    'front.profileRingDelete': 'Elimina anello',
    'front.profileRingNameLabel': 'Nome (max. 38 caratteri)',
    'front.profileRingNameHelper': 'Prodotto, azienda, utente, canale, contenuto, ecc.',
    'front.profileRingDescriptionLabel': 'Descrizione (max. 280 caratteri)',
    'front.profileRingLinkLabel': 'Link',
    'front.profileRingLinkHelper': 'Aggiungi un link a un social network per permettere agli utenti di accedere al prodotto, azienda, utente, canale, post, contenuto, ecc.',
    'front.profileRingLocationLabel': 'Posizione',
    'front.profileRingLocationHelper': 'Aggiungi una posizione',
    'profile.deleteContentTitle': 'Elimina contenuto',
    'profile.deleteContentBody': 'Sei sicuro di voler eliminare questo contenuto dal tuo profilo?',
    'front.reactions': 'Reazioni',
    'front.deletePublicationTitle': 'Rimuovi post',
    'front.deletePublicationBody': 'Il tuo profilo è attualmente pubblico nella "Home"; rimuovendolo, solo tu potrai vederlo in "Il tuo profilo".',
    'front.category': 'Categoria',
    'front.presentationAddCarouselHint': 'Aggiungi fino a 3 immagini al carosello',
    'front.presentationTitleLabel': 'Titolo',
    'front.presentationTitlePlaceholder': 'Scrivi un titolo...',
    'front.presentationBodyLabel': 'Presentazione',
    'front.presentationBodyPlaceholder': 'Scrivi la tua presentazione...',
    'front.presentationLockedTitle': 'Modifica bloccata',
    'front.presentationLockedBody': 'Il tuo profilo è attualmente pubblicato.',
    'front.selectSocialNetwork': 'Seleziona un social network',
    'front.link': 'Link',
    'front.linkMustBeFrom': 'Il link deve essere di',
    'front.incorporate': 'Aggiungi',
    'front.answerPlaceholder': 'Risposta...',
    'front.chooseCorrectOption': 'Scegli l\'opzione corretta',
    'front.addMoreSurveyOptions': 'Aggiungi più opzioni al sondaggio',
    'front.image': 'Immagine',
    'front.text': 'Testo',
    'front.draftPlaceholder': 'Scrivi...',
    'front.draftIntimacyPlaceholder': 'Scrivi un\'intimità...',
    'front.intimaciesPublishBlocked': 'Non puoi ancora pubblicare intimità. Prima crea "La tua presentazione".',
    'front.homeSwipeTutorialHint': 'Quando la barra indicatrice nella parte superiore dello schermo è attiva, scorri verso il basso per cambiare pubblicazione.',
    'front.channelLabel': 'Canale:',
    'chat.tabYourChannel': 'Il tuo canale',
    'chat.tabChannel': 'Canale',
    'chat.tabJoined': 'Uniti',
    'chat.tabYourGroups': 'I tuoi gruppi',
    'chat.tabGroups': 'Gruppi',
    'chat.postOnHomeToActivateChannel': 'Pubblica nella Home per attivare il tuo canale',
    'chat.noChannelsYet': 'Non hai ancora canali',
    'chat.createGroup': 'Crea un gruppo',
    'chat.notJoinedAnyGroupYet': 'Non ti sei ancora unito a nessun gruppo',
    'chat.back': 'Indietro',
    'chat.interactPlaceholder': 'Interagisci...',
    'chat.timeExpired': 'Tempo scaduto',
    'chat.remainingPrefix': 'Manca',
    'chat.hour': 'ora',
    'chat.hours': 'ore',
    'chat.minute': 'minuto',
    'chat.minutes': 'minuti',
    'chat.reply': 'Rispondi',
    'chat.generalTab': 'Generale',
    'chat.yourThreadsTab': 'Thread',
    'chat.recommendationsTab': 'Raccomandazioni',
    'chat.eventsTab': 'Eventi',
    'chat.readingsTab': 'Letture',
    'chat.imagesTab': 'Immagini',
    'chat.limitWarningMessage': 'Hai già interagito nel canale di {user}. Attendi la risposta.',
    'chat.hostLimitedInteractions': '{user} ha limitato le tue interazioni',
    'chat.waitHostReplyThread': 'Attendi la risposta dell’host per continuare il thread',
    'chat.waitCreatorReplyAgain': 'Attendi la risposta dell’host prima di rispondere di nuovo',
    'chat.signInToSendMessages': 'Accedi per inviare messaggi.',
    'chat.unableToSendMessage': 'Impossibile inviare il messaggio.',
    'chat.connectionErrorSendingMessage': 'Errore di connessione durante l\'invio del messaggio.',
    'chat.addCaptionPlaceholder': 'Aggiungi una didascalia...',
    'chat.imageCaptionPlaceholder': 'Didascalia immagine...',
    'reading.screenTitle': 'Crea lettura',
    'reading.titlePlaceholder': 'Titolo',
    'reading.subtitlePlaceholder': 'Sottotitolo',
    'reading.leadPlaceholder': 'Occhiello...',
    'reading.dateLabel': 'Data',
    'reading.datePlaceholder': 'GG/MM/AAAA',
    'reading.chooseCategoryLabel': 'Scegli una categoria',
    'reading.categoryLabel': 'Categoria',
    'reading.searchCategoryPlaceholder': 'Cerca categoria',
    'reading.hypeCostLabel': 'Costo dell\'Hype',
    'reading.hypeCostInfo': 'Scegli la quantità di chiavi bianche che vuoi che ogni utente del canale ti doni per questa lettura per spingere il tuo profilo. Più chiavi ricevi, più il tuo profilo e il tuo canale si diffonderanno nella schermata Hype.',
    'reading.hypeCostAmountLabel': 'Quantità',
    'reading.bodyPlaceholder': 'Corpo...',
    'reading.intertitlePlaceholder': 'Intertitolo',
    'reading.imageLimitReachedTitle': 'Limite raggiunto',
    'reading.imageLimitReachedBody': 'Puoi aggiungere solo fino a 3 immagini nella lettura.',
    'reading.galleryPermissionTitle': 'Permesso galleria',
    'reading.galleryPermissionMessage': 'L’app ha bisogno di accedere alla tua galleria per selezionare le foto.',
    'reading.galleryPermissionAskLater': 'Chiedi più tardi',
    'reading.galleryPermissionRequiredTitle': 'Permesso richiesto',
    'reading.galleryPermissionRequiredBody': 'Devi abilitare l’accesso a Foto dalle Impostazioni per poter selezionare immagini.',
    'reading.openSettings': 'Apri impostazioni',
    'reading.imagePickErrorTitle': 'Errore',
    'reading.imagePickErrorBody': 'Non è stato possibile selezionare l’immagine. Riprova.',
    'reading.citeSelectionToastBody': 'Seleziona una parte del testo del corpo.',
    'reading.intertitleToastBody': 'Devi scrivere testo nel corpo dopo un intertitolo prima di aggiungerne un altro.',
    'reading.bodyMinSuffix': 'min',
    'reading.addImage': 'Aggiungi immagine',
    'reading.maxImagesHint': 'Massimo 3',
    'reading.selectedCitationLabel': 'Citazione selezionata',
    'reading.citedUsersTitle': 'Utenti citati',
    'reading.citedUsersInputLabel': '@utenti',
    'reading.voiceNoteIntroTitle': 'Nota iniziale',
    'reading.voiceNoteIntroDescription': 'Verrà riprodotta prima del titolo.',
    'reading.voiceNoteInlineTitle': 'Nota vocale nel corpo',
    'reading.voiceNoteInlineDescription': 'Verrà riprodotta tra i blocchi del corpo.',
    'reading.voiceNoteOutroTitle': 'Nota finale',
    'reading.voiceNoteOutroDescription': 'Verrà riprodotta sotto il corpo.',
    'reading.voiceNoteRecordAction': 'Registra',
    'reading.voiceNoteRerecordAction': 'Registra di nuovo',
    'reading.voiceNoteRecordingStatus': 'Registrazione',
    'chat.selectGroup': 'Seleziona un gruppo',
    'chat.toPrefix': 'A:',
    'chat.request': 'Richiedi',
    'chat.requestPending': 'In attesa',
    'chat.requestAccepted': 'Accettata',
    'chat.requestBlocked': 'Bloccata',
    'chat.members': 'Membri',
    'chat.messageHidden': 'Messaggio nascosto',
    'chat.messageVisible': 'Messaggio visibile',
    'event.enableTasks': 'Attiva attività',
    'event.assignTask': 'Assegna attività',
    'event.activateRequests': 'Attiva richieste',
    'event.rewardTasks': 'Ricompensa attività',
    'event.describeTask': 'Descrivi l’attività',
    'event.userPlaceholder': '@utente',
    'event.amount': 'Quantità',
    'event.type': 'Tipo di evento',
    'event.requiredSuffix': '(obbligatorio)',
    'event.name': 'Nome dell’evento',
    'event.description': 'Descrizione dell’evento',
    'event.dateAndTime': 'Data e ora',
    'event.dayMonthYear': 'Giorno/Mese/Anno',
    'event.time': 'Ora',
    'event.durationTitle': 'Durata dell’evento',
    'event.duration': 'Durata',
    'event.endsIn': 'Termina tra',
    'event.link': 'Link',
    'event.selectSocialNetwork': 'Seleziona social network',
    'event.addLink': 'Aggiungi link',
    'event.location': 'Posizione',
    'event.selectLocation': 'Seleziona posizione',
    'event.searchType': 'Cerca tipo di evento',
    'event.typeOption.social': 'Sociale',
    'event.typeOption.cultural': 'Culturale',
    'event.typeOption.artistic': 'Artistico',
    'event.typeOption.business': 'Imprenditoriale',
    'event.typeOption.economic': 'Economico',
    'event.typeOption.academic': 'Accademico',
    'event.typeOption.sports': 'Sportivo',
    'event.typeOption.entertainment': 'Intrattenimento',
    'event.typeOption.political': 'Politico',
    'event.typeOption.religious': 'Religioso',
    'event.typeOption.marketing': 'Marketing',
    'event.typeOption.charity': 'Solidale',
    'event.typeOption.audiovisual': 'Audiovisivo',
    'event.typeOption.scientific': 'Scientifico',
    'event.typeOption.technological': 'Tecnologico',
    'event.typeOption.children': 'Infantile',
    'event.typeOption.musical': 'Musicale',
    'event.typeOption.automotive': 'Automobilistico',
    'event.typeOption.fitness': 'Fitness',
    'event.typeOption.gastronomic': 'Gastronomico',
    'event.typeOption.work': 'Lavorativo',
    'chat.lockedYourGroupsMessage': 'Autentica il tuo account per creare i tuoi gruppi',
    'chat.lockedJoinedGroupsMessage': 'Per accedere alle chat dei gruppi a cui ti sei unito, autentica il tuo account',
    'chat.hostOnlyEventHypeMessage': 'Gli utenti che si uniscono al tuo canale sono gli unici che possono far crescere il tuo evento',
    'chat.hostOnlyReadingHypeMessage': 'Gli utenti che si uniscono al tuo canale sono gli unici che possono far crescere la tua lettura',
    'groups.edit': 'Modifica',
    'groups.delete': 'Elimina',
    'groups.deleteConfirmTitle': 'Conferma eliminazione',
    'groups.deleteConfirmBody': 'Sei sicuro di voler eliminare questo gruppo? Tutti gli utenti nel gruppo verranno rimossi.',
    'groups.limit': 'Limita',
    'groups.limited': 'Limitato',
    'groups.expel': 'Espelli',
    'groups.expelAndBlock': 'Espelli e blocca',
    'groups.leaveGroup': 'Esci dal gruppo',
    'groups.leaveAndBlock': 'Esci e blocca',
    'groups.limitUsers': 'Limita utenti',
    'groups.expelUsers': 'Espelli utenti',
    'groups.selectAll': 'Seleziona tutti',
    'groups.deselect': 'Deseleziona',
    'groups.noMembersToShow': 'Nessun membro da mostrare',
    'notifications.groupJoinRequestMessage': '{user} ti ha inviato una richiesta per unirti al suo gruppo {group}. Vuoi unirti?',
    'notifications.ignore': 'Ignora',
    'notifications.accept': 'Accetta',
    'notifications.read': 'Letto',
    'notifications.unread': 'Non letto',
    'notifications.empty': 'Non hai notifiche',
    'config.title': 'Impostazioni',
    'config.accountCenter': 'Centro account',
    'config.blockedUsers': 'Utenti bloccati',
    'config.devicePermissions': 'Permessi del dispositivo',
    'config.helpCenter': 'Centro assistenza',
    'config.information': 'Informazioni su Keinti',
    'config.yourAccountIn': 'Il tuo account su',
    'aboutKeinti.title': 'Informazioni su Keinti',
    'aboutKeinti.moreAboutKeinti': 'Scopri di più su Keinti',
    'aboutKeinti.body': `Per tutti gli utenti di Keinti:

Keinti è un social network incentrato sulla scoperta di profili e sull'interazione diretta tra persone. Il nostro obiettivo è offrire un ambiente dinamico che aiuti gli utenti ad aumentare la propria visibilità, all'interno di Keinti e come complemento alla loro presenza online.

Ogni utente può creare un profilo pubblico composto da una Presentazione e da contenitori dove condividere riflessioni personali e aspetti della propria identità. Questa struttura aiuta gli altri membri a comprendere meglio interessi e stile. Dalla schermata Home, la piattaforma promuove l'esplorazione continua di profili creati da persone di tutto il mondo.

Quando un profilo suscita interesse, gli utenti possono unirsi al canale generale dell'host/pubblicatore e interagire attraverso thread di conversazione. I pubblicatori possono anche invitare gli utenti a unirsi ai loro gruppi per rafforzare e far crescere le loro comunità.

Per quanto riguarda lo sviluppo, Keinti è stato costruito interamente con il supporto dell'intelligenza artificiale (ChatGPT, Claude Sonnet e Gemini) tramite Visual Studio Code. Keinti ha avviato un processo di ricerca di collaborazione per accogliere partner e collaboratori specializzati (sviluppo, design grafico e altri ruoli), con l'obiettivo prioritario di rafforzare la sicurezza e migliorare l'esperienza visiva del prodotto.

Per proposte di collaborazione, contattare: keintisoporte@gmail.com

Cordiali saluti,
Il Team Keinti`,
    'verifyKeinti.objectivesTitle': 'Obiettivi per ottenere Keinti Verificato:',
    'verifyKeinti.objective1': '• Autentica prima il tuo account.',
    'verifyKeinti.complete': 'Completa',
    'verifyKeinti.completed': 'Completato',
    'verifyKeinti.objective2': '• Ottieni 100.000 aperture sui tuoi post di intimità',
    'verifyKeinti.objective3': '• In un singolo mese, ottieni almeno 1.000 visualizzazioni da altri utenti sulle immagini che condividi nelle chat de "Il tuo canale"',
    'verifyKeinti.objective4': '• Fai unire 100.000 utenti alla chat de "Il tuo canale"',
    'verifyKeinti.objective5': '• Crea almeno un gruppo in "I tuoi gruppi" e fai in modo che almeno 200 membri attivi si uniscano a questi gruppi.',
    'verifyKeinti.objective6': '• Pubblica il tuo profilo almeno 40 volte nella "Home".',
    'verifyKeinti.benefitsTitle': 'Vantaggi di Keinti Verificato:',
    'verifyKeinti.benefit1': '• Ottieni il badge dorato.',
    'verifyKeinti.benefit2': '• Genera entrate per ogni utente che accede alle chat de "Il tuo canale".',
    'verifyKeinti.benefit3': '• Genera entrate per ogni 1.000 visualizzazioni delle immagini che condividi nelle chat de "Il tuo canale"',
    'verifyKeinti.benefit4': '• Ottieni altri privilegi futuri offerti da Keinti nei prossimi aggiornamenti.',
    'verifyKeinti.tabObjectives': 'Obiettivi',
    'verifyKeinti.tabBenefits': 'Vantaggi',
    'verifyKeinti.importantNotice': 'Avviso importante',
    'verifyKeinti.importantNoticeBody': 'La monetizzazione arriverà presto su Keinti. Verifica il tuo account per ottenere tutti i vantaggi quando saranno disponibili.',
    'verifyKeinti.verifyAction': 'Verifica',
    'verifyKeinti.accountVerifiedLabel': 'Account verificato',
    'devicePermissions.description': 'Qui puoi vedere i permessi che hai concesso all\'app.',
    'devicePermissions.galleryTitle': 'Galleria (foto)',
    'devicePermissions.galleryDescription': 'Utilizzata quando aggiungi un\'immagine al tuo avatar, alla tua presentazione o alle intimità.',
    'devicePermissions.statusGranted': 'Concesso',
    'devicePermissions.statusDenied': 'Non concesso',
    'devicePermissions.statusUnknown': 'Non disponibile',
    'devicePermissions.iosHint': 'Su iOS questo permesso è gestito dal sistema e verrà richiesto quando selezioni un\'immagine.',
    'devicePermissions.revokeConfirmTitle': 'Rimuovi permesso',
    'devicePermissions.revokeConfirmBody': 'Sei sicuro di voler rimuovere il permesso di Keinti di accedere alla tua galleria foto?',
    'devicePermissions.reEnableConfirmTitle': 'Permesso galleria',
    'devicePermissions.reEnableConfirmBody': 'Vuoi consentire a Keinti di accedere alla tua galleria foto?',
    'devicePermissions.osRequestTitle': 'Permesso galleria',
    'devicePermissions.osRequestMessage': 'L\'app ha bisogno di accedere alla tua galleria per selezionare le foto.',
    'devicePermissions.osRequestAskLater': 'Chiedi dopo',
    'devicePermissions.osRequestDeny': 'Annulla',
    'devicePermissions.osRequestAllow': 'Consenti',
    'accountCenter.title': 'Centro account',
    'accountCenter.personalData': 'Dati personali',
    'accountCenter.securityControl': 'Controllo sicurezza',
    'accountCenter.changePassword': 'Cambia password',
    'accountCenter.adminSelfies': 'Admin: revisione selfie',
    'accountCenter.closeAccount': 'Esci',
    'accountCenter.deleteAccount': 'Elimina account',
    'accountCenter.deleteAccountConfirmTitle': 'Elimina account',
    'accountCenter.deleteAccountConfirmBody': 'Sei sicuro di voler eliminare il tuo account? Questa azione è irreversibile e rimuoverà i tuoi dati dal sistema.',
    'accountCenter.accountDeletionPolicyLink': 'Visualizza la politica di eliminazione dell\'account',
    'personalData.title': 'Dati personali',
    'personalData.description': 'Keinti utilizza queste informazioni per verificare la tua identità e proteggere la comunità.',
    'personalData.contactInfo': 'Informazioni di contatto',
    'personalData.birthDate': 'Data di nascita',
    'personalData.gender': 'Genere',
    'personalData.nationality': 'Nazionalità',
    'securityControl.passwordAndAuth': 'Password e autenticazione',
    'securityControl.passwordAndAuthDescription': 'In questa sezione puoi autenticare il tuo profilo e cambiare la password.',
    'securityControl.accountAuth': 'Autenticazione dell\'account',
    'securityControl.privacyPolicy': 'Informativa sulla privacy',
    'securityControl.cookiesAdPolicy': 'Politica sui cookie e pubblicità',
    'securityControl.termsOfUse': 'Termini di utilizzo',
    'securityControl.childSafetyStandards': 'Standard di sicurezza dei minori',
    'securityControl.accountDeletionPolicy': 'Politica di eliminazione dell\'account',
    'securityControl.verifyYourKeinti': 'Verifica il tuo Keinti',
    'accountAuth.description': 'Per autenticare il tuo account devi completare 2 passaggi: un selfie di verifica e un codice Google Authenticator. La revisione del selfie può richiedere fino a 24 ore.',
    'accountAuth.step1Title': 'Passaggio 1: selfie di verifica',
    'accountAuth.step1Body': 'Scatta un selfie affinché il team possa verificare che sei una persona reale e maggiorenne.',
    'accountAuth.step1Hint': 'Assicurati di avere una buona illuminazione e di inquadrare il viso.',
    'accountAuth.step1Action': 'Scatta selfie',
    'accountAuth.step1Pending': 'In revisione…',
    'accountAuth.pendingReviewNotice': 'Il tuo selfie è ancora in revisione. Questa verifica può richiedere fino a 24 ore. Se, trascorso questo tempo, toccando "Aggiorna stato" continua a comparire "In revisione…", contatta keintisoporte@gmail.com.',
    'accountAuth.step1Blocked': 'Bloccato',
    'accountAuth.refreshStatus': 'Aggiorna stato',
    'accountAuth.selfieAccepted': 'Selfie accettato',
    'accountAuth.statusNotSubmitted': 'Stato: non ancora inviato.',
    'accountAuth.statusPending': 'Stato: in revisione (fino a 24 ore).',
    'accountAuth.statusAccepted': 'Stato: accettato. Passaggio 2 disponibile.',
    'accountAuth.statusFailed': 'Stato: fallito. Riprova con migliore illuminazione e inquadratura.',
    'accountAuth.statusBlocked': 'Stato: bloccato. Non puoi più inviare selfie finché un amministratore non ti sblocca.',
    'accountAuth.errorTitle': 'Errore',
    'accountAuth.selfieUploadError': 'Impossibile caricare il selfie. Riprova.',
    'accountAuth.step2Title': 'Passaggio 2: Google Authenticator',
    'accountAuth.step2Body': 'Configura Google Authenticator e verifica il codice per completare l\'autenticazione.',
    'accountAuth.step2Locked': 'Questo passaggio verrà attivato quando il selfie sarà accettato.',
    'accountAuth.step2Ready': 'Pronto per configurare Google Authenticator.',
    'accountAuth.step2AlreadyEnabled': 'Google Authenticator è già attivato.',
    'accountAuth.generateTotp': 'Genera configurazione',
    'accountAuth.secretLabel': 'Chiave segreta',
    'accountAuth.secretHint': 'Aggiungi un account in Google Authenticator usando questa chiave (manuale). Poi inserisci il codice a 6 cifre.',
    'accountAuth.codeLabel': 'Codice',
    'accountAuth.codePlaceholder': '123456',
    'accountAuth.verifyCode': 'Verifica codice',
    'accountAuth.totpSetupError': 'Impossibile generare la configurazione.',
    'accountAuth.verifyError': 'Impossibile verificare il codice.',
    'accountAuth.successTitle': 'Completato',
    'accountAuth.successBody': 'Il tuo account è stato autenticato e il badge è ora attivo.',
    'accountAuth.completed': 'Autenticazione completata.',
    'accountAuth.badgeExpiresIn': 'Il badge scade tra:',
    'changePassword.requirements': 'La password deve avere almeno 10 caratteri e includere almeno un carattere speciale (#!@_$€%).',
    'changePassword.currentPassword': 'Password attuale',
    'changePassword.currentPasswordInvalid': 'Password errata',
    'changePassword.newPassword': 'Nuova password',
    'changePassword.repeatNewPassword': 'Ripeti nuova password',
    'changePassword.passwordsDontMatch': 'Le password non corrispondono',
    'changePassword.forgot': 'Hai dimenticato la password?',
    'changePassword.success': 'Password aggiornata',
    'changePassword.locked': 'Hai raggiunto il numero massimo di tentativi. Riprova più tardi.',
    'changePassword.attemptsRemaining': 'Tentativi rimanenti: {count}',
    'changePassword.accountLocked': 'Il tuo account è stato bloccato. Verrai disconnesso automaticamente.',
    'changePassword.checkCompleted': 'Verifica completata',
    'blockedUsers.title': 'Utenti bloccati',
    'blockedUsers.empty': 'Non hai utenti bloccati',
    'adminSelfies.title': 'Admin · Selfie',
    'adminSelfies.tabPending': 'In attesa',
    'adminSelfies.tabBlocked': 'Bloccati',
    'adminSelfies.refresh': 'Aggiorna',
    'adminSelfies.emptyPending': 'Nessun selfie in attesa.',
    'adminSelfies.emptyBlocked': 'Nessun utente bloccato.',
    'adminSelfies.reasonPlaceholder': 'Motivo (facoltativo)',
    'adminSelfies.accept': 'Accetta',
    'adminSelfies.reject': 'Rifiuta',
    'adminSelfies.block': 'Blocca',
    'adminSelfies.unblock': 'Sblocca',
    'blockedUsers.reasonPlaceholder': 'Nessun motivo',
  },
};

export function isSupportedLanguage(input: unknown): input is Language {
  const raw = (input ?? '').toString().trim().toLowerCase();
  return (SUPPORTED_LANGUAGES as readonly string[]).includes(raw);
}

export function normalizeLanguage(input: unknown): Language {
  const raw = (input ?? '').toString().trim().toLowerCase();
  return isSupportedLanguage(raw) ? raw : 'es';
}
